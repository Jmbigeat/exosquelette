...
export function OnboardingFlow({ onComplete }) {
  ...
  var targetRole = roleSt[0];
  var setTargetRole = roleSt[1];
  var profileSt = useState("");
  var profileText = profileSt[0];
  var setProfileText = profileSt[1];
  var offersSt = useState("");
  var offersText = offersSt[0];
  var setOffersText = offersSt[1];
  var previousRoleSt = useState("");
  var previousRole = previousRoleSt[0];
  var setPreviousRole = previousRoleSt[1];
  var progressSt = useState(0);
  var scanProgress = progressSt[0];
  var setScanProgress = progressSt[1];
  var messagesSt = useState([]);
  var scanMessages = messagesSt[0];
  var setScanMessages = messagesSt[1];
  var signalsSt = useState(null);
  var offerSignals = signalsSt[0];
  var setOfferSignals = signalsSt[1];
  var seedsSt = useState([]);
  var seeds = seedsSt[0];
  var setSeeds = seedsSt[1];
      ...
      <div style={{ padding: "8px 0" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 11, color: canNext ? "#495670" : "#e94560", marginTop: 4, textAlign: "right" }}>
            {canNext
              ? profileText.trim().length + " caractères"
              : "Minimum 20 caractères (" + profileText.trim().length + "/20)"}
          </div>
        </div>

        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 12, color: "#8892b0", fontWeight: 600, marginBottom: 6 }}>
            Ton rôle actuel ou dernier rôle en poste <span style={{ fontSize: 10, color: "#495670" }}>(optionnel)</span>
          </div>
          <input
            value={previousRole}
            onChange={function (e) {
              setPreviousRole(e.target.value);
            }}
          ...
          onClick={function () {
            ...
            if (!previousRole.trim()) {
              ...
              for (var i = 0; i < ROLE_CLUSTERS.length; i++) {
                var words = ROLE_CLUSTERS[i].label
                  .toLowerCase()
                  .split(/[\s\/]+/)
                  .filter(function (w) {
                    return w.length > 3;
                ...
                if (found) {
                  setPreviousRole(ROLE_CLUSTERS[i].label.split(" / ")[0]);
                  break;
                }
              }
            }
  ...
  if (phase === "offers") {
    ...
    return (
      <div style={{ padding: "8px 0" }}>
        ...
        <button onClick={goToScan} style={btnPrimary(true)}>
          Suivant
        </button>
  ...
  if (phase === "ready") {
    ...
    var detectedCount = offerSignals
      ...
      : 0;
    var scorePct =
      offerSignals && offerSignals.totalSignals > 0 ? Math.min(45, 20 + offerSignals.totalSignals * 3) : 32;
    ...
    return (
      <div style={{ padding: "8px 0" }}>
        ...
        <button onClick={handleStart} style={btnPrimary(true)}>
          Commencer la Forge
        </button>
