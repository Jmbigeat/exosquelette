...
import {
  analyzeTakeDepth,
  auditBrickVulnerability,
  getBrickFields,
  matchKpiToReference,
  takeToiPillar,
} from "@/lib/sprint/bricks";
...
var SIDE_PROJECT_OWNERSHIP =
  /sur\s+mon\s+temps\s+libre|à\s+titre\s+personnel|sans\s+employeur|sans\s+client|en\s+dehors\s+du\s+boulot|mon\s+projet|ma\s+passion|j'ai\s+créé\s+seul|de\s+zéro\s+sans\s+équipe/i;
var SIDE_PROJECT_TYPE =
  /side.?project|projet\s+perso|open.?source|bénévolat|association|blog|app|outil|contribution\s+libre/i;
...
export function FeedbackToast({ brick, onDone }) {
  ...
  var catLabel =
    brick.brickType === "cicatrice"
      ? "Cicatrice"
      : brick.brickType === "take"
        ? "Prise de position"
        : brick.brickCategory && CATEGORY_LABELS[brick.brickCategory]
          ? CATEGORY_LABELS[brick.brickCategory].label
          : "";
  var isHard =
    brick.brickCategory === "decision" || brick.brickCategory === "influence" || brick.brickType === "cicatrice";
  return (
    <div
      style={{
        position: "fixed",
        ...
        background: isHard ? "#0f3460" : "#0f3460",
        border: isHard ? "2px solid #4ecca3" : "1px solid #e94560",
        borderRadius: 12,
        padding: "16px 20px",
        maxWidth: 420,
        width: "90%",
    ...
    >
      ...
      {brick.type !== "mission" && brick.usedIn && (
        <div>
          ...
          {brick.usedIn.map(function (u, i) {
            return (
              <div key={i} style={{ fontSize: 12, color: "#8892b0", paddingLeft: 8, marginBottom: 2 }}>
                {"→"} {u}
              </div>
            );
          })}
      ...
      {isHard && brick.type !== "mission" && (
        <div style={{ fontSize: 11, color: "#4ecca3", marginTop: 8, borderTop: "1px solid #16213e", paddingTop: 6 }}>
          {brick.brickType === "cicatrice"
            ? "Échec assumé. C'est la brique la plus rare de ton Score."
            : brick.brickCategory === "decision"
              ? "Arbitrage documenté. Aucun outil de CV ne produit ça."
              : "Influence prouvée. Le recruteur ne peut pas tester ça autrement qu'en entretien."}
        </div>
      )}
    </div>
  );
...
export function AddBrick({ onAdd }) {
  ...
  if (!isOpen) {
    return (
      ...
      >
        <div>
          {"\u2795"} L'IA a rat{"é"} quelque chose ?
        </div>
        <div style={{ fontSize: 11, fontWeight: 400, color: "#495670", marginTop: 4 }}>
          D{"é"}cris-le en 1 phrase, je le structure pour toi.
        </div>
      </button>
    );
  }
  return (
    <div style={{ background: "#0f3460", borderRadius: 10, padding: 16, marginTop: 16 }}>
      <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, letterSpacing: 1, marginBottom: 10 }}>
        AJOUTER UNE BRIQUE
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                fontSize: 11,
                fontWeight: 700,
                background: act ? cat.color + "22" : "#1a1a2e",
                color: act ? cat.color : "#495670",
      ...
      <textarea
        ...
        placeholder={
          category === "chiffre"
            ? "Ex : J'ai formé 12 commerciaux, 8 ont atteint leur quota en 4 mois."
            : category === "decision"
              ? "Ex : Le board voulait X, j'ai choisi Y parce que Z."
              : "Ex : 3 directeurs bloquaient, j'ai aligné tout le monde en montrant le coût d'opportunité."
        }
        style={{
          width: "100%",
          minHeight: 70,
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button
          ...
          style={{
            ...
            border: "none",
            borderRadius: 8,
            cursor: text.trim().length >= 10 ? "pointer" : "default",
            fontWeight: 700,
            fontSize: 13,
          }}
        >
...
export function Interrogation({
  ...
  var missionItems = bricks.filter(function (b) {
    return b.type === "mission";
  });
  var processed = seeds.filter(function (s) {
    return bricks.some(function (b) {
      return b.id === s.id;
    });
  });
  var pending = seeds.filter(function (s) {
    return !bricks.some(function (b) {
      return b.id === s.id;
    });
  });
      ...
      <div>
        <div style={{ textAlign: "center", padding: 24 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: "#ccd6f6", marginBottom: 6 }}>
            {validated.length} brique{validated.length > 1 ? "s" : ""} + {missionItems.length} mission
            {missionItems.length > 1 ? "s" : ""}
          </div>
          <div style={{ fontSize: 13, color: "#8892b0", lineHeight: 1.6 }}>
            {missionItems.length > 0
              ? "Les missions deviennent des briques quand tu apportes la preuve."
              : "Elles alimenteront ton CV, tes réponses d'entretien et tes posts LinkedIn automatiquement."}
          </div>
        </div>
  ...
  if (phase === "forging") {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#ccd6f6", marginBottom: 8 }}>
  ...
  if (phase === "triage") {
    ...
    return (
      <div>
        ...
        >
          <div style={{ fontSize: 11, color: "#3498db", fontWeight: 600, marginBottom: 8, letterSpacing: 1 }}>
            TRIAGE
          </div>
          <div style={{ fontSize: 14, color: "#ccd6f6", lineHeight: 1.6, marginBottom: 12 }}>
            Ta réponse ne contient pas de chiffre.
            {hasDecision || hasInfluence
              ? " Mais l'IA détecté des marqueurs de " +
                (hasDecision ? "decision" : "") +
                (hasDecision && hasInfluence ? " et d'" : "") +
                (hasInfluence ? "influence" : "") +
                "."
              : ""}
            {!hasDecision && !hasInfluence && hasRichContent
              ? " Mais ta réponse est riche (" + answer.trim().split(" ").length + " mots)."
              : ""}
            {!hasDecision && !hasInfluence && !hasRichContent
              ? " L'IA n'a pas détecté de marqueurs de décision ou d'influence."
              : ""}
          </div>
          <div style={{ background: "#0f3460", borderRadius: 8, padding: 10, marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: "#495670", fontWeight: 600, marginBottom: 4 }}>
              CE QUE L'IA A DÉTECTÉ DANS TA RÉPONSE
            </div>
            <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5 }}>
              ...
              {hasDecision && (
                <span>
                  {" "}
                  {"·"} Marqueurs de décision : <span style={{ color: "#9b59b6" }}>oui</span>
                </span>
              )}
              {hasInfluence && (
                <span>
                  {" "}
                  {"·"} Marqueurs d'influence : <span style={{ color: "#3498db" }}>oui</span>
                </span>
              )}
              {!hasDecision && (
                <span>
                  {" "}
                  {"·"} Marqueurs de décision : <span style={{ color: "#495670" }}>non</span>
                </span>
              )}
              {!hasInfluence && (
                <span>
                  {" "}
                  {"·"} Marqueurs d'influence : <span style={{ color: "#495670" }}>non</span>
                </span>
              )}{" "}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#8892b0", fontWeight: 400, marginTop: 2 }}>
              L'IA structure ta réponse sans chiffre. La brique sera moins précise mais existera.
            </div>
          </button>
          <button
            onClick={function () {
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#8892b0", fontWeight: 400, marginTop: 2 }}>
              L'IA te donne les étapes pour récupérer la preuve. La brique sera forgée quand tu reviendras.
            </div>
          </button>
          <button
            onClick={function () {
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#495670", fontWeight: 400, marginTop: 2 }}>
              Tu repars de zéro sur cette question.
            </div>
          </button>
        </div>
      </div>
  ...
  if (phase === "mission") {
    ...
    var brickCount = bricks.filter(function (b) {
      return b.status === "validated";
    }).length;
    ...
    return (
      <div>
        ...
        >
          <div style={{ fontSize: 11, color: "#ff9800", fontWeight: 600, marginBottom: 8, letterSpacing: 1 }}>
            MISSION ASSIGNÉE
          </div>
          ...
          <div style={{ background: "#0f3460", borderRadius: 8, padding: 10, marginBottom: 10 }}>
            <div style={{ fontSize: 11, color: "#495670", fontWeight: 600, marginBottom: 4 }}>
              POURQUOI UNE MISSION ET PAS UNE BRIQUE
            </div>
            <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5 }}>
              Ta réponse ne contient pas de chiffre. Sans chiffre, l'IA forge du vent. Un recruteur détecté la
              différence entre une preuve et une impression.
            </div>
          </div>
          {isHighMissions && (
            <div style={{ background: "#0f3460", borderRadius: 8, padding: 10, borderLeft: "3px solid #e94560" }}>
              ...
              <div style={{ fontSize: 12, color: "#ccd6f6", lineHeight: 1.6 }}>
                C'est ta {missionCount}e mission sur {missionCount + brickCount} questions. {missionRatio}% de tes
                réponses n'ont pas de chiffre. Ce n'est pas un problème de mémoire. C'est un mode de fonctionnement : tu
                opères sans mesurer l'impact de ce que tu fais.
              </div>
              <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.6, marginTop: 6 }}>
                Chaque mission que tu complètes ne remplit pas seulement ton arsenal. Elle installe un réflexe : mesurer
                ce que tu fais pendant que tu le fais. Le professionnel qui négocie avec des preuves fixe son prix.
                Celui qui négocie avec des impressions accepte celui qu'on lui donne.
              </div>
            </div>
          )}
        </div>
    ...
    return (
      <div>
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5 }}>
            Reformule ta réponse en incluant ta part de responsabilité.
          </div>
        </div>
        <button
          onClick={function () {
  ...
  if (phase === "correcting") {
    return (
      <div>
        <div style={{ fontSize: 11, color: "#9b59b6", fontWeight: 600, marginBottom: 8, letterSpacing: 1 }}>
          RETOUCHE DE LA BRIQUE
        </div>
        <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5, marginBottom: 12 }}>
          Modifie le texte. Chaque correction enseigne ta voix à l'IA. Le Style Engine apprend de tes choix, pas de tes
          validations.
        </div>
        <textarea
          value={editText}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            style={{
              flex: 1,
              padding: 14,
              background: editText.trim().length >= 10 ? "linear-gradient(135deg, #9b59b6, #8e44ad)" : "#1a1a2e",
              color: editText.trim().length >= 10 ? "#fff" : "#495670",
              border: "none",
              borderRadius: 10,
              cursor: editText.trim().length >= 10 ? "pointer" : "default",
              fontWeight: 700,
              fontSize: 14,
            }}
          >
  ...
  if (phase === "anon_review" || phase === "anon_review_correct") {
    ...
    if (hasSensitive) {
      var sorted = audit.findings.slice().sort(function (a, b) {
    ...
    var typeLabels = {
      ...
      marqueur_secteur: "Marqueur secteur",
    };
    ...
    function handleAnonConfirm() {
      ...
      var reviewedSeed = Object.assign({}, seed, {
        generatedText: effectiveText,
        structuredFields: seed.structuredFields || buildStructuredFields(seed),
        sideProject: detectSideProject(effectiveText),
        anonymizedText: anonEdit.trim(),
        anonAuditTrail: auditTrail,
        advocacyText: generateAdvocacyText(effectiveText, seed.brickCategory, seed.type, seed.nightmareText),
        internalAdvocacy: generateInternalAdvocacy(effectiveText, seed.brickCategory, seed.type, seed.elasticity),
      });
      if (isCorrection) {
      ...
      <div>
        ...
        <div
          style={{
            ...
            padding: 20,
            marginBottom: 16,
            borderLeft: "3px solid " + audit.confidenceColor,
          }}
        >
          ...
          <div style={{ background: "#0f3460", borderRadius: 8, padding: 10, marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: "#495670", fontWeight: 600, marginBottom: 6 }}>
              RAPPORT D'AUDIT — {audit.passesClean}/{audit.passesTotal} PASSES PROPRES
            </div>
            {audit.passes.map(function (p, i) {
              var clean = p.findings.length === 0;
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                  <span style={{ fontSize: 11, color: clean ? "#4ecca3" : "#e94560" }}>
                    {clean ? "\u2705" : "\u274C"}
                  </span>
                  ...
                  {!clean && (
                    <span style={{ fontSize: 10, color: "#e94560" }}>
                      ({p.findings.length} élément{p.findings.length > 1 ? "s" : ""})
                    </span>
                  )}
                </div>
              );
          ...
          <div style={{ fontSize: 13, color: "#ccd6f6", lineHeight: 1.6, marginBottom: 12 }}>
            {hasSensitive
              ? audit.totalFindings +
                " élément" +
                (audit.totalFindings > 1 ? "s" : "") +
                " détecté" +
                (audit.totalFindings > 1 ? "s" : "") +
                " sur 3 passes. Vérifie et corrige avant d'archiver."
          ...
          <div style={{ fontSize: 11, color: "#8892b0", marginBottom: 6 }}>
            Édite la version transportable ci-dessous :
          </div>
          <textarea
            value={anonEdit}
            onChange={function (e) {
              setAnonEdit(e.target.value);
            }}
            style={{
              width: "100%",
              minHeight: 80,
              padding: 12,
              background: "#0f3460",
              border: "2px solid " + audit.confidenceColor,
              borderRadius: 10,
              color: "#ccd6f6",
              fontSize: 13,
              lineHeight: 1.6,
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            onClick={function () {
              setPhase(isCorrection ? "correcting" : "review");
              setAnonEdit("");
              setAnonAudit(null);
            }}
            style={{
  ...
  if (phase === "review") {
    ...
    if (seed.type === "take") {
      var takeAnalysis =
        verbData && verbData.takeAnalysis ? verbData.takeAnalysis : analyzeTakeDepth(answer, seed.surfacePatterns);
        ...
        <div>
          <div style={{ background: "#0f3460", borderRadius: 10, padding: 20, marginBottom: 16 }}>
            <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: "#3498db", fontWeight: 600, letterSpacing: 1 }}>
                PRISE DE POSITION {takeAnalysis.level === "deep" ? "FORGÉE" : "DÉTECTÉE"}
              </span>
              {(function () {
                var ready =
            ...
            <div style={{ fontSize: 14, color: "#ccd6f6", lineHeight: 1.7, marginBottom: 14, fontStyle: "italic" }}>
              "{answer}"
            </div>
            ...
            <div
              style={{
                ...
                padding: 10,
                marginBottom: 10,
                borderLeft: "3px solid " + (takeAnalysis.level === "deep" ? "#4ecca3" : "#ff9800"),
              }}
            >
              <div
                style={{
                  fontSize: 11,
                  color: takeAnalysis.level === "deep" ? "#4ecca3" : "#ff9800",
                  fontWeight: 600,
                  marginBottom: 4,
                }}
              >
              ...
              <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5 }}>
                {takeAnalysis.level === "deep"
                  ...
                  : "Ta position est là. Mais elle manque de " +
                    (takeAnalysis.foundDepth.indexOf("contrarian") === -1
                      ? "contrepoint (qu'est-ce que les autres pensent à tort ?)"
                      : takeAnalysis.foundDepth.indexOf("personal") === -1
                        ? "vécu (qu'est-ce que TU as vu que les autres n'ont pas vu ?)"
                        : takeAnalysis.foundDepth.indexOf("causal") === -1
                          ? "logique causale (pourquoi c'est vrai ?)"
                          : "spécificité (donne un exemple concret).")}
              </div>
              {takeAnalysis.foundDepth.length > 0 && (
                <div style={{ fontSize: 10, color: "#495670", marginTop: 4 }}>
                  Marqueurs détectés : {takeAnalysis.foundDepth.join(", ")}
                </div>
              )}
            </div>
            ...
            <div style={{ background: "#1a1a2e", borderRadius: 8, padding: 10, borderLeft: "3px solid #3498db" }}>
              ...
              <div style={{ fontSize: 13, color: "#ccd6f6", fontWeight: 700, marginBottom: 2 }}>
                {pillarPreview.title}
              </div>
    ...
    var computedInternalAdvocacy = generateInternalAdvocacy(
      effectiveText,
      seed.brickCategory,
      seed.type,
      seed.elasticity
      ...
      <div>
        <div style={{ background: "#0f3460", borderRadius: 10, padding: 20, marginBottom: 16 }}>
          <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
            <span
              style={{
                fontSize: 11,
                color: seed.type === "cicatrice" ? "#ff9800" : cat ? cat.color : "#e94560",
                fontWeight: 600,
                letterSpacing: 1,
              }}
            >
            ...
            {cat && (
              <span
                style={{ fontSize: 10, color: cat.color, background: "#1a1a2e", padding: "2px 8px", borderRadius: 10 }}
              >
                {cat.label}
              </span>
            )}
          ...
          <div style={{ fontSize: 15, color: "#ccd6f6", lineHeight: 1.6, marginBottom: 14 }}>
            &quot;{effectiveText}&quot;
          </div>
          ...
          {kpiMatch && (
            <div
              style={{
                ...
                borderLeft:
                  "3px solid " +
                  (isSousPression ? "#e94560" : kpiMatch.elasticity === "élastique" ? "#4ecca3" : "#8892b0"),
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <span
                  style={{
                    fontSize: 11,
                    color: isSousPression ? "#e94560" : kpiMatch.elasticity === "élastique" ? "#4ecca3" : "#8892b0",
                    fontWeight: 600,
                  }}
                >
                  {isSousPression
                    ? "\uD83D\uDEA8 KPI SOUS PRESSION"
                    : kpiMatch.elasticity === "élastique"
                      ? "↗\uFE0F KPI ÉLASTIQUE"
                      : "↔\uFE0F KPI STABLE"}
                </span>
                <span
                  style={{ fontSize: 10, color: "#495670", background: "#0f3460", padding: "2px 8px", borderRadius: 8 }}
                >
                  {kpiMatch.name}
                </span>
              </div>
              ...
              {isSousPression && (
                <div
                  style={{
                    background:
                      (seed.generatedText && hasDecisionMarkers(seed.generatedText) ? "#ff9800" : "#e94560") + "22",
                    borderRadius: 6,
                    padding: 8,
                    marginTop: 6,
                  }}
                >
                  <div
                    style={{
                      fontSize: 12,
                      color: seed.generatedText && hasDecisionMarkers(seed.generatedText) ? "#ff9800" : "#e94560",
                      fontWeight: 600,
                      lineHeight: 1.5,
                    }}
                  >
            ...
            (function () {
              var matchedCauchemar = getActiveCauchemars().filter(function (c) {
                return c.kpis.some(function (k) {
                  return seed.kpi.indexOf(k) !== -1 || k.indexOf(seed.kpi) !== -1;
                });
              })[0];
              ...
              return (
                <div
                  style={{
                    ...
                    padding: 10,
                    marginBottom: 10,
                    borderLeft: "3px solid " + (isSousPression ? "#e94560" : "#ff9800"),
                  }}
                >
                  <div
                    style={{
                      fontSize: 11,
                      color: isSousPression ? "#e94560" : "#ff9800",
                      fontWeight: 600,
                      marginBottom: 4,
                    }}
                  >
                    {isSousPression ? "\uD83D\uDEA8 BLUFF CRITIQUE" : "\u26A0\uFE0F LEVIER DE NÉGOCIATION"} —{" "}
                    {matchedCauchemar.label.toUpperCase()}
                  </div>
                  <div style={{ fontSize: 12, color: "#ccd6f6", lineHeight: 1.5, marginBottom: 6 }}>
                    Ce cauchemar coûte {costStr}
                    {"€"}/{matchedCauchemar.costUnit} dans le secteur. {matchedCauchemar.costContext}
                  </div>
                  {isSousPression ? (
                    <div
                      style={{
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#ff9800", lineHeight: 1.5 }}>
                        Si tu revendiques cette solution et que le problème persiste après ton arrivée, tu deviens la
                        cible. Ta preuve doit être reproductible.
                      </div>
                    </div>
                  )}
                </div>
              ...
              return (
                <div
                  style={{
                    ...
                    padding: 10,
                    marginBottom: 10,
                    borderLeft: "3px solid " + vuln.color,
                  }}
                >
                  ...
                  {vuln.level === "vulnerable" && (
                    <div style={{ fontSize: 11, color: "#e94560", marginTop: 6, lineHeight: 1.4 }}>
                      Corrige cette brique avant de l'archiver. Le bouton "Retoucher" te permet de l'enrichir.
                    </div>
                  )}
                </div>
              );
          ...
          {seed.type === "cicatrice" && answer && (
            ...
            >
              ...
              <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5 }}>
                Tu viens de raconter un échec. Le système le transforme en preuve de maturité. 95% des candidats mentent
                ou esquivent. Tu te sépares d'eux.
              </div>
            </div>
          )}
          ...
          {computedAdvocacy &&
            (function () {
              var isHard =
                seed.brickCategory === "decision" || seed.brickCategory === "influence" || seed.type === "cicatrice";
              return (
                <div
                  style={{
                    background: isHard ? "#0f3460" : "#1a1a2e",
                    borderRadius: isHard ? 10 : 8,
                    padding: isHard ? 16 : 10,
                    borderLeft: "3px solid #4ecca3",
                    marginBottom: 10,
                    boxShadow: isHard ? "0 2px 12px rgba(78,204,163,0.15)" : "none",
                  }}
                >
                  <div
                    style={{
                      fontSize: isHard ? 12 : 11,
                      color: "#4ecca3",
                      fontWeight: 600,
                      marginBottom: isHard ? 8 : 4,
                    }}
                  >
                    {isHard
                      ? "\uD83C\uDFAF CE QUE TON INTERVIEWEUR DIRA A SON DIRECTEUR"
                      : "CE QUE TON INTERVIEWEUR DIRA A SON DIRECTEUR"}
                  </div>
                  <div style={{ fontSize: isHard ? 14 : 12, color: "#ccd6f6", lineHeight: 1.6, fontStyle: "italic" }}>
                    "{computedAdvocacy}"
                  </div>
                  {isHard && (
          ...
          {computedInternalAdvocacy && trajectoryToggle === "j_y_suis" && (
            ...
            >
              <div style={{ fontSize: 11, color: "#3498db", fontWeight: 600, marginBottom: 4 }}>
                CE QUE TON N+1 PERD SI TU PARS
              </div>
          ...
          {seed.controlRisk && (
            ...
            >
              <div style={{ fontSize: 11, color: "#ff9800", fontWeight: 600, marginBottom: 4 }}>
                {"\u26A0\uFE0F"} AUDIT DE CONTRÔLE
              </div>
              <div style={{ fontSize: 12, color: "#ccd6f6", lineHeight: 1.5 }}>{seed.controlRisk}</div>
              <div style={{ fontSize: 11, color: "#e94560", lineHeight: 1.5, marginTop: 6 }}>
                Si tu te positionnes comme le remède et que le problème persiste, tu deviens la cible. Ta brique doit
                décrire ce que TU contrôles, pas ce que ton équipe a fait.
              </div>
            </div>
          )}
          ...
          {seed.nightmareText && (
            ...
            >
              <div style={{ fontSize: 11, color: "#e74c3c", fontWeight: 600, marginBottom: 4 }}>
                LE CAUCHEMAR QUE TU RÉSOUS
              </div>
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                ...
                <span
                  style={{ fontSize: 10, color: "#495670", background: "#0f3460", padding: "2px 8px", borderRadius: 8 }}
                >
                  {verbData.foundProcess.length + verbData.foundAvoidance.length} verbe
                  {verbData.foundProcess.length + verbData.foundAvoidance.length > 1 ? "s" : ""} signalé
                  {verbData.foundProcess.length + verbData.foundAvoidance.length > 1 ? "s" : ""}
                </span>
              </div>
              {verbData.foundProcess.length > 0 && (
                <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5, marginBottom: 4 }}>
                  Verbes de processus détectés :{" "}
                  <span style={{ color: "#e94560" }}>{verbData.foundProcess.join(", ")}</span>. Le recruteur lit : rôle
                  secondaire.
                </div>
              )}
              {verbData.foundAvoidance.length > 0 && (
                <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5, marginBottom: 4 }}>
                  Verbes d'évitement détectés :{" "}
                  <span style={{ color: "#e94560" }}>{verbData.foundAvoidance.join(", ")}</span>. Le recruteur lit : pas
                  d'engagement.
                </div>
              )}
              {verbData.foundResult.length > 0 && (
                <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5, marginBottom: 4 }}>
                  Verbes de résultat : <span style={{ color: "#4ecca3" }}>{verbData.foundResult.join(", ")}</span>. Le
                  recruteur lit : acteur.
                </div>
              )}
              <button
                onClick={function () {
                  setVerbDismissed(true);
                }}
                style={{
                  padding: "4px 10px",
                  fontSize: 11,
          ...
          {verbDismissed && (
            ...
            >
              <div style={{ fontSize: 11, color: "#495670", fontWeight: 600 }}>
                LANGAGE : alerte écartée par toi. L'IA en prend note.
              </div>
            </div>
          )}
          ...
          {seed.elasticityNote && (
            <div
              style={{
                ...
                borderRadius: 8,
                padding: 10,
                borderLeft: "3px solid " + (elast ? elast.color : "#8892b0"),
                marginBottom: 10,
              }}
            >
              <div style={{ fontSize: 11, color: elast ? elast.color : "#8892b0", fontWeight: 600, marginBottom: 4 }}>
          ...
          {seed.anonymizedText && (
            ...
            >
              <div style={{ fontSize: 11, color: "#95a5a6", fontWeight: 600, marginBottom: 4 }}>
                APERÇU — VERSION TRANSPORTABLE
              </div>
              <div style={{ fontSize: 12, color: "#ccd6f6", lineHeight: 1.5 }}>"{seed.anonymizedText}"</div>
              <div style={{ fontSize: 11, color: "#495670", marginTop: 6 }}>
                Tu pourras vérifier et éditer cette version avant archivage. L'IA détecte les éléments sensibles. C'est
                toi qui valides.
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
            <span
              style={{
                background: "#1a1a2e",
                color: seed.type === "cicatrice" ? "#ff9800" : "#e94560",
                fontSize: 11,
                padding: "4px 10px",
                borderRadius: 20,
                fontWeight: 600,
          ...
          <div style={{ background: "#1a1a2e", borderRadius: 8, padding: 10, borderLeft: "3px solid #e94560" }}>
            <div style={{ fontSize: 11, color: "#495670", fontWeight: 600, marginBottom: 6 }}>
              SI VALIDÉE, CETTE BRIQUE ALIMENTERA :
            </div>
            {seed.usedIn.map(function (u, i) {
              return (
                <div key={i} style={{ fontSize: 12, color: "#8892b0", marginBottom: 2 }}>
                  {"→"} {u}
                </div>
              );
            })}
        ...
        {almostEditing && (
          <div style={{ background: "#1a1a2e", borderRadius: 10, padding: 16, marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: "#ff9800", fontWeight: 600, letterSpacing: 1, marginBottom: 8 }}>
              TA VERSION
            </div>
            <textarea
              value={almostText}
              onChange={function (e) {
                setAlmostText(e.target.value);
              }}
              style={{
                width: "100%",
                minHeight: 90,
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={function () {
                  ...
                  var correctedSeed = Object.assign({}, seed, {
                    generatedText: effectiveText,
                    originalText: effectiveText,
                    structuredFields: buildStructuredFields(seed),
                    sideProject: detectSideProject(almostText.trim()),
                    ...
                    internalAdvocacy: generateInternalAdvocacy(
                      almostText.trim(),
                      seed.brickCategory,
                      seed.type,
                      seed.elasticity
                ...
                disabled={almostText.trim().length < 10}
                style={{
                  flex: 1,
                  padding: 14,
                  ...
                  border: "none",
                  borderRadius: 10,
                  cursor: almostText.trim().length >= 10 ? "pointer" : "default",
                  fontWeight: 700,
                  fontSize: 14,
                }}
              >
        ...
        {!almostEditing && (
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={function () {
                var forgedSeed = Object.assign({}, seed, {
                  generatedText: effectiveText,
                  structuredFields: buildStructuredFields(seed),
                  sideProject: detectSideProject(effectiveText),
                  advocacyText: generateAdvocacyText(effectiveText, seed.brickCategory, seed.type, seed.nightmareText),
            ...
            <button
              onClick={function () {
                setAlmostText(effectiveText);
                setAlmostEditing(true);
              }}
              style={{
                flex: 1,
                padding: 14,
            ...
            >
              Presque {"—"} voici ma version
            </button>
            <button
              onClick={function () {
                setEditText(effectiveText);
                setPhase("correcting");
              }}
              style={{
                padding: "14px 12px",
  ...
  function handleSubmit() {
    ...
    if (seed.type === "take") {
      ...
      if (takeDepth.level === "surface") {
        setConfrontMsg(
          "Ta réponse est celle de 90% des professionnels de ton secteur. C'est un récit dominant, pas une prise de position. Un recruteur qui lit ça ne retient rien. Creuse. Qu'est-ce que ton expérience t'a montré que les articles de blog ne disent pas ?"
        );
        setPhase("confront");
        return;
      }
    ...
    if (seed.blameDetection && hasBlame(text)) {
      setConfrontMsg(
        "Tu blâmes le produit ou le prix. Quel était le budget réel du prospect ? Quelle étape de qualification as-tu sautée ? C'est là que le deal s'est perdu."
      );
      setPhase("confront");
      return;
    }
    ...
    if (seed.externalizeDetection && hasExternalization(text)) {
      setConfrontMsg(
        "Tu n'as cité aucun facteur sous ton contrôle. Le recruteur entend : cette personne ne prend pas de responsabilité. Trouve ta part dans cet échec."
      );
      setPhase("confront");
      return;
    }
  ...
  var qLabel =
    seed.type === "take"
      ...
      : seed.type === "cicatrice"
        ? "CICATRICE"
        : categoryLabels[seed.brickCategory] || "HYPOTHÈSE";

  return (
    <div>
      <div style={{ marginBottom: 14 }}>
        <div
          style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#495670", marginBottom: 4 }}
        >
          <span>
            Question {processed.length + 1} / {seeds.length}
          </span>
          <span>
            {validated.length} brique{validated.length > 1 ? "s" : ""}{" "}
            {missionItems.length > 0
              ? "+ " + missionItems.length + " mission" + (missionItems.length > 1 ? "s" : "")
              : ""}
          </span>
        </div>
        <div style={{ width: "100%", background: "#1a1a2e", borderRadius: 6, height: 4, overflow: "hidden" }}>
          <div
            style={{
              width: (processed.length / seeds.length) * 100 + "%",
              height: "100%",
              background: "#e94560",
              borderRadius: 6,
      ...
      <div style={{ background: "#0f3460", borderRadius: 10, padding: 20, marginBottom: 16 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
            ...
            style={{
              ...
              color:
                seed.type === "take"
                  ...
                  : seed.type === "cicatrice"
                    ? "#ff9800"
                    : CATEGORY_LABELS[seed.brickCategory]
                      ? CATEGORY_LABELS[seed.brickCategory].color
                      : "#e94560",
              fontWeight: 600,
              letterSpacing: 1,
            }}
          >
            {qLabel} #{seed.id}
            {seed.type !== "take" && seed.type !== "cicatrice" ? " — Basée sur ton profil × le marché" : ""}
          </span>
          {seed.elasticity && ELASTICITY_LABELS[seed.elasticity] && (
            <span
              style={{
                fontSize: 10,
                color: ELASTICITY_LABELS[seed.elasticity].color,
                background: "#1a1a2e",
                padding: "2px 8px",
                borderRadius: 10,
              }}
        ...
        <div style={{ fontSize: 16, color: "#ccd6f6", lineHeight: 1.6, marginBottom: 14, fontWeight: 600 }}>
          {seed.question}
        </div>
      ...
      <div style={{ fontSize: 11, color: canSubmit ? "#495670" : "#e94560", marginBottom: 12, textAlign: "right" }}>
        {canSubmit
          ? seed.type === "take"
            ? "L'IA analyse la profondeur de ta position"
            : seed.type === "unfair_advantage"
              ? "L'IA croise avec tes briques"
              : seed.brickCategory === "chiffre" && seed.type === "preuve" && !hasNumbers(answer)
                ? "Attention : pas de chiffre détecté"
                : "L'IA structure ta réponse"
          : "Remplis au moins 2 champs"}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <button
...
var STRESS_ANGLES = [
  ...
  },
  {
    ...
    fields: [
      {
        key: "situation",
        label: "Quelle était la situation",
        placeholder: "Ex : le board voulait couper le budget R&D de 40%",
      },
      {
        key: "decision",
        label: "Qu'as-tu décidé",
        placeholder: "Ex : j'ai proposé un pivot vers le segment mid-market",
      },
  ...
  },
  {
    ...
    attack:
      "Tu l'as fait là-bas. Tu le refais ici ? Qu'est-ce qui était spécifique au contexte et qu'est-ce qui marche partout ?",
    fields: [
      {
        key: "specific",
        label: "Le contexte était spécifique comment ?",
        placeholder: "Ex : un marché de niche avec 200 prospects identifiés",
      },
      {
        key: "transferable",
        label: "Qu'est-ce qui marche quel que soit le contexte ?",
        placeholder: "Ex : le framework de qualification en 4 étapes que j'ai structuré",
      },
    ],
  },
  {
    ...
    fields: [
      {
        key: "who",
        label: "Qui a bougé grâce à toi",
        placeholder: "Ex : 3 directeurs régionaux qui bloquaient le déploiement",
      },
      {
        key: "how",
        label: "Comment tu les as convaincus",
        placeholder: "Ex : présentation du coût d'inaction chiffré devant le COMEX",
      },
    ],
  },
  {
    ...
    fields: [
      {
        key: "benchmark",
        label: "Ton résultat vs le marché",
        placeholder: "Ex : la moyenne secteur est 15%, j'ai atteint 34%",
      },
    ],
  },
];
...
var ANGLE4_SOLO = {
  ...
  fields: [
    {
      key: "who",
      label: "Qui a utilisé / testé / acheté ?",
      placeholder: "Ex : 3 early adopters qui ont testé le prototype en beta",
    },
    {
      key: "how",
      label: "Comment tu les as convaincus ?",
      placeholder: "Ex : démonstration live du gain de temps sur leur workflow",
    },
  ],
};
...
function analyzeStressResponse(angleKey, responseFields) {
  var fullText = Object.keys(responseFields)
    .map(function (k) {
      return responseFields[k] || "";
    })
    .join(" ")
    .toLowerCase();
  ...
  if (angleKey === "angle1") {
    ...
    var hasPeriod = [
      ...
      return fullText.indexOf(m) !== -1;
    });
    var hasComparison = ["%", "moyenne", "comparé", "compare", "versus", "vs", "base", "avant", "après", "apres"].some(
      function (m) {
        return fullText.indexOf(m) !== -1;
      }
    );
  ...
  if (angleKey === "angle2") {
    ...
    var hasResult =
      /\d/.test(fullText) ||
      ["résultat", "resultat", "impact", "obtenu", "atteint", "gagné", "gagne"].some(function (m) {
        return fullText.indexOf(m) !== -1;
      });
  ...
  if (angleKey === "angle5") {
    var hasBenchmark =
      /\d/.test(fullText) &&
      ["moyenne", "marché", "marche", "benchmark", "secteur", "industrie", "comparé", "compare", "%"].some(
        function (m) {
          return fullText.indexOf(m) !== -1;
        }
      );
 ...
 */
export function BrickStressTest({
  bricks,
  onBrickUpdate,
  nightmareCosts,
  offersArray,
    ...
    cauchemars.forEach(function (c) {
      if (
        ...
        c.kpis.some(function (k) {
          return brick.kpi.indexOf(k) !== -1 || k.indexOf(brick.kpi) !== -1;
        })
      ) {
        found = c;
      }
  ...
  function hasOfferData() {
    return (
      ...
      offersArray.some(function (o) {
        return o.parsedSignals && o.parsedSignals.totalSignals > 0;
      })
    );
  }
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, letterSpacing: 1, marginBottom: 8 }}>
        STRESS TEST
        <Tooltip term="Stress test" text={VOCABULARY.stressTest} /> — BLINDAGE
        <Tooltip term="Blindage" text={VOCABULARY.blindage} /> DES BRIQUES
      </div>
        ...
        return (
          <div
            ...
            style={{
              ...
              borderRadius: 10,
              marginBottom: 8,
              border: "1px solid " + (isArmored ? "#4ecca3" + "44" : "#16213e"),
              overflow: "hidden",
            }}
          >
            ...
            <div
              onClick={function () {
                setExpandedId(isExpanded ? null : brick.id);
                setActiveAngle(null);
                setResponseFields({});
              }}
              style={{
            ...
            >
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 4, flexWrap: "wrap" }}>
                  <span
                    style={{
                      fontSize: 10,
                      color: ARMOR_COLORS[armor.status],
                      background: ARMOR_COLORS[armor.status] + "22",
                      padding: "2px 8px",
                      borderRadius: 6,
                      fontWeight: 700,
                    }}
              ...
              <span style={{ fontSize: 14, color: "#495670", marginLeft: 8, flexShrink: 0 }}>
                {isExpanded ? "\u25B2" : "\u25BC"}
              </span>
            </div>
            ...
            {isExpanded && (
              <div style={{ padding: "0 14px 14px 14px" }}>
                  ...
                  {[
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ fontSize: 9, color: c.ok ? "#4ecca3" : "#495670", fontWeight: 600 }}>
                          {c.label}
                        </div>
                      </div>
                    );
                ...
                {brick.stressTest &&
                  (function () {
                    var pastResults = STRESS_ANGLES.filter(function (a) {
                      return brick.stressTest[a.key] && brick.stressTest[a.key].attempted;
                    });
                        ...
                        {pastResults.map(function (a) {
                          ...
                          return (
                            <div key={a.key} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                              <span style={{ fontSize: 10, color: VERDICT_COLORS[st.verdict], fontWeight: 600 }}>
                                {VERDICT_LABELS[st.verdict]}
                              </span>
                ...
                {isArmored &&
                  (function () {
                    ...
                    var currentDraft =
                      transferDrafts[draftKey] != null ? transferDrafts[draftKey] : brick.transferStatement || "";
                    // Pre-fill from angle3 if empty and angle3 completed
                    if (
                      !currentDraft &&
                      brick.stressTest &&
                      brick.stressTest.angle3 &&
                      brick.stressTest.angle3.response &&
                      brick.stressTest.angle3.response.transferable
                    ) {
                    ...
                    if (draftLen >= 20) {
                      ...
                      if (targetCluster) {
                        var roleWords = targetCluster.label
                          .toLowerCase()
                          .split(/[\s\/]+/)
                          .filter(function (w) {
                            return w.length > 3;
                        ...
                        roleWords.forEach(function (w) {
                          if (draftLower.indexOf(w) !== -1) hasRoleKw = true;
                        });
                      }
                      ...
                      if (activeCauchs) {
                        activeCauchs.forEach(function (c) {
                          ...
                          words.forEach(function (w) {
                            if (draftLower.indexOf(w) !== -1) hasNightmareKw = true;
                          });
                          if (c.kpis)
                            c.kpis.forEach(function (k) {
                              if (draftLower.indexOf(k.toLowerCase()) !== -1) hasNightmareKw = true;
                            });
                        });
                      }
                    ...
                    return (
                      ...
                      >
                        <div style={{ fontSize: 12, color: "#ccd6f6", fontWeight: 600, marginBottom: 6 }}>
                          Ce que cette brique prouve pour le poste visé
                        </div>
                        <textarea
                          value={currentDraft}
                          onChange={function (e) {
                        ...
                        >
                          <div
                            style={{ fontSize: 10, color: tooShort ? "#e94560" : isGeneric ? "#ff9800" : "#495670" }}
                          >
                            {tooShort
                              ? "Minimum 20 caractères"
                              : isGeneric
                                ? "Ton transfert est générique. Mentionne le rôle ou un cauchemar précis."
                                : draftLen + " caractères"}
                          </div>
                          {!saved && draftLen >= 20 && (
                            <button
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {STRESS_ANGLES.map(function (angle) {
                      ...
                      var alreadyAttempted =
                        brick.stressTest && brick.stressTest[angle.key] && brick.stressTest[angle.key].attempted;
                      ...
                      return (
                        <button
                          ...
                          style={{
                            padding: "10px 14px",
                            textAlign: "left",
                            cursor: alreadyAttempted ? "default" : "pointer",
                            background: alreadyAttempted ? "#0a0a1a" : isRec ? "#e94560" + "22" : "#0f3460",
                            border: alreadyAttempted
                              ? "1px solid #16213e"
                              : isRec
                                ? "2px solid #e94560"
                                : "1px solid #16213e",
                            borderRadius: 8,
                            opacity: alreadyAttempted ? 0.4 : 1,
                        ...
                        >
                          <div
                            style={{
                              fontSize: 12,
                              color: alreadyAttempted ? "#495670" : isRec ? "#e94560" : "#8892b0",
                              fontWeight: isRec ? 700 : 600,
                            }}
                          >
                            {isRec && !alreadyAttempted ? "→ " : ""}
                            {displayAngle.attack.split(".")[0]}
                          </div>
                          {isRec && !alreadyAttempted && (
                            <div style={{ fontSize: 10, color: "#e94560", marginTop: 2 }}>
                              Recommandé — remplit la case « {angle.armorCase} »
                            </div>
                          )}
                        </button>
                      );
                ...
                {activeAngle && (
                  <div>
                    ...
                    >
                      <div style={{ fontSize: 14, color: "#e94560", fontWeight: 700, lineHeight: 1.6 }}>
                        {activeAngle.attack}
                      </div>
                    </div>
                    ...
                    {nightmare && (
                      ...
                      >
                        <div style={{ fontSize: 11, color: "#ff9800", fontWeight: 600, marginBottom: 4 }}>
                          COÛT DU CAUCHEMAR — {nightmare.label.toUpperCase()}
                        </div>
                        <div style={{ fontSize: 12, color: "#ccd6f6", lineHeight: 1.5 }}>
                          {formatCost(nightmare.costRange[0])}-{formatCost(nightmare.costRange[1])}
                          {"€"}/{nightmare.costUnit}. {nightmare.costContext}
                        </div>
                      </div>
                    )}
                    {!nightmare && (
                      <div style={{ background: "#0f3460", borderRadius: 8, padding: 10, marginBottom: 10 }}>
                        <div style={{ fontSize: 11, color: "#495670", lineHeight: 1.5 }}>
                          Chaque réponse crédible réduit le risque perçu par le recruteur. Ton chiffre est ton bouclier.
                        </div>
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
                      {activeAngle.fields.map(function (f) {
                        return (
                          <div key={f.key}>
                            <div style={{ fontSize: 11, color: "#8892b0", fontWeight: 600, marginBottom: 3 }}>
                              {f.label}
                            </div>
                            <input
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <button
                        onClick={function () {
                          handleValidate(brick, activeAngle);
                        }}
                        disabled={Object.keys(responseFields).every(function (k) {
                          return !responseFields[k] || responseFields[k].trim().length === 0;
                        })}
                        style={{
                          flex: 1,
                          padding: 12,
