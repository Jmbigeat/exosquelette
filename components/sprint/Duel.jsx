...
var DUEL_CRISES = [
  ...
  },
  {
    ...
    diagnostic: {
      ...
      recadre: [
        "trois cauchemars",
        "voici le problème que je resous",
        "la preuve",
        "mesurable",
        "reproductible",
        "mon arbitrage",
...
export function Duel({ questions, bricks, onComplete, targetRoleId, interviewCoaching }) {
  ...
  function shouldFireCrisis() {
    return !crisisUsed && idx >= 1 && idx <= 3 && Math.random() < 0.5;
  }
  function shouldFireContradict() {
    return !contradictUsed && idx >= 0 && idx <= 3 && Math.random() < 0.4;
  }
  function shouldFireSilence() {
    return !silenceUsed && idx >= 1 && idx <= 3 && Math.random() < 0.35;
  }
  ...
  function analyzeCrisisResponse(text, crisis) {
    ...
    var extCount = 0;
    var recCount = 0;
    crisis.diagnostic.externalize.forEach(function (m) {
      if (lower.indexOf(m) !== -1) extCount++;
    });
    crisis.diagnostic.recadre.forEach(function (m) {
      if (lower.indexOf(m) !== -1) recCount++;
    });
    if (recCount > extCount)
      return { verdict: "recadrage", color: "#4ecca3", msg: "Tu as recadré sur ta valeur. Réflexe de positionnement." };
    if (extCount > recCount)
      return {
        verdict: "externalisation",
        color: "#ff9800",
        msg: "Tu as externalisé. Le recruteur lit : cette personne subit les événements au lieu de les cadrer.",
      };
    return {
      verdict: "neutre",
      color: "#8892b0",
      msg: "Réponse neutre. Ni externalisation ni recadrage clair. En entretien, l'absence de positionnement est un positionnement : celui du suiveur.",
    };
  }
      ...
      <div>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: "#ccd6f6", marginBottom: 6 }}>
            Simulateur de Duel
            <Tooltip term="Duel" text={VOCABULARY.duel} />
          </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input
            value={ctx.role}
            onChange={function (e) {
              setCtx(Object.assign({}, ctx, { role: e.target.value }));
            }}
          ...
          <input
            value={ctx.company}
            onChange={function (e) {
              setCtx(Object.assign({}, ctx, { company: e.target.value }));
            }}
        ...
        <button
          onClick={function () {
            if (canBegin) setPhase("discovery");
          }}
          disabled={!canBegin}
          style={{
            width: "100%",
  ...
  if (phase === "discovery") {
    ...
    var roleDiscovery =
      discoveryQs[targetRoleId] || "Avant que je déroule mon parcours, quels sont vos enjeux clés sur ce poste ?";
    ...
    return (
      <div>
        ...
        >
          <div style={{ fontSize: 11, color: "#3498db", fontWeight: 600, letterSpacing: 1, marginBottom: 10 }}>
            COACHING PRE-DUEL
          </div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#ccd6f6", marginBottom: 12 }}>
            Avant de répondre, pose une question.
          </div>
          <div style={{ fontSize: 13, color: "#8892b0", lineHeight: 1.7, marginBottom: 16 }}>
            Le piège le plus courant des profils seniors : le monologue. Tu racontes pendant 6 minutes. Le recruteur
            décroche après 90 secondes. Pas parce que tu es mauvais. Parce que tu n'as pas calibré.
          </div>
          <div style={{ fontSize: 13, color: "#8892b0", lineHeight: 1.7, marginBottom: 16 }}>
            Avant chaque réponse, pose une de ces questions au recruteur. Il te donnera la cible. Tu réponds avec la
            brique qui matche. Chaque mot que tu prononces est pertinent parce qu'il l'a demandé.
          </div>
          <div style={{ background: "#1a1a2e", borderRadius: 8, padding: 12, marginBottom: 8 }}>
            <div style={{ fontSize: 11, color: "#3498db", fontWeight: 600, marginBottom: 4 }}>
              QUESTION DISCOVERY (adaptée à ton rôle)
            </div>
            <div style={{ fontSize: 14, color: "#ccd6f6", fontStyle: "italic", lineHeight: 1.5 }}>
              "{roleDiscovery}"
            </div>
          </div>
          <div style={{ background: "#1a1a2e", borderRadius: 8, padding: 12, marginBottom: 8 }}>
            <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, marginBottom: 4 }}>
              DÉCLENCHEUR (révèle l'urgence du décideur)
            </div>
            <div style={{ fontSize: 14, color: "#ccd6f6", fontStyle: "italic", lineHeight: 1.5 }}>
              "{triggerDiscovery}"
            </div>
          ...
          <div style={{ background: "#1a1a2e", borderRadius: 8, padding: 12, marginBottom: 8 }}>
            <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, marginBottom: 4 }}>
              CICATRICE RECRUTEUR (révèle le profil à éviter)
            </div>
            <div style={{ fontSize: 14, color: "#ccd6f6", fontStyle: "italic", lineHeight: 1.5 }}>
              "{antiProfileDiscovery}"
            </div>
          </div>
          <div style={{ background: "#1a1a2e", borderRadius: 8, padding: 12, marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 12, color: "#495670", lineHeight: 1.5, marginTop: 12 }}>
            Un vrai senior n'est pas celui qui en dit le plus. C'est celui qui écoute et cible. Il articule son vécu
            autour du problème de l'autre.
          </div>
          <div
            style={{
              background: "#9b59b6" + "15",
              borderRadius: 8,
              padding: 12,
              marginTop: 12,
              border: "1px solid #9b59b6" + "33",
            }}
          >
            ...
            <div style={{ fontSize: 13, color: "#ccd6f6", lineHeight: 1.7 }}>
              Quand le recruteur te pose une question, ta première phrase est un chiffre ou un résultat. Le contexte
              vient après. Pas avant. "J'ai réduit le cycle de 14 à 9 jours" puis le comment. Jamais l'inverse.
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            <div style={{ fontSize: 13, color: "#ccd6f6", lineHeight: 1.7 }}>
              Le recruteur va se taire après ta réponse. Ne remplis pas le silence. Laisse-le revenir vers toi. Celui
              qui parle en premier après un silence perd le cadre de la négociation.
            </div>
          </div>
          {interviewCoaching && (
            <div
            ...
            >
              <div style={{ fontSize: 11, color: "#3498db", fontWeight: 600, letterSpacing: 1, marginBottom: 8 }}>
                QUESTIONS À POSER EN ENTRETIEN
              </div>
              <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.6, marginBottom: 10 }}>
                Ces questions prouvent ta maîtrise sans rien affirmer. Pose-les quand le recruteur demande "Avez-vous
                des questions ?"
              </div>
              <div
                style={{
  ...
  if (phase === "pitch_chrono") {
    var pitchTake = bricks.find(function (b) {
      return b.brickType === "take" && b.status === "validated";
    });
    var pitchBricks = bricks
      .filter(function (b) {
        return b.status === "validated" && b.brickType !== "take";
      })
      .slice(0, 3);
    var pitchDiscoveryQ =
      pitchBricks.length > 0 && pitchBricks[0].discoveryQuestions && pitchBricks[0].discoveryQuestions.length > 0
        ? pitchBricks[0].discoveryQuestions[0]
    ...
    var chronoBlocs = [
      {
        ...
        time: "0-15s",
        color: "#e94560",
        content: pitchTake ? pitchTake.text : "Définis ton Take dans les Piliers.",
        hint: "Ouvre sur le problème du décideur. Pas sur toi.",
      },
      {
        label: "PREUVE 1",
        time: "15-30s",
        color: "#4ecca3",
        content: pitchBricks[0] ? pitchBricks[0].text : "Brique manquante.",
      ...
      },
      {
        ...
        time: "30-45s",
        color: "#4ecca3",
        content: pitchBricks[1] ? pitchBricks[1].text : "Brique manquante.",
        hint: "Preuve complémentaire. Autre angle.",
      },
      {
        label: "MÉTHODE",
        time: "45-70s",
        color: "#3498db",
        content: pitchBricks[2] ? pitchBricks[2].text : "Comment tu transfères ici.",
        hint: "Ce que tu feras chez eux. Pas ce que tu as fait ailleurs.",
      },
      {
        label: "QUESTION",
        time: "70-90s",
        color: "#ff9800",
        content: pitchDiscoveryQ,
        hint: "Tu reprends le cadre. Le recruteur parle.",
      },
    ];
    ...
    var selectedInterruption =
      pitchInterruptions[Math.abs(hashCode(ctx.company + ctx.role + "pitchint")) % pitchInterruptions.length];
    ...
    return (
      <div>
        ...
        >
          <div style={{ fontSize: 11, color: "#9b59b6", fontWeight: 600, letterSpacing: 1, marginBottom: 6 }}>
            PITCH 90 SECONDES — CHRONO
          </div>
          <div style={{ fontSize: 13, color: "#8892b0", lineHeight: 1.7, marginBottom: 16 }}>
            Pas de texte à réciter. Une structure à suivre. 5 blocs. 90 secondes. Le chrono donne le rythme. Tes mots
            viennent de toi.
          </div>
          ...
          {!pitchInterrupted && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, marginBottom: 8 }}>
                ENTRAÎNEMENT : prononce ton pitch à voix haute. Puis clique.
              </div>
              <button
                onClick={function () {
                  setPitchInterrupted(true);
                }}
                style={{
                  width: "100%",
                  padding: 14,
              ...
              >
                J'ai fait mon pitch {"→"} Interruption
              </button>
            </div>
          )}
          ...
          {pitchInterrupted && !pitchInterruptResponse && (
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 700, color: "#e94560", marginBottom: 10 }}>
                {selectedInterruption}
              </div>
              <div style={{ fontSize: 11, color: "#8892b0", marginBottom: 8 }}>
                Le recruteur ne te laisse pas finir. C'est fait exprès. Reprends le cadre.
              </div>
              <textarea
                value={pitchAnswer}
                onChange={function (e) {
                  setPitchAnswer(e.target.value);
                }}
              ...
              <button
                onClick={function () {
                  if (pitchAnswer.trim().length >= 10) {
                    ...
                    setEntropyLog(function (prev) {
                      return prev.concat([
                        {
                          type: "pitch_interrupt",
                          scenario: selectedInterruption,
                          answer: pitchAnswer.trim(),
                          color: "#9b59b6",
                          diagnostic:
                            "Le recruteur teste ta capacité à tenir ta ligne quand il te coupe. Ta réponse montre si tu reprends le cadre ou si tu le perds.",
                        },
                      ]);
                    });
                  }
                }}
                disabled={pitchAnswer.trim().length < 10}
                style={{
                  width: "100%",
                  padding: 12,
                  ...
                  border: "none",
                  borderRadius: 8,
                  cursor: pitchAnswer.trim().length >= 10 ? "pointer" : "default",
                  fontWeight: 700,
                  fontSize: 13,
                }}
              >
          ...
          {pitchInterruptResponse && (
            ...
            >
              <div style={{ fontSize: 11, color: "#4ecca3", fontWeight: 600, marginBottom: 6 }}>
                INTERRUPTION ENCAISSÉE
              </div>
              <div style={{ fontSize: 12, color: "#ccd6f6", lineHeight: 1.5, marginBottom: 8 }}>
                Ta réponse : "{pitchInterruptResponse}"
              </div>
              <div style={{ fontSize: 11, color: "#8892b0", lineHeight: 1.5 }}>
                En entretien réel, le recruteur coupe pour tester ta résistance. Le candidat qui panique reformule tout
                depuis le début. Le candidat préparé répond à l'objection en une phrase et reprend son fil. Tu viens de
                t'entraîner à reprendre le fil.
              </div>
            </div>
          )}
        </div>
  ...
  if (phase === "done") {
    ...
    var crisisEvents = entropyLog.filter(function (e) {
      return e.type === "crisis";
    });
    var contradictEvents = entropyLog.filter(function (e) {
      return e.type === "contradiction";
    });
    var silenceEvents = entropyLog.filter(function (e) {
      return e.type === "silence";
    });
    return (
      <div>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 13, color: "#8892b0" }}>
            {answered.length} réponse{answered.length > 1 ? "s" : ""} forgée{answered.length > 1 ? "s" : ""}.{" "}
            {entropyLog.length} événement{entropyLog.length > 1 ? "s" : ""} imprévu{entropyLog.length > 1 ? "s" : ""}.
          </div>
        </div>
        ...
        {entropyLog.length > 0 && (
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, letterSpacing: 1, marginBottom: 8 }}>
              {"\u26A1"} ÉVÉNEMENTS IMPRÉVUS ({entropyLog.length})
            </div>
            {entropyLog.map(function (ev, i) {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "#ccd6f6", lineHeight: 1.4, marginBottom: 4 }}>
                    Ta réponse : "{ev.answer}"
                  </div>
    ...
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#ccd6f6", marginBottom: 8 }}>
          Confrontation brique vs réponse...
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ textAlign: "center", padding: "30px 10px" }}>
        ...
        <div style={{ background: "#0f3460", borderRadius: 10, padding: 16, borderLeft: "3px solid #e94560" }}>
          <div style={{ fontSize: 15, color: "#ccd6f6", lineHeight: 1.6, fontStyle: "italic" }}>
            "{activeCrisis.scenario}"
          </div>
        </div>
        <textarea
          value={crisisAnswer}
          onChange={function (e) {
            setCrisisAnswer(e.target.value);
          }}
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          <button
            onClick={function () {
              ...
              setEntropyLog(function (prev) {
                return prev.concat([
                  {
                    type: "crisis",
                    scenario: activeCrisis.scenario,
                    answer: crisisAnswer.trim(),
                    diagnostic: diag.msg,
                    color: diag.color,
                  },
                ]);
            ...
            disabled={crisisAnswer.trim().length < 10}
            style={{
              flex: 1,
              padding: 14,
              ...
              border: "none",
              borderRadius: 10,
              cursor: crisisAnswer.trim().length >= 10 ? "pointer" : "default",
              fontWeight: 700,
              fontSize: 14,
            }}
          >
      ...
      <div>
        <div
          style={{
            ...
            padding: 20,
            marginBottom: 16,
            borderLeft: "3px solid " + (lastCrisisLog ? lastCrisisLog.color : "#8892b0"),
          }}
        >
          <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, letterSpacing: 1, marginBottom: 10 }}>
            {"\uD83D\uDEA8"} DIAGNOSTIC DE CRISE
          </div>
          <div style={{ fontSize: 13, color: "#ccd6f6", lineHeight: 1.6, marginBottom: 10 }}>
            {lastCrisisLog ? lastCrisisLog.diagnostic : ""}
          </div>
          <div style={{ fontSize: 12, color: "#495670", lineHeight: 1.5 }}>
            En entretien, les crises arrivent. Le recruteur pose une mine pour voir comment tu réagis sous pression. Ce
            n'était pas prévu. C'est le point.
          </div>
        </div>
        <button
          onClick={function () {
  ...
  if (phase === "contradiction") {
    return (
      <div>
        ...
        >
          <div style={{ fontSize: 11, color: "#9b59b6", fontWeight: 600, letterSpacing: 1, marginBottom: 10 }}>
            {"\u2694\uFE0F"} RELANCE CONTRADICTOIRE
          </div>
          <div style={{ fontSize: 13, color: "#8892b0", lineHeight: 1.5, marginBottom: 12 }}>
            Le recruteur ne passe pas à la question suivante. Il te regarde et dit :
          </div>
          <div style={{ fontSize: 15, color: "#ccd6f6", lineHeight: 1.6, fontStyle: "italic" }}>
            "{activeContradict}"
          </div>
        ...
        <textarea
          value={contradictAnswer}
          onChange={function (e) {
            setContradictAnswer(e.target.value);
          }}
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          <button
            onClick={function () {
              ...
              var defends =
                lower.indexOf("je maintiens") !== -1 ||
                lower.indexOf("les chiffres") !== -1 ||
                lower.indexOf("la preuve") !== -1 ||
                lower.indexOf("je confirme") !== -1 ||
                lower.indexOf("mesurable") !== -1;
              var nuances =
                lower.indexOf("c'est vrai que") !== -1 ||
                lower.indexOf("effectivement") !== -1 ||
                lower.indexOf("je reconnais") !== -1 ||
                lower.indexOf("en partie") !== -1;
              var folds =
                lower.indexOf("vous avez raison") !== -1 ||
                lower.indexOf("je comprends") !== -1 ||
                lower.indexOf("peut-etre") !== -1;
              var diagMsg = defends
                ? "Tu as tenu ta position avec des faits. Le recruteur respecte la solidité."
                : nuances
                  ? "Tu as nuancé intelligemment. Le recruteur voit de la maturité."
                  : folds
                    ? "Tu as plié. Le recruteur lit : cette personne doute de ses propres résultats."
                    : "Réponse lue. En entretien, la contradiction teste si tu crois à tes propres chiffres.";
              var diagColor = defends ? "#4ecca3" : nuances ? "#3498db" : folds ? "#e94560" : "#8892b0";
              setEntropyLog(function (prev) {
                return prev.concat([
                  {
                    type: "contradiction",
                    scenario: activeContradict,
                    answer: contradictAnswer.trim(),
                    diagnostic: diagMsg,
                    color: diagColor,
                  },
                ]);
            ...
            disabled={contradictAnswer.trim().length < 10}
            style={{
              flex: 1,
              padding: 14,
              background:
                contradictAnswer.trim().length >= 10 ? "linear-gradient(135deg, #9b59b6, #8e44ad)" : "#1a1a2e",
              color: contradictAnswer.trim().length >= 10 ? "#fff" : "#495670",
              border: "none",
              borderRadius: 10,
              cursor: contradictAnswer.trim().length >= 10 ? "pointer" : "default",
              fontWeight: 700,
              fontSize: 14,
            }}
          >
  ...
  if (phase === "silence_challenge") {
    return (
      <div>
        ...
        >
          <div style={{ fontSize: 11, color: "#ff9800", fontWeight: 600, letterSpacing: 1, marginBottom: 10 }}>
            {"\uD83E\uDD10"} SILENCE TACTIQUE
          </div>
          <div style={{ fontSize: 13, color: "#8892b0", lineHeight: 1.5, marginBottom: 12 }}>
            Le recruteur te regarde. Pause de 4 secondes. Puis :
          </div>
                ...
                return prev.concat([
                  {
                    type: "silence",
                    scenario: "C'est tout ?",
                    answer: "J'ai tenu ma position.",
                    diagnostic:
                      "Tu as tenu ta réponse. Conviction affichée. Le recruteur note : cette personne ne doute pas de ce qu'elle dit.",
                    color: "#4ecca3",
                  },
                ]);
              });
  ...
  if (phase === "silence_complete") {
    return (
      <div>
        ...
        >
          <div style={{ fontSize: 11, color: "#ff9800", fontWeight: 600, letterSpacing: 1, marginBottom: 6 }}>
            TU COMPLÈTES
          </div>
          <div style={{ fontSize: 12, color: "#8892b0", lineHeight: 1.5 }}>
            Tu as choisi de compléter. Le recruteur écoute. Mais il a noté que tu as douté de ta première réponse.
          </div>
        </div>
        <textarea
          value={silenceAnswer}
          onChange={function (e) {
            setSilenceAnswer(e.target.value);
          }}
        ...
        <button
          onClick={function () {
            ...
            var diagMsg = hasNewInfo
              ? "Tu as ajouté du contenu substantiel. Le recruteur lit : cette personne approfondit sous pression. Positif."
              : "Tu as ajouté peu. Le recruteur lit : doute sur la première réponse, mais rien de neuf. Négatif.";
            var diagColor = hasNewInfo ? "#3498db" : "#ff9800";
            setEntropyLog(function (prev) {
              return prev.concat([
                {
                  type: "silence",
                  scenario: "C'est tout ? (complété)",
                  answer: silenceAnswer.trim(),
                  diagnostic: diagMsg,
                  color: diagColor,
                },
              ]);
          ...
          disabled={silenceAnswer.trim().length < 5}
          style={{
            width: "100%",
            padding: 14,
            ...
            border: "2px solid #ff9800",
            borderRadius: 10,
            cursor: silenceAnswer.trim().length >= 5 ? "pointer" : "default",
            fontWeight: 700,
            fontSize: 14,
          }}
        >
          Soumettre le complément
        </button>
      </div>
    );
  }
    ...
    return (
      <div>
        <div style={{ background: "#0f3460", borderRadius: 10, padding: 20, marginBottom: 16 }}>
          ...
          {lastResult &&
            lastResult.wordWarning &&
            (function () {
              var roleMirrors = {
                enterprise_ae:
                  "En poste, tu laisserais le client driver le deal. Le prospect décroche après 90 secondes.",
                ...
                senior_pm:
                  "En poste, tu présenterais une roadmap sans hiérarchie. L'équipe ne sait pas par où commencer.",
                ai_architect:
                  "En poste, tu expliquerais le modèle pendant 20 minutes. Le CPO veut le cas d'usage en 2.",
              ...
              var mirror =
                roleMirrors[targetRoleId] ||
                "En poste, tu prendrais trop de temps à arriver au point. Le décideur décroche.";
              return (
                <div
                  style={{
                    background: "#1a1a2e",
                ...
                >
                  <div style={{ fontSize: 11, color: "#9b59b6", fontWeight: 600, marginBottom: 4 }}>
                    MIROIR DE POSTE
                  </div>
        ...
        >
          {idx + 1 >= questions.length ? "Voir le bilan" : "Question suivante"} {"→"}
        </button>
      </div>
    );
  }
  ...
  function handleDuelSubmit() {
    ...
    var warning = null;
    if (wordCount > 80)
      warning = "Ta réponse fait " + wordCount + " mots. En entretien, tu perds ton interlocuteur après 60. Condense.";
    setResults(function (prev) {
      return prev.concat([{ question: q.question, answer: answer.trim(), brickRef: q.brickRef, wordWarning: warning }]);
    });
  ...
  return (
    <div>
      ...
      <div style={{ background: "#0f3460", borderRadius: 10, padding: 20, marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 16, color: "#ccd6f6", lineHeight: 1.6, fontWeight: 600, marginBottom: 10 }}>
          &quot;{q.question}&quot;
        </div>
      ...
      />
      <div
        style={{ fontSize: 11, color: wordCount > 80 ? "#ff9800" : "#495670", marginBottom: 12, textAlign: "right" }}
      >
        {wordCount} mot{wordCount > 1 ? "s" : ""}
        {wordCount > 80 ? " — trop long pour un entretien" : ""}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <button
        ...
        <button
          onClick={function () {
            setResults(function (prev) {
              return prev.concat([{ question: q.question, answer: null, brickRef: q.brickRef, wordWarning: null }]);
            });
            var next = idx + 1;
            if (next >= questions.length) {
              setPhase("done");
