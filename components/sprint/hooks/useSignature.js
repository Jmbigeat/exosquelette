...
import {
  ...
  generateMaskedHypotheses,
  computeMetaPatterns,
  crossReferenceSignature,
  validateSignatureFormulation,
  isSignatureArmored,
} from "@/lib/sprint/signature";
...
export function useSignature(initialState, bricks) {
  ...
  function handleSigCrossNext(chosenHypIndex) {
    if (
      sigCrossRef.current &&
      sigCrossRef.current.type === "divergence" &&
      chosenHypIndex != null &&
      sigHypothesesRef.current
    ) {
  ...
  function renderSignatureOverlay() {
        ...
        <div style={overlayStyle}>
          <div style={cardStyle}>
            <div style={subtitleStyle}>
              TA SIGNATURE
              <Tooltip term="Signature" text={VOCABULARY.signature} />
            </div>
            <div style={titleStyle}>La question</div>
            <div style={labelStyle}>
              Pour quoi tes anciens collègues ou ton manager te sollicitent quand personne d{"'"}autre ne résout le
              problème ?
            </div>
            <textarea
              style={inputStyle}
            ...
            {showFallback && (
              <div style={{ fontSize: 12, color: "#e94560", marginTop: 8, lineHeight: 1.5 }}>
                Pense à la dernière fois qu{"'"}un collègue t{"'"}a appelé en urgence. C{"'"}était pour résoudre quoi
                exactement ?
              </div>
            )}
        ...
        <div style={overlayStyle}>
          <div style={cardStyle}>
            ...
            {cross.type === "convergence" && (
              <div>
                <div style={{ fontSize: 13, color: "#4ecca3", fontWeight: 700, marginBottom: 8 }}>
                  Convergence forte
                </div>
                <div style={{ fontSize: 13, color: "#ccd6f6", lineHeight: 1.6, marginBottom: 8 }}>
                  {cross.diagnostic}
                </div>
                <div
                  style={{
                ...
                >
                  <div style={{ fontSize: 12, color: "#4ecca3", fontWeight: 600, marginBottom: 4 }}>
                    Hypothèse confirmée
                  </div>
                  <div style={{ fontSize: 13, color: "#ccd6f6", lineHeight: 1.5 }}>
                    {cross.matchedHypothesis ? cross.matchedHypothesis.text : ""}
                  </div>
                </div>
                <button
                  style={btnStyle(true)}
                  onClick={function () {
                    handleSigCrossNext(null);
                  }}
                >
                  Suivant
                </button>
            ...
            {cross.type === "partial" && (
              <div>
                <div style={{ fontSize: 13, color: "#3498db", fontWeight: 700, marginBottom: 8 }}>
            ...
            {cross.type === "divergence" && (
              <div>
                ...
                <div style={{ fontSize: 12, color: "#8892b0", marginBottom: 12 }}>
                  Choisis la force que tu veux armer :
                </div>
                {hyps.map(function (h, i) {
                  return (
                    <button
    ...
    if (sigScreen === "formulate") {
      ...
      var assistHyp =
        sigHypothesesRef.current && sigHypothesesRef.current.length > 0 ? sigHypothesesRef.current[0] : null;
      ...
      return (
        <div style={overlayStyle}>
          <div style={cardStyle}>
            ...
            <div style={labelStyle}>En une phrase, qu{"'"}est-ce que tu es le seul à prouver ?</div>
            <textarea
              style={inputStyle}
              value={sigFormulation}
              onChange={function (e) {
                setSigFormulation(e.target.value);
                setSigValidationError(null);
              }}
            ...
            {showAssist && assistHyp && (
              <div
                style={{
                  ...
                  border: "1px dashed #e94560" + "60",
                  borderRadius: 10,
                }}
              >
                <div style={{ fontSize: 11, color: "#e94560", fontWeight: 600, marginBottom: 6 }}>
                  Suggestion assistée
                </div>
                ...
                <button
                  onClick={function () {
                    setSigFormulation(assistHyp.text);
                    setSigValidationError(null);
                  }}
                  style={{
                    marginTop: 8,
                    padding: "6px 12px",
                    background: "#e94560" + "30",
            ...
            <button
              style={btnStyle(sigFormulation.trim().length > 0)}
              onClick={function () {
  ...
  return {
    ...
    sigScreen: sigScreen,
    sigThresholdReached: sigThresholdReached,
