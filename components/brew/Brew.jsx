...
function pillarWeeks(brewWeeks, pillarId) {
  return brewWeeks.filter(function (w) {
    return w.pillar_id === pillarId;
  });
}

function publishedWeeks(brewWeeks, pillarId) {
  return brewWeeks.filter(function (w) {
    return w.pillar_id === pillarId && w.published;
  });
}
...
function computeDiagnostic(brewWeeks, pillarId, pillarTitle) {
  ...
  var total = 0;
  published.forEach(function (w) {
    total += totalReactions(w);
  });
  var strong = total >= 5;
  ...
  if (strong && trend === "up")
    return (
      ...
      " (" +
      pillarTitle +
      ") laisse des traces chez les " +
      dom.label +
      ". Passe à un autre pilier pour diversifier tes preuves."
    );
  if (strong && trend !== "down")
    return (
      "Ton pilier " +
      pillarId +
      " (" +
      pillarTitle +
      ") est déposé. Contacte les profils qui réagissent. Concentre tes publications sur les piliers sans trace."
    );
  if (!strong && trend === "up")
    return (
      "Ton pilier " + pillarId + " (" + pillarTitle + ") commence à exister. Dépose une deuxième trace avant de juger."
    );
  return (
    "Ton pilier " + pillarId + " (" + pillarTitle + ") ne laisse pas de trace. Change l'angle ou abandonne ce pilier."
  );
}
...
export default function Brew(props) {
  ...
  useEffect(
    function () {
      ...
      var thisWeekRows = brewWeeks.filter(function (w) {
        return w.week_start === monday;
      });
      var prevWeeks = brewWeeks.filter(function (w) {
        return w.week_start !== monday;
      });
      ...
      pillars.forEach(function (p, idx) {
        var pid = idx + 1;
        var existing = thisWeekRows.find(function (w) {
          return w.pillar_id === pid;
        });
        var prev = prevWeeks
          .filter(function (w) {
            return w.pillar_id === pid;
  ...
  if (density.score < 70) {
    ...
    gateBlocked = true;
    gateMessage =
      "Le Brew est réservé aux abonnés. ~19€/mois. Livrables calibrés + Brew actif + négociation salariale.";
  }
  ...
  if (gateBlocked) {
    return (
      ...
      >
        <div style={{ fontSize: 12, color: "#e94560", fontWeight: 700, letterSpacing: 2, marginBottom: 16 }}>
          ABNEG@TION — BREW
        </div>
        <div style={{ fontSize: 14, color: "#ccd6f6", marginBottom: 20, maxWidth: 400, lineHeight: 1.6 }}>
          {gateMessage}
        </div>
  ...
  function handleSave() {
    ...
    if (user.id === "dev") {
      ...
      setBrewWeeks(function (prev) {
        var filtered = prev.filter(function (w) {
          return w.week_start !== monday;
        });
    ...
    saveBrewWeek(user.id, monday, rows)
      .then(function () {
  ...
  function getDiltsForPillar(pillarId) {
    var pillarPosts = posts.filter(function (p) {
      return p.pillarId === String(pillarId) || p.pillar === (pillars[pillarId - 1] && pillars[pillarId - 1].title);
    });
  ...
  function getPublishRecommendation() {
      ...
      if ((coverageMap[i] || 0) === 0) {
        ...
        return {
          ...
          message:
            ...
            " (" +
            pillars[i - 1].title +
            ") n'existe pas sur ton profil. Dépose ta première trace cette semaine.",
          type: "no_trace",
          dilts: dilts,
        };
      }
      ...
      if ((coverageMap[i] || 0) === 1) {
        ...
        return {
          ...
          message:
            ...
            " (" +
            pillars[i - 1].title +
            ") a une seule trace au niveau " +
            (dilts.level || 2) +
            ". Dépose une deuxième trace" +
            (targetLabel ? " au niveau " + dilts.target + " (" + targetLabel + ")" : "") +
            ".",
          type: "under_covered",
          dilts: dilts,
        };
    ...
    for (var i = 1; i <= Math.min(4, pillars.length); i++) {
      var dilts = getDiltsForPillar(i);
      var pillarPostsForStagnation = posts.filter(function (p) {
        return p.pillarId === String(i) || p.pillar === (pillars[i - 1] && pillars[i - 1].title);
      });
    ...
    if (stagnatingPillar) {
      ...
      return {
        pillarId: stagnatingPillar.pillarId,
        message:
          "Ton pilier " +
          stagnatingPillar.pillarId +
          " (" +
          pillars[stagnatingPillar.pillarId - 1].title +
          ") stagne au niveau " +
          stagnatingPillar.level +
          ". Monte au niveau " +
          target +
          " (" +
          (DILTS_LEVEL_NAMES[target] || "") +
          ").",
        type: "stagnation",
        dilts: stagnatingPillar.dilts,
      };
    }
    ...
    for (var i = 2; i <= Math.min(4, pillars.length); i++) {
      if ((coverageMap[i] || 0) < leastCount) {
        leastCount = coverageMap[i] || 0;
        leastDense = i;
      }
    }
    return {
      pillarId: leastDense,
      message:
        "Tes preuves se diversifient. Publie le prochain post sur ton pilier le moins dense : pilier " +
        leastDense +
        " (" +
        pillars[leastDense - 1].title +
        ").",
      type: "diversify",
      dilts: getDiltsForPillar(leastDense),
    };
  }
  ...
  function generateBrewDM(pillarId) {
    ...
    var pillarContext = pillar
      ? { pillarId: pillarId, pillarTitle: pillar.title, pillarTheme: pillar.desc || pillar.title }
      : null;
  ...
  function handlePrepareRegeneration(pillarId, targetLevel) {
        ...
        return [
          {
            id: Date.now() + "",
            pillar_id: pillarId,
            target_dilts_level: targetLevel,
            status: "pending",
            created_at: new Date().toISOString(),
          },
        ].concat(prev);
    ...
    createBrewInstruction(user.id, pillarId, targetLevel)
      .then(function () {
  ...
  var recommendedPost = posts.find(function (p) {
    return (
      p.pillarId === String(pubRecommendation.pillarId) ||
      p.pillar === (pillars[pubRecommendation.pillarId - 1] && pillars[pubRecommendation.pillarId - 1].title)
    );
  });
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          >
            ABNEG@TION — BREW
            <Tooltip term="Brew" text={VOCABULARY.brew} />
          </div>
          <div style={{ fontSize: 10, color: "#495670", marginTop: 2 }}>Cockpit stratégique de preuves LinkedIn</div>
        </div>
        <a href="/sprint" style={{ fontSize: 11, color: "#8892b0", textDecoration: "none" }}>
          ← La Forge
        </a>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ background: "#111125", borderRadius: 8, padding: "8px 14px", fontSize: 11 }}>
          ...
          <span style={{ color: "#ccd6f6", fontWeight: 700 }}>
            {
              Object.keys(coverageMap).filter(function (k) {
                return coverageMap[k] > 0;
              }).length
            }
            /{pillars.length}
          </span>
      ...
      <div style={{ background: "#111125", borderRadius: 12, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 11, color: "#495670", marginBottom: 12 }}>
          Qu'as-tu publié cette semaine ? Qui a réagi ?
        </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                padding: 12,
                marginBottom: 10,
                borderLeft: "3px solid " + (isPublished ? "#4ecca3" : "#1a1a3e"),
              }}
            >
              <div
                style={{
                  ...
                  alignItems: "center",
                  marginBottom: isPublished ? 8 : 0,
                }}
              >
                <div>
                  <span style={{ fontSize: 12, fontWeight: 700, color: isPublished ? "#ccd6f6" : "#495670" }}>
                    Pilier {pid} — {pillar.title}
                  </span>
                ...
                <div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 8 }}>
                    {CATEGORY_KEYS.map(function (cat) {
                      ...
                      return (
                        <div key={cat}>
                          <div style={{ fontSize: 10, color: CATEGORY_COLORS[cat], marginBottom: 2 }}>
                            {CATEGORY_LABELS[cat]}
                          </div>
                          <input
                            type="number"
                            ...
                            placeholder={
                              weekCount === 0 ? "Combien de " + CATEGORY_LABELS[cat].toLowerCase() + " ont réagi ?" : ""
                            }
                            style={{
                              width: "100%",
                              background: "#1a1a2e",
                              ...
                              borderRadius: 6,
                              color: data._prefilled ? "#495670" : "#ccd6f6",
                              fontSize: 12,
                              padding: "6px 8px",
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Optionnel — un fait saillant"
                      style={{
                        width: "100%",
                        background: "#1a1a2e",
                        border: "1px solid #495670",
      ...
      <div style={{ background: "#111125", borderRadius: 12, padding: 16, marginBottom: 20 }}>
        ...
        {weekCount === 0 ? (
          <div style={{ fontSize: 11, color: "#495670", lineHeight: 1.6 }}>
            Déclare ta première semaine pour voir les analyses. La carte de visibilité, les tendances et le diagnostic
            apparaîtront ici.
          </div>
        ) : (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: "#8892b0", fontWeight: 700, marginBottom: 8 }}>
                Carte de visibilité des traces
              </div>
              {pillars.map(function (pillar, idx) {
                var pid = idx + 1;
                ...
                if (published.length === 0)
                  return (
                    <div key={pid} style={{ fontSize: 10, color: "#495670", marginBottom: 6 }}>
                      Pilier {pid} ({pillar.title}) — aucune trace
                    </div>
                  );
                var dom = dominantCategory(published);
                ...
                if (grandTotal === 0)
                  return (
                    <div key={pid} style={{ fontSize: 10, color: "#495670", marginBottom: 6 }}>
                      Pilier {pid} ({pillar.title}) — publié, 0 réaction déclarée
                    </div>
                  );

                return (
                  <div key={pid} style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 11, color: "#ccd6f6", fontWeight: 600, marginBottom: 4 }}>
                      Pilier {pid} — {pillar.title}
                    </div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {pillars.map(function (pillar, idx) {
                    ...
                    return (
                      <div
                        key={pid}
                        style={{ background: "#0d0d1a", borderRadius: 8, padding: "6px 12px", fontSize: 11 }}
                      >
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {pillars.map(function (pillar, idx) {
                ...
                if (!diag)
                  return (
                    <div key={pid} style={{ fontSize: 10, color: "#495670", marginBottom: 4 }}>
                      Pilier {pid} : pas encore de trace.
                    </div>
                  );
                return (
                  <div
            ...
            <div>
              ...
              {pillars.map(function (pillar, idx) {
                ...
                if (!dilts.level)
                  return (
                    <div key={pid} style={{ fontSize: 10, color: "#495670", marginBottom: 4 }}>
                      Pilier {pid} ({pillar.title}) : aucun post généré.
                    </div>
                  );
                ...
                return (
                  ...
                  >
                    Pilier {pid} ({pillar.title}) : {dilts.count} trace{dilts.count > 1 ? "s" : ""} au niveau{" "}
                    {dilts.level} ({DILTS_LEVEL_NAMES[dilts.level] || ""}).
                    {dilts.target
                      ? " Cible : déposer une trace niveau " +
                        dilts.target +
                        " (" +
                        targetLabel +
                        "). Le recruteur qui scrolle ton profil doit voir la montée."
                      : " Pilier au sommet. Diversifie."}
                  </div>
                );
              })}
            </div>
      ...
      <div style={{ background: "#111125", borderRadius: 12, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ background: "#0d0d1a", borderRadius: 10, padding: 12, marginBottom: 12 }}>
          ...
          {recommendedPost && (
            <div>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                {recommendedPost.firstComment && (
                  <CopyBtn text={recommendedPost.firstComment} label="Copier le commentaire" />
                )}
                {pubRecommendation.dilts && pubRecommendation.dilts.target && (
                  <button
                    onClick={function () {
                      handlePrepareRegeneration(pubRecommendation.pillarId, pubRecommendation.dilts.target);
                    }}
                    style={{
                      padding: "5px 12px",
                      background: "#1a1a2e",
          ...
          {!recommendedPost && (
            <div style={{ fontSize: 10, color: "#495670" }}>
              Aucun post généré pour ce pilier. Retourne à la Forge pour générer tes posts LinkedIn.
            </div>
          )}
              ...
              {instructions.map(function (inst) {
                return (
                  <div key={inst.id} style={{ fontSize: 10, color: "#8892b0", marginBottom: 2 }}>
                    Pilier {inst.pillar_id} → régénérer au niveau {inst.target_dilts_level} (
                    {DILTS_LEVEL_NAMES[inst.target_dilts_level] || ""})
                  </div>
                );
              })}
            </div>
        ...
        {weekCount >= 1 && (
          <div style={{ background: "#0d0d1a", borderRadius: 10, padding: 12, marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 11, color: "#ccd6f6", lineHeight: 1.6 }}>
              Cette semaine, like 3-5 publications de{" "}
              <span style={{ color: CATEGORY_COLORS[contactDom.key], fontWeight: 600 }}>{contactDom.label}</span> dans
              ton secteur. Cible les profils que tu contacteras la semaine prochaine.
            </div>
            <div style={{ fontSize: 10, color: "#495670", marginTop: 6 }}>
              Le like précède le DM. Le recruteur qui reçoit ton message après avoir vu ton profil dans ses
              notifications réagit différemment.
            </div>
          </div>
        )}
        ...
        {weekCount >= 1 && (
          <div style={{ background: "#0d0d1a", borderRadius: 10, padding: 12 }}>
            ...
            <div style={{ fontSize: 11, color: "#ccd6f6", lineHeight: 1.6, marginBottom: 8 }}>
              Ton pilier {pubRecommendation.pillarId} (
              {pillars[pubRecommendation.pillarId - 1] && pillars[pubRecommendation.pillarId - 1].title}) attire des{" "}
              {contactDom.label}. Voici ton DM calibré.
            </div>
            <div style={{ fontSize: 10, color: "#495670", marginBottom: 8 }}>
              2-3 contacts cette semaine. Personnalise le prénom avant d'envoyer.
            </div>
            ...
            {contactScripts && contactScripts.dm && (
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ marginTop: 4 }}>
                  <CopyBtn text={contactScripts.dm} label="Copier le DM" />
                </div>
              </div>
            )}
            ...
            {contactScripts && contactScripts.email && (
              <div>
                ...
                >
                  {contactScripts.email}
                </div>
                <div style={{ marginTop: 4 }}>
                  <CopyBtn text={contactScripts.email} label="Copier l'email" />
                </div>
              </div>
            )}
            ...
            {!contactScripts && (
              <div style={{ fontSize: 10, color: "#495670" }}>
                Pas assez de briques validées pour générer un DM. Retourne à la Forge.
              </div>
            )}
          </div>
        )}
        ...
        {weekCount === 0 && (
          <div>
            ...
            <div style={{ fontSize: 11, color: "#495670", marginTop: 8 }}>
              Déclare ta première semaine pour débloquer les blocs LIKER et CONTACTER.
            </div>
          </div>
        )}
      </div>
