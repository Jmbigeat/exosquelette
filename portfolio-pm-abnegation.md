...
### Appel Découverte calibré sur l'échelle Méroé (24 mars 2026)
...
### Orchestration multi-agent — compétence documentée (25 mars 2026)

Le workflow Abneg@tion est un système multi-agent à 3 acteurs avec des scopes séparés. Agent 1 : Claude.ai (arbitrage, spec, stratégie — ne touche pas le repo). Agent 2 : Claude Code (implémentation — ne décide pas, exécute le prompt, propose un diff). Agent 3 : QA agent (vérification post-merge — ne code pas, ne décide pas, rapporte). JM est l'orchestrateur humain : il décide quel agent intervient à quel moment, il est le pont entre les agents (le rapport de Claude.ai devient le prompt de Claude Code), il review les outputs avant validation.
7 architectures multi-agent évaluées. 4 rejetées (BMAD, orchestration Task tool, Agent Analyst, Agent Dev autonome). 1 retenue (QA déterministe). 2 formes légères adoptées (two-instance kickoff pour parallélisme recherche/scaffold, Cowork pour maintenance docs). Réévaluation documentée le 25 mars : 3 rejets réévalués (Cursor, multi-agent, auto mode), 4 confirmés, 3 nouveaux items identifiés (Promptfoo, Cowork, Two-Instance).
La compétence n'est pas "utiliser du multi-agent." La compétence est savoir quand le multi-agent aide et quand il nuit, et documenter les deux. Le choix de NE PAS automatiser l'orchestration est une décision d'architecture. Document : arbitrages-orchestration-ia.md.
Preuve PM : évaluer 7 approches, en retenir 3 sous forme légère, documenter chaque rejet avec motif et condition de réévaluation. Le recruteur PM voit un jugement d'architecture, pas un follow-the-hype.
