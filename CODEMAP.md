...
**Rôle :** Données de référence — rôles cibles, KPIs par rôle, cauchemars recruteur, templates briques, piliers, questions duel, channels scripts, données marché.
**Exports :** `SCAN_STEPS_ACTIF`, `SCAN_STEPS_PASSIF`, `STEPS`, `KPI_REFERENCE`, `CAUCHEMAR_TEMPLATES_BY_ROLE`, `OFFER_URGENCY_KEYWORDS`, `SECTOR_KEYWORDS`, `TARGET_ROLES`, `ROLE_CLUSTERS`, `CAUCHEMARS_CIBLES`, `MARKET_DATA`, `BRICK_FIELDS`, `SEED_TEMPLATES`, `ROLE_PILLARS`, `DUEL_CRISES`, `DUEL_CONTRADICTIONS`, `DUEL_QUESTIONS`, `SCRIPT_CHANNELS`, `SIGNAL_TYPES`, `COMMENT_TOPICS`, `COMMENT_AVOID_PATTERNS`, `VISION_2026_FORMATS`, `ELASTICITY_LABELS`, `CATEGORY_LABELS`, `EFFORT_WEIGHTS`, `REPLACEMENT_DATA_BY_ROLE`, `STRESS_ANGLES`
