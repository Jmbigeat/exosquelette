...
export function scoreHook(hook, fullContent) {
  ...
  var isGeneric = genericPatterns.some(function (p) {
    return lowerNorm.indexOf(normalize(p)) !== -1;
  });
  ...
  if (ilYaMatch) {
    ...
    var hasTrenchant =
      afterMatch.indexOf("j'ai") !== -1 || afterMatch.indexOf("j'étais") !== -1 || afterMatch.indexOf("tout a") !== -1;
  ...
  var ennemiMarkers = [
    ...
    "la verite sur",
    "personne ne dit",
    "tout le monde repete",
  ];
  var provocMarkers = ["vraiment", "serieusement", "encore"];
  var hasEnnemi = ennemiMarkers.some(function (m) {
    return lowerNorm.indexOf(normalize(m)) !== -1;
  });
  if (!hasEnnemi && lower.indexOf("?") !== -1) {
    hasEnnemi = provocMarkers.some(function (m) {
      return lowerNorm.indexOf(normalize(m)) !== -1;
    });
  }
  ...
  var hasConsensus = consensusMarkers.some(function (m) {
    return lowerNorm.indexOf(normalize(m)) !== -1;
  });
  ...
  var hasAlienation = alienMarkers.some(function (m) {
    return lowerNorm.indexOf(normalize(m)) !== -1;
  });
  if (!hasAlienation)
    hasAlienation = judgmentWords.some(function (m) {
      return lowerNorm.indexOf(normalize(m)) !== -1;
    });
  if (!hasAlienation)
    hasAlienation = imperatives.some(function (m) {
      return lowerNorm.indexOf(normalize(m)) !== -1;
    });
  if (!hasAlienation)
    hasAlienation = provoc.some(function (m) {
      return lowerNorm.indexOf(normalize(m)) !== -1;
    });
  ...
  var tests = [
    {
      ...
      label: "So What",
      passed: soWhatPassed,
      message: soWhatPassed
        ? "L'accroche sort du lot."
        : "Ton accroche est générique. Elle ne provoque aucune réaction.",
    },
    {
      name: "ennemi",
      label: "Ennemi",
      passed: hasEnnemi,
      message: hasEnnemi
        ? "Antagoniste détecté. Le hook prend position."
        : "Aucun adversaire. Ton hook est trop consensuel.",
    },
    {
      name: "consensus",
      label: "Consensus",
      ...
      message: hasConsensus
        ? "Le hook est contre-intuitif."
        : "Tout le monde serait d'accord avec cette phrase. Casse le consensus.",
    },
    {
      name: "alienation",
      label: "Aliénation",
      passed: hasAlienation,
      message: hasAlienation
        ? "Le hook filtre. Il attire ta cible et repousse le reste."
        : "Ton hook ne filtre personne. Il plaît à tout le monde, donc à personne.",
    },
  ];

  var passedCount = tests.filter(function (t) {
    return t.passed;
  }).length;
...
export function analyzeBodyRetention(content) {
  ...
  if (/^[\-\*•]\s/m.test(content) || /^\d+\.\s/m.test(content)) {
    issues.push({
      type: "bullets",
      message: "Prose brute uniquement. Supprime les listes à puces. LinkedIn n'est pas un PowerPoint.",
    });
  }
  ...
  if (content.length > 1500) {
    issues.push({
      type: "length",
      message:
        "Ton post dépasse 1500 caractères. Coupe. Chaque phrase qui n'apporte rien affaiblit celles qui apportent.",
    });
  }
  ...
  paragraphs.forEach(function (p, i) {
    if (p.length > 250) {
      issues.push({
        type: "longParagraph",
        message: "Un paragraphe trop long perd le lecteur. Coupe en blocs de 2-3 lignes.",
      });
    }
  });
...
export function marieHookFullPost(fullContent, hook) {
  ...
  var hookHasEnnemi = ennemiMarkersBody.some(function (m) {
    return lowerHook.indexOf(normalize(m)) !== -1;
  });
  var bodyHasEnnemi = ennemiMarkersBody.some(function (m) {
    return lowerBody.indexOf(normalize(m)) !== -1;
  });
  ...
  var autoTests = [
    {
      name: "ennemiDev",
      label: "Ennemi développé",
      passed: ennemiDev,
      message: ennemiDev
        ? "L'antagoniste est développé dans le corps."
        : "Tu nommes un ennemi en accroche mais tu ne le développes pas. Le lecteur attend le combat.",
    },
    {
      name: "densite",
      ...
      passed: densiteOk,
      message: densiteOk
        ? "Chaque paragraphe porte du contenu."
        : paragraphs.length + " paragraphes pour " + factsCount + " fait(s). Densifie ou coupe.",
    },
  ];
  ...
  var qualitative = [
    {
      id: "authenticity",
      label: "Authenticité",
      question: "En lisant ce post à voix haute, est-ce que ça sonne comme toi ou comme un template ?",
    },
    {
      id: "memorability",
      label: "Mémorabilité",
      question: "Ferme les yeux. Quelle phrase reste ? Si aucune ne reste, le post est oubliable.",
    },
  ];
...
export function meroeAudit(fullContent, hook) {
  ...
  // Test ANGLE UNIQUE
  var hookKeywords = hook
    .split(/\s+/)
    .filter(function (w) {
      return (
        w.length > 4 &&
        ["dans", "avec", "pour", "mais", "plus", "cette", "votre", "c'est", "qu'il", "qu'on", "tout", "comme"].indexOf(
          w.toLowerCase().replace(/[^a-zà-ÿ]/gi, "")
        ) === -1
      );
    })
    .map(function (w) {
      return normalize(w.replace(/[^a-zà-ÿ]/gi, ""));
    });
  var lastParagraph = paragraphs.length > 0 ? normalize(paragraphs[paragraphs.length - 1]) : "";
  var keywordsInLast = hookKeywords.filter(function (k) {
    return k.length > 4 && lastParagraph.indexOf(k) !== -1;
  });
  ...
  paragraphs.forEach(function (p, i) {
    ...
    var sentences = p.split(/\.\s/).filter(function (s) {
  ...
  var miroirAutoTests = [
    {
      ...
      passed: angleUnique,
      message: angleUnique
        ? "Le post reste sur son axe."
        : "Ton post commence sur un sujet et finit sur un autre. Une idée. Un post.",
    },
    {
      name: "structure",
      label: "Structure",
      passed: structureOk,
      message: structureOk
        ? "Structure propre. Chaque paragraphe porte une idée."
        : "Un paragraphe contient " + maxSentences + " phrases. Coupe. 1 paragraphe = 1 idée = 2 phrases max.",
    },
  ];
  ...
  var miroirQualitative = [
    {
      ...
      label: "Force du hook",
      question:
        "Le hook est-il fort parce qu'il est incarné (vécu), ou fort parce qu'il est malin (technique) ? Le premier tient. Le second lasse.",
    },
    {
      id: "incarnation",
      label: "Incarnation",
      question:
        "Vois-tu une image en lisant ce post (un lieu, un moment, un visage) ? Si non, le post est abstrait. Le concret ancre.",
    },
  ];
  ...
  var luisEnrique = [
    {
      ...
      question: "Ce post apporte-t-il une utilité réelle au lecteur ou est-ce du bruit de plus sur son fil ?",
    },
    { id: "clarity_vs_ego", label: "Clarté vs ego", question: "Es-tu clair ou est-ce que tu te regardes écrire ?" },
    {
      id: "reader_vs_self",
      label: "Lecteur vs auteur",
      question: "Ce post parle-t-il à ceux que tu veux aider ou à ton propre ego ?",
    },
  ];
...
export function generateHookVariants(originalHook, failedTests, post) {
  ...
  failedTests.forEach(function (testName) {
    if (testName === "ennemi" && variants.length < 2) {
      ...
      variants.push(
        hookClean.replace(/^(.{1})/, function (m) {
          return m.toUpperCase();
        }) +
          " ? Non. " +
          (words.length > 5
            ? words.slice(0, 5).join(" ") + " n'est pas la solution."
    ...
    if (testName === "alienation" && variants.length < 2) {
      ...
      variants.push(
        "Ton " +
          (words.length > 3 ? words.slice(1, 4).join(" ") : hookClean.toLowerCase()) +
          " est vide. Et tu le sais."
      );
    }
