...
export function auditExternalCV(cvText, offerAnalysis, cauchemars) {
  ...
  var decisionMarkers = ["décidé", "arbitré", "tranché", "négocié", "convaincu", "restructuré", "construit", "lancé", "piloté", "conçu", "imposé", "transformé"];
  var executionMarkers = ["participé", "contribué", "aidé", "assisté", "géré", "supporté", "intervenu"];
  var decisionCount = 0;
  var executionCount = 0;
  decisionMarkers.forEach(function(m) {
    var re = new RegExp(m, "gi");
    var matches = lower.match(re);
    if (matches) decisionCount += matches.length;
  });
  executionMarkers.forEach(function(m) {
    var re = new RegExp(m, "gi");
    var matches = lower.match(re);
  ...
  var hasTension = /malgré|alors que|face à|sous contrainte|en dépit de|dans un contexte/i.test(cvText);
