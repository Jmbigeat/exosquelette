...
export function auditExternalCV(cvText, offerAnalysis, cauchemars) {
  ...
  if (detectEnglish(cvText)) {
    return {
      ...
      tests: [],
      message:
        "Ton CV semble rédigé en anglais. L'outil analyse les marqueurs français. Colle la version française de ton CV pour un diagnostic fiable.",
    };
  }
  ...
  tests.push({
    ...
    passed: t1Passed,
    label: t1Passed
      ? "Ton CV couvre " + coveredCauch + "/" + totalCauch + " cauchemars du recruteur"
      : "Ton CV ne répond à aucun cauchemar du recruteur",
    message: t1Passed
      ? ""
      : "Le recruteur cherche quelqu'un qui résout " +
        (mainCauchLabel || "le problème principal") +
        ". Ton CV n'en parle pas.",
  });
  ...
  tests.push({
    ...
    message: kpiLinked
      ? ""
      : "Le recruteur cherche un chiffre sur " +
        (kpiName || "le KPI principal") +
        ". Ton CV ne contient aucune donnée sur ce sujet.",
  });
  ...
  tests.push({
    ...
    passed: t3Passed,
    label: t3Passed
      ? "Ton CV montre des décisions, pas juste de l'exécution"
      : "Ton CV liste des tâches, pas des décisions",
    message: t3Passed
      ? ""
      : "Le recruteur cherche " +
        (eLabel === "élastique"
          ? "de l'influence pure"
          : eLabel === "sous_pression"
            ? "de la résistance sous pression"
            : "des décisions") +
        ". Ton CV dit 'participé à' et 'contribué à'. Il ne dit pas ce que TU as décidé.",
  });
  ...
  tests.push({
    ...
    message: t4Passed
      ...
      : "Ton CV dit '" +
        (foundToxic[0] ||
          weakVerbs.find(function (wv) {
            return lower.indexOf(wv.toLowerCase()) !== -1;
          }) ||
          "") +
        "'. " +
        totalGeneric +
        " candidats sur 10 utilisent ce mot. Le recruteur l'ignore.",
  });

  // ── Test 5 — Calibration ATMT ──
  var hasAccroche = /\d+/.test(cvText);
  var hasTension =
    /malgré|alors que|face à|sous contrainte|en dépit de|dans un contexte|bug|bugs|problème|problèmes|racine commune|dette|régression|en urgence|à froid|sans budget|pas de budget|zéro budget|en retard|retard|bloqué|cassé|perdu|échoué|crise|panne|incident/i.test(
      cvText
    );
  var hasMethode =
    /rattrapé|réduit|construit|lancé|restructuré|négocié|piloté|déployé|transformé|doublé|triplé|multiplié/i.test(
      cvText
    );
  var detectedRole = offerAnalysis ? offerAnalysis.detectedRoleId : "";
  var roleLabel = offerAnalysis ? offerAnalysis.detectedRoleLabel : "";
  var roleKeywords = (roleLabel || "")
    .toLowerCase()
    .split(/[\s\/\(\)—,]+/)
    .filter(function (w) {
      return w.length > 3;
  ...
  var layers = [];
  var missingLayers = [];
  if (hasAccroche) layers.push("Accroche");
  else missingLayers.push("Accroche");
  if (hasTension) layers.push("Tension");
  else missingLayers.push("Tension");
  if (hasMethode) layers.push("Méthode");
  else missingLayers.push("Méthode");
  if (hasTransfert) layers.push("Transfert");
  else missingLayers.push("Transfert");
