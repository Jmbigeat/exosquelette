...
function testNotGeneric(content, bricks) {
  ...
  if (validated.length === 0)
    return {
      passed: false,
      message:
        "Ce livrable est trop générique. Il ne contient aucun élément de ton Coffre-Fort. Retourne à la Forge et valide au moins 1 brique avec un chiffre ou un contexte précis.",
      correctionHint: "Injecte au moins 1 chiffre ou contexte spécifique depuis les briques du candidat.",
    };
...
function testContainsProof(content, bricks) {
  ...
  return {
    ...
    message: found
      ? ""
      : "Ce livrable ne contient aucune preuve. Le recruteur lit des affirmations sans fait. Blinde au moins 1 brique dans la Forge.",
...
function testRecipientFirst(type, content, cauchemars, audience) {
  // Fiche de combat: first cauchemar listed must have coverage > 0
  if (type === "fiche_combat") {
    var lower = (content || "").toLowerCase();
    var cauchSection = lower.indexOf("cauchemars du décideur");
    if (cauchSection === -1) {
      return {
        passed: false,
        message: "La fiche ne contient pas de section cauchemars.",
        correctionHint: "Trie les cauchemars par couverture — le candidat défend ce qu'il prouve.",
      };
    }
    // Check that first cauchemar has "brique" mention (coverage > 0)
    var afterCauch = lower.slice(cauchSection, cauchSection + 500);
    var hasCoverage = /\d+\s*brique/.test(afterCauch);
    return {
      passed: hasCoverage,
      message: hasCoverage ? "" : "Les cauchemars ne sont pas triés par couverture réelle.",
      correctionHint: hasCoverage ? "" : "Trie les cauchemars par couverture — le candidat défend ce qu'il prouve.",
    };
  }
  ...
  if (type === "cv") {
    ...
    if (idxCompetences !== -1 && idxRealisations !== -1 && idxCompetences < idxRealisations) {
      return {
        passed: false,
        message:
          "Ton CV place les compétences avant les réalisations. Le recruteur scanne en 6 secondes. Il cherche des résultats, pas des étiquettes. Inverse l'ordre.",
  ...
  if (type === "interview_prep") {
    ...
    return {
      ...
      message: passed
        ? ""
        : "Une ou plusieurs versions entretien parlent de toi avant de parler du problème. Chaque version ouvre sur le contexte du destinataire.",
      correctionHint: passed
        ? ""
        : "Chaque version entretien ouvre sur le contexte du destinataire, pas sur le candidat.",
    };
  }
  ...
  if (audience === "internal") {
    ...
    var costWords =
      /coût|remplacement|vacance|risque|impact|productivité|turnover|départ|perte|budget|charge|recrutement/i;
    var selfWords = /^(je |j'ai |j'|mon parcours|mes compétences|ma carrière)/i;
    if (selfWords.test(first)) {
      return {
        passed: false,
        message:
          "Ton livrable parle de toi avant de parler du coût pour ton manager. Le risque de ton départ ouvre. Tes preuves suivent.",
  ...
  if (selfOpeners.test(first)) {
    return {
      passed: false,
      message:
        "Ton livrable parle de toi avant de parler du recruteur. Inverse. Le cauchemar du recruteur ouvre. Tes preuves suivent.",
...
function testChannelCalibration(type, content) {
  ...
  if (type === "dm") {
    if (len > 300) {
      passed = false;
      msg = "DM trop long (" + len + " car.). Maximum 300 caractères.";
    }
  } else if (type === "email") {
    if (len > 1500) {
      passed = false;
      msg = "Email trop long (" + len + " car.). Maximum 1500 caractères.";
    }
  } else if (type === "cv") {
    ...
    if (bulletLines.length > 5) {
      passed = false;
      msg = "CV contient " + bulletLines.length + " lignes. Maximum 5 pour un scan en 6 secondes.";
    }
    var longParas = bulletLines.filter(function (l) {
      return (l.match(/\./g) || []).length > 2;
    });
    if (longParas.length > 0) {
      passed = false;
      msg = (msg ? msg + " " : "") + "Certaines lignes CV dépassent 2 phrases.";
    }
  } else if (type === "bio") {
    if (len > 300) {
      passed = false;
      msg = "Bio trop longue (" + len + " car.). Maximum 300 caractères.";
    }
  } else if (type === "plan30j") {
    var weekMarkers = (content || "").match(/S[1-4]|semaine\s*[1-4]/gi) || [];
    if (weekMarkers.length < 4) {
      passed = false;
      msg = "Plan 30j ne contient pas les 4 marqueurs de semaine (S1-S4).";
    }
  } else if (type === "plan90j") {
    var monthMarkers = (content || "").match(/M[1-3]|mois\s*[1-3]/gi) || [];
    if (monthMarkers.length < 3) {
      passed = false;
      msg = "Plan 90j ne contient pas les 3 marqueurs de mois (M1-M3).";
    }
  } else if (type === "report") {
    if (!/\d+\s*[K€kM]|\d+\s*€/.test(content || "")) {
      passed = false;
      msg = "Le rapport ne contient aucun chiffre de coût en €.";
    }
  } else if (type === "argument") {
    if (len > 2000) {
      passed = false;
      msg = "Argumentaire trop long (" + len + " car.). Maximum 2000 caractères.";
    }
  } else if (type === "posts") {
    if (len > 1500) {
      passed = false;
      msg = "Post trop long (" + len + " car.). Maximum 1500 caractères.";
    }
    if (/^[•\-]\s/m.test(content || "")) {
      passed = false;
      msg = (msg ? msg + " " : "") + "Le post contient des listes à puces. Prose uniquement.";
    }
  } else if (type === "followup") {
    if (len > 2000) {
      passed = false;
      msg = "Message post-entretien trop long (" + len + " car.). Maximum 2000 caractères.";
    }
  } else if (type === "email_signature") {
    if (len > 80) {
      passed = false;
      msg = "Signature email trop longue (" + len + " car.). Maximum 80 caractères.";
    }
  } else if (type === "fiche_combat") {
    if (len < 1500) {
      passed = false;
      msg = "Fiche de combat trop courte (" + len + " car.). Minimum 1500 caractères.";
    } else if (len > 2500) {
      passed = false;
      msg = "Fiche de combat trop longue (" + len + " car.). Maximum 2500 caractères. Compresse les parades. 1 phrase par parade.";
    }
  } else if (type === "negotiation_bundle") {
    if (len > 8000) {
      passed = false;
      msg = "Dossier négociation trop long (" + len + " car.). Condense les sections.";
    }
  } else if (type === "questions") {
    var qMatches = (content || "").match(/QUESTION\s+\d+/gi) || [];
    if (qMatches.length < 5 || qMatches.length > 8) {
      passed = false;
      msg = "Questions entretien : " + qMatches.length + " questions (attendu 5-8).";
    }
    var levelMatches = (content || "").match(/Niveau\s+[3-6]/gi) || [];
    if (levelMatches.length < qMatches.length) {
      passed = false;
      msg = (msg ? msg + " " : "") + "Certaines questions n'ont pas de niveau (3-6).";
    }
    ...
    if (len > 10000) {
      passed = false;
      msg = "Préparation entretien trop longue. Condense les versions.";
    }
  }
  ...
  return {
    ...
    message: passed
      ? ""
      : "Ce livrable dépasse le format du canal. " + msg + " Un " + type + " trop long perd le lecteur.",
...
export function auditDeliverable(type, content, bricks, cauchemars, audience) {
  ...
  if (a.passed) {
    ...
    failedList.push({ principle: "A", message: a.message });
    hints.push(a.correctionHint);
  }
  if (b.passed) {
    passedList.push("B");
  } else {
    failedList.push({ principle: "B", message: b.message });
    hints.push(b.correctionHint);
  }
  if (c.passed) {
    passedList.push("C");
  } else {
    failedList.push({ principle: "C", message: c.message });
    hints.push(c.correctionHint);
  }
  if (d.passed) {
    passedList.push("D");
  } else {
    failedList.push({ principle: "D", message: d.message });
    hints.push(d.correctionHint);
  }
  ...
  return {
    ...
    passed: passedList,
    failed: failedList,
    correctionHints: hints.filter(function (h) {
      return h && h.length > 0;
    }),
  };
}
