...
export async function loadBrewWeeks(userId) {
  ...
  var res = await client.from("brew_weeks").select("*").eq("user_id", userId).order("week_start", { ascending: false });
...
export async function saveBrewWeek(userId, weekStart, pillarRows) {
  ...
  var res = await client.from("brew_weeks").upsert(rows, { onConflict: "user_id,week_start,pillar_id" });
  return res;
}
...
export async function markInstructionDone(instructionId) {
  var client = getClient();
  var res = await client.from("brew_instructions").update({ status: "done" }).eq("id", instructionId);
  return res;
}
...
export async function isWeekDeclared(userId) {
  ...
  var res = await client.from("brew_weeks").select("id").eq("user_id", userId).eq("week_start", weekStart).limit(1);
