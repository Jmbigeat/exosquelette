...
import { detectBridges } from "../sprint/analysis.js";
...
export function generateOnePager(
  ...
  var bridges = detectBridges(bricks, cauchemars);
  var parcourBlock = buildParcourBlock(selected, bridges);
...
function buildParcourBlock(selected, bridges) {
  ...
  // Fil conducteur from bridges (16o)
  if (bridges && bridges.length > 0) {
    var best = bridges[0];
    var order = { cauchemar: 3, kpi: 2, keyword: 1 };
    for (var bi = 1; bi < bridges.length; bi++) {
      if ((order[bridges[bi].type] || 0) > (order[best.type] || 0)) {
        best = bridges[bi];
      }
    }
    var filLabel = best.label;
    if (best.contextA && best.contextB) {
      filLabel += " (" + best.contextA + " → " + best.contextB + ")";
    }
    lines.push("Fil conducteur : " + filLabel);
  }
