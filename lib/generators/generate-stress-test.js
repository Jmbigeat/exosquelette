...
export function generateStressTest(brick, targetRoleId, offersArray) {
  ...
  // ===== SOURCE 4: ANGLES ÉLASTIQUES (frictions + intersections, 16k) =====
  if (brick.brickType === "elastic" || brick.elasticity === "élastique") {
    var fricPool = STRESS_ANGLES.friction;
    angles.push({
      type: "friction",
      label: "Friction de transfert",
      attack: fricPool[Math.abs(hashCode(brick.id + "fric")) % fricPool.length],
      defense:
        "Le recruteur teste ta lucidité. Un candidat qui connaît les limites de son analogie est crédible. Un candidat qui survend le transfert est suspect.",
      source: "elastic",
    });

    var intPool = STRESS_ANGLES.intersection;
    angles.push({
      type: "intersection",
      label: "Intersection cachée",
      attack: intPool[Math.abs(hashCode(brick.id + "int")) % intPool.length],
      defense:
        "Une intersection inconsciente est plus forte qu'une analogie consciente. Le recruteur entend la différence entre « le sport m'a appris la persévérance » (cliché) et « je chronomètre chaque tâche comme mes séries » (preuve).",
      source: "elastic",
    });
  }
