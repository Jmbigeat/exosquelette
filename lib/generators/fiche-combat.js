...
import { ROLE_CLUSTERS, SALARY_RANGES_BY_ROLE, ROLE_VALUE_RATIO, SENIORITY_LEVELS, SENIORITY_CALIBRATION, INTERVIEW_REFLEXES } from "../sprint/references.js";
...
export function generateFicheCombat(bricks, targetRoleId, cauchemars, signature, seniorityLevel, currentSalary, duelResults, offerSignals, hints) {
  ...
  var seniorityKey = sLevel === "leader" ? "leader" : sLevel === "manager" ? "manager" : "ic";
  var sortedReflexes = INTERVIEW_REFLEXES.slice().sort(function (a, b) {
    var aMatch = a.dominantFor.indexOf(seniorityKey) !== -1 ? 0 : 1;
    var bMatch = b.dominantFor.indexOf(seniorityKey) !== -1 ? 0 : 1;
    return aMatch - bMatch;
  });

  var reflexBlock = sortedReflexes.map(function (r) {
    var isDominant = r.dominantFor.indexOf(seniorityKey) !== -1;
    var line = r.label + (isDominant ? " ⚠️" : "") + "\n";
    line += "Risque : " + r.risk + "\n";
    line += "Parade : " + r.counter;
    return line;
  }).join("\n\n");
  ...
  blocks.push("");
  blocks.push("RÉFLEXES À SURVEILLER");
  blocks.push(reflexBlock);
