...
import { ROLE_CLUSTERS, SENIORITY_LEVELS, SENIORITY_CALIBRATION, INTEGRATION_MILESTONES, getRoleCluster } from "../sprint/references.js";
...
export function generateDiscoveryCall(bricks, targetRoleId, cauchemars, offerSignals, seniorityLevel, signature, hints) {
  ...
  // ── Q6 — INTÉGRATION (16i) ──
  var integCluster = getRoleCluster(targetRoleId);
  var m1Specific = INTEGRATION_MILESTONES.month1.byCluster[integCluster];
  if (m1Specific && m1Specific[0]) {
    questions.push({
      label: "Intégration",
      text: "À quoi ressemble un premier mois réussi pour ce poste ? Qu'est-ce que la personne doit avoir livré à J+30 pour que vous sachiez que c'était le bon choix ?",
      breadcrumb: "Tu connais déjà la réponse probable : " + m1Specific[0].toLowerCase() + ". Écoute si le recruteur confirme ou corrige.",
      signalPositif: "Le recruteur décrit des jalons précis. Il a pensé l'intégration. Bon signe sur le management.",
      signalAlerte: "Le recruteur n'a pas de réponse. L'intégration sera improvisée.",
    });
  }
