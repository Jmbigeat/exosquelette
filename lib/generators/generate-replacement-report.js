...
export function generateReplacementReport(bricks, targetRoleId, currentSalary, internalSignals, hints) {
  ...
  var recruitmentCost = replData ? replData.recruitmentCost : Math.round(salaryBase * 0.2);
  ...
  var rampUpCost = Math.round(((salaryBase * rampUpMonths) / 12) * 0.4);
  ...
  if (elasticBricks.length > 0) {
    report +=
      "• " +
      elasticBricks.length +
      " compétence" +
      (elasticBricks.length > 1 ? "s" : "") +
      " élastique" +
      (elasticBricks.length > 1 ? "s" : "") +
      " (non remplaçable par l'IA)\n";
  }
  if (blindedBricks.length > 0) {
    report +=
      "• " +
      blindedBricks.length +
      " résultat" +
      (blindedBricks.length > 1 ? "s" : "") +
      " chiffré" +
      (blindedBricks.length > 1 ? "s" : "") +
      " :\n";
  ...
  report += "\n---\n";
  report +=
    "Ce rapport n'est pas une menace. C'est une cartographie. Le manager qui retient coûte moins cher que le manager qui remplace.";
