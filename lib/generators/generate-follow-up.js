...
export function generateFollowUp(followUpInput, bricks, targetRoleId, cauchemars, vault, hints) {
  ...
  var challenges = (input.challenges || []).filter(function (c) {
    return c && c.trim().length >= 5;
  });
  ...
  // Scoring specific to this generator, not extracted (token overlap matching per challenge)
  ...
  if (validated.length >= 2) {
    ...
    if (!proofText) {
      var armoredBricks = validated.filter(function (b) {
        return b.blinded || (b.armorScore && b.armorScore >= 3);
      });
      ...
      var action = caseCore.actionVerb
        ? caseCore.actionVerb.charAt(0).toUpperCase() + caseCore.actionVerb.slice(1)
        : "Action ciblée";
      var result = caseCore.resultNumber || caseCore.mainNumber || "résultat mesurable";
      proofText =
        "Un élément qui peut être utile :\nSituation : " +
        situation +
        ".\nAction : " +
        action +
        ".\nRésultat : " +
  ...
  var nextStep =
    "Prochaine étape : je suis disponible pour approfondir le sujet " +
    (firstChallenge.length > 50 ? firstChallenge.slice(0, 47) + "..." : firstChallenge) +
    " si c'est pertinent.\n" +
    closingQ;
