import { KPI_REFERENCE, INTEGRATION_MILESTONES, getRoleCluster } from "../sprint/references.js";
...
export function generatePlan30jRH(bricks, targetRoleId, offerSignals, hints) {
  ...
  // Scoring specific to this generator, not extracted (prefers blinded brick covering cauchemar)
  ...
  if (strongestBrick) {
    ...
    plan +=
      "• Méthode testée : " +
      (s2Action ? s2Action : "action ciblée") +
      (s2Num ? " — résultat obtenu : " + s2Num : "") +
      "\n";
  }
  ...
  var decisionBrick = validated.find(function (b) {
  ...
  // ── JALONS D'INTÉGRATION (16i) ──
  var cluster = getRoleCluster(targetRoleId);
  var w1 = INTEGRATION_MILESTONES.week1;
  var m1 = INTEGRATION_MILESTONES.month1;

  plan += "\n--- Mes jalons d'intégration ---\n\n";
  plan += w1.label + "\n";
  w1.universal.forEach(function (item) {
    plan += "• " + item + "\n";
  });
  if (w1.byCluster[cluster]) {
    w1.byCluster[cluster].forEach(function (item) {
      plan += "• " + item + "\n";
    });
  }
  plan += "\n" + m1.label + "\n";
  m1.universal.forEach(function (item) {
    plan += "• " + item + "\n";
  });
  if (m1.byCluster[cluster]) {
    m1.byCluster[cluster].forEach(function (item) {
      plan += "• " + item + "\n";
    });
  }
