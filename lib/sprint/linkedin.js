...
import {
  ...
  detectDiltsLevel,
  computeDiltsTarget,
  selectBrickForDiltsTarget,
  getDiltsPlafond,
  getDiltsCeilingForOutput,
} from "./dilts.js";
...
export function generateLinkedInPosts(bricks, vault, targetRoleId) {
  ...
  // Chantier 19 — per-post-index target calibration
  // Post 1 = Comportement (2), Post 2 = Capacités (3), Post 3 = Croyances (4), Post 4 = Identité (5)
  var perPostTargets = [2, 3, 4, 5];
  ...
  sortedPillars.slice(0, 4).forEach(function (pillar, postIdx) {
    // Chantier 19 — use per-post target, capped by global targetLevel + 2
    var postTargetLevel = Math.min(perPostTargets[postIdx] || targetLevel, targetLevel + 2);
    var postCalibration = DILTS_CALIBRATION[postTargetLevel] || DILTS_CALIBRATION[2];

    // BRICK SELECTION — biased toward target Dilts level
    var brick = selectBrickForDiltsTarget(validated, postTargetLevel, usedBrickIds);
    if (!brick) return;
    ...
    if (pillar.source === "take" && pillar.desc) {
      ...
      these =
        pillar.title + ". " + (pillar.desc || "C'est une réalité que l'expérience révèle, pas les articles de blog.");
    }
    ...
    situation = situation
      .replace(/[\+\-]?\d+[\.,]?\d*\s*[%KM€]*/g, "")
      .replace(
        /\d+[\.,]?\d*\s*(mois|semaines|jours|ans|comptes|commerciaux|personnes|équipes|EUR|euros|millions?)/gi,
        "$1"
      )
      .replace(/\(\s*\)/g, "")
      .replace(/\s{2,}/g, " ")
    ...
    if (postCalibration.framingOpen) {
      situation = postCalibration.framingOpen + situation;
    } else if (brick.brickType === "cicatrice") {
      situation = "J'ai appris ça à mes dépens. " + situation;
    } else if (brick.brickCategory === "decision") {
      situation = "J'ai dû trancher dans ce contexte. " + situation;
    } else if (brick.brickCategory === "influence") {
      situation = "Le plus dur n'était pas la méthode. " + situation;
    }

    // LINE 9 — Closing calibré par niveau Dilts
    var closing = postCalibration.framingClose;
    // Ajoute une question ouverte adaptée au niveau
    var question = "";
    if (postTargetLevel <= 2) {
      question =
        pillar.title && pillar.title.length < 60
    ...
    var postObj = {
      ...
      contextLine: "Ce post attire les profils sensibles à " + (pillar.title || "ce thème").replace(/\.$/, "") + ".",
      diltsLevel: dilts.dominant,
      diltsName: dilts.name,
    ...
    var expertPassed =
      expert.miroir.filter(function (m) {
        return m.passed;
      }).length +
      expert.luisEnrique.filter(function (l) {
        return l.passed;
      }).length;
    ...
    postObj.diltsTarget = postTargetLevel;
...
export function scoreHook(text) {
  ...
  var soWhat =
    lower.indexOf("?") !== -1 ||
    lower.indexOf("personne") !== -1 ||
    lower.indexOf("jamais") !== -1 ||
    lower.indexOf("problème") !== -1 ||
    lower.indexOf("coûte") !== -1 ||
    lower.indexOf("coute") !== -1 ||
    lower.indexOf("erreur") !== -1;

  // B. Ennemi — antagoniste identifié
  var ennemi =
    lower.indexOf("pas") !== -1 ||
    lower.indexOf("jamais") !== -1 ||
    lower.indexOf("personne ne") !== -1 ||
    lower.indexOf("erreur") !== -1 ||
    lower.indexOf("mythe") !== -1 ||
    lower.indexOf("mensonge") !== -1 ||
    lower.indexOf("faux") !== -1 ||
    lower.indexOf("à tort") !== -1;

  // C. Consensus — dit le contraire de ce que tout le monde pense
  var consensus =
    lower.indexOf("contrairement") !== -1 ||
    lower.indexOf("tout le monde") !== -1 ||
    lower.indexOf("consensus") !== -1 ||
    lower.indexOf("on pense que") !== -1 ||
    lower.indexOf("idée reçue") !== -1 ||
    lower.indexOf("idee recue") !== -1 ||
    lower.indexOf("à rebours") !== -1;

  // D. Aliénation — prise de position risquée
  var alienation =
    lower.indexOf("refuse") !== -1 ||
    lower.indexOf("déteste") !== -1 ||
    lower.indexOf("n'en peux plus") !== -1 ||
    lower.indexOf("stop") !== -1 ||
    lower.indexOf("marre") !== -1 ||
    lower.indexOf("insupportable") !== -1 ||
    ennemi;

  // E. Authenticité — vécu ou template
  var authenticite =
    lower.indexOf("j'ai") !== -1 ||
    lower.indexOf("j'étais") !== -1 ||
    lower.indexOf("mon") !== -1 ||
    lower.indexOf("ma ") !== -1 ||
    lower.indexOf("quand j") !== -1 ||
    lower.indexOf("ce jour") !== -1;
...
export function analyzeBodyRetention(text) {
  ...
  if (paragraphs.length >= 3) {
    ...
    middle.forEach(function (p, i) {
      if (p.length > 300)
        issues.push("Paragraphe " + (i + 2) + " trop long (" + p.length + " car.). Découpe ou resserre.");
      var hasHook =
        p.indexOf("?") !== -1 ||
        p.indexOf("!") !== -1 ||
        p.toLowerCase().indexOf("mais") !== -1 ||
        p.toLowerCase().indexOf("pourtant") !== -1 ||
        p.toLowerCase().indexOf("sauf que") !== -1;
      if (!hasHook && p.length > 100)
        issues.push(
          "Paragraphe " + (i + 2) + " : pas de relance. Ajoute une tension pour tirer le lecteur au suivant."
        );
    });
  }
...
export function expertWritingAudit(text) {
  ...
  miroir.push({
    ...
    passed: hookStrong,
    detail: hookStrong
      ? "Accroche solide (" + hookResult.score + "/10)"
      : "Accroche faible (" + hookResult.score + "/10). Reformule.",
  });
  ...
  paragraphs.forEach(function (p) {
    ...
    if (pl.indexOf("churn") !== -1 || pl.indexOf("retention") !== -1 || pl.indexOf("rétention") !== -1)
      detectedSubjects++;
    if (pl.indexOf("pipeline") !== -1 || pl.indexOf("prospection") !== -1) detectedSubjects++;
    if (pl.indexOf("management") !== -1 || pl.indexOf("equipe") !== -1 || pl.indexOf("équipe") !== -1)
      detectedSubjects++;
    if (pl.indexOf("produit") !== -1 || pl.indexOf("roadmap") !== -1) detectedSubjects++;
    if (pl.indexOf("negociation") !== -1 || pl.indexOf("négociation") !== -1 || pl.indexOf("deal") !== -1)
      detectedSubjects++;
    subjects.push(detectedSubjects);
  });
  var uniqueTopics = subjects.filter(function (s) {
    return s > 0;
  }).length;
  var singleTopic = uniqueTopics <= 2;
  miroir.push({
    label: "Clarté de l'angle",
    passed: singleTopic,
    detail: singleTopic ? "Un sujet par post. OK." : "Trop de sujets mélangés. Recentre.",
  });
  ...
  miroir.push({
    label: "Incarnation",
    passed: incarnation,
    detail: incarnation ? "Vécu personnel détecté." : "Trop générique. Ajoute une expérience vécue.",
  });

  // Structure — 1 idée par paragraphe
  var goodStructure =
    paragraphs.length >= 3 &&
    paragraphs.every(function (p) {
      return p.length < 400;
    });
  miroir.push({
    label: "Structure",
    passed: goodStructure,
    detail: goodStructure ? "Paragraphes bien découpés." : "Découpe en blocs plus courts. 1 idée = 1 paragraphe.",
  });
  ...
  // Utilité vs bruit
  var hasInsight =
    lower.indexOf("apprend") !== -1 ||
    lower.indexOf("découvert") !== -1 ||
    lower.indexOf("compris") !== -1 ||
    lower.indexOf("résultat") !== -1 ||
    lower.indexOf("concret") !== -1 ||
    lower.indexOf("methode") !== -1 ||
    lower.indexOf("méthode") !== -1;
  luisEnrique.push({
    label: "Utilité vs bruit",
    passed: hasInsight,
    detail: hasInsight
      ? "Le lecteur apprend quelque chose."
      : "Le lecteur n'apprend rien de concret. Ajoute un enseignement.",
  });

  // Clarté vs complaisance
  var filler = (
    lower.match(
      /en effet|en fait|il faut dire que|force est de constater|il est important de|fondamentalement|évidemment/g
    ) || []
  ).length;
  var nofiller = filler < 2;
  luisEnrique.push({
    label: "Clarté vs complaisance",
    passed: nofiller,
    detail: nofiller ? "Phrases directes." : filler + " expressions creuses détectées. Coupe le gras.",
  });
  ...
  luisEnrique.push({
    label: "Lecteur vs ego",
    passed: readerFocused,
    detail: readerFocused ? "Équilibre je/vous correct." : "Trop de 'je' (" + jeCount + "). Réoriente vers le lecteur.",
  });
...
export function generateFirstComment(post, bricks, vault) {
  ...
  if (brickType === "cicatrice") {
    ...
    opener =
      "Un détail que je n'ai pas mis dans le post : le plus dur n'était pas les chiffres. C'était de convaincre les gens autour de la table.";
  ...
  if (pillarSource === "take") {
    ...
    bridge =
      " Et ça rejoint une conviction que j'observe sur le terrain : " +
      pillarTitle.charAt(0).toLowerCase() +
      pillarTitle.slice(1).replace(/\.$/, "") +
      ".";
  }
  ...
  if (
    pillarLower.indexOf("churn") !== -1 ||
    pillarLower.indexOf("r" + "é" + "tention") !== -1 ||
    pillarLower.indexOf("retention") !== -1
  ) {
    question = "Quel signal vous alerte avant qu'un client parte ?";
  } else if (
    pillarLower.indexOf("pipeline") !== -1 ||
    pillarLower.indexOf("prospection") !== -1 ||
    pillarLower.indexOf("vente") !== -1 ||
    pillarLower.indexOf("commercial") !== -1
  ) {
    question = "Quel deal perdu vous a le plus appris sur votre process de vente ?";
  } else if (
    pillarLower.indexOf("equipe") !== -1 ||
    pillarLower.indexOf("équipe") !== -1 ||
    pillarLower.indexOf("management") !== -1 ||
    pillarLower.indexOf("talent") !== -1
  ) {
    question = "Quel signe vous dit qu'un collaborateur va partir avant qu'il ne le dise ?";
  } else if (
    pillarLower.indexOf("produit") !== -1 ||
    pillarLower.indexOf("roadmap") !== -1 ||
    pillarLower.indexOf("feature") !== -1 ||
    pillarLower.indexOf("backlog") !== -1
  ) {
    question = "Quelle feature avez-vous tuée alors que tout le monde la voulait ?";
  } else if (
    pillarLower.indexOf("n" + "é" + "gociation") !== -1 ||
    pillarLower.indexOf("negociation") !== -1 ||
    pillarLower.indexOf("salaire") !== -1 ||
    pillarLower.indexOf("deal") !== -1
  ) {
    question = "Quelle négociation vous a appris que le premier chiffre posé décide de tout ?";
  } else if (
    pillarLower.indexOf("ia") !== -1 ||
    pillarLower.indexOf("intelligence artificielle") !== -1 ||
    pillarLower.indexOf("mod" + "è" + "le") !== -1 ||
    pillarLower.indexOf("modele") !== -1
  ) {
    question = "Quel projet IA vous a appris que le vrai obstacle n'est pas technique ?";
  } else if (
    pillarLower.indexOf("process") !== -1 ||
    pillarLower.indexOf("ops") !== -1 ||
    pillarLower.indexOf("workflow") !== -1 ||
    pillarLower.indexOf("simplifier") !== -1
  ) {
    question = "Quel process avez-vous supprimé et que personne n'a regretté ?";
  } else if (
    pillarLower.indexOf("dette") !== -1 ||
    pillarLower.indexOf("code") !== -1 ||
    pillarLower.indexOf("tech") !== -1 ||
    pillarLower.indexOf("build") !== -1
  ) {
    question = "Quelle décision technique avez-vous repoussée jusqu'à ce qu'elle coûte cher ?";
  } else if (
    pillarLower.indexOf("conseil") !== -1 ||
    pillarLower.indexOf("consultant") !== -1 ||
    pillarLower.indexOf("diagnostic") !== -1
  ) {
    question = "Quelle recommandation difficile a changé la trajectoire d'un client ?";
  } else if (
    pillarLower.indexOf("strat" + "é" + "g") !== -1 ||
    pillarLower.indexOf("strateg") !== -1 ||
    pillarLower.indexOf("comex") !== -1 ||
    pillarLower.indexOf("m&a") !== -1
  ) {
    ...
    if (pillarTitle.length > 10) {
      question =
        "Quelle expérience vous a convaincu que " +
        pillarTitle.charAt(0).toLowerCase() +
        pillarTitle.slice(1).replace(/\.$/, "") +
        " ?";
    } else {
...
export function generatePositions(bricks, vault) {
  ...
  sorted.slice(0, 2).forEach(function (pillar) {
    ...
    var available = validated.filter(function (b) {
      return usedBrickIds.indexOf(b.id) === -1;
    });
    ...
    if (brick.brickType === "cicatrice") {
      text +=
        ...
        brick.text +
        " L'échec m'a obligé à changer de méthode. Depuis, je mesure autrement.";
    } else if (brick.brickCategory === "decision") {
      text +=
        "J'ai dû trancher dans ce contexte. " +
        brick.text +
        " L'arbitrage n'était pas évident. Mais c'est là que se construit la crédibilité.";
    } else if (brick.brickCategory === "influence") {
      text +=
        "Le plus dur n'était pas la méthode. C'était l'alignement. " +
        brick.text +
        " Quand tout le monde tire dans la même direction, le résultat suit.";
    } else {
    ...
    // Closing
    text +=
      "\n\nLe vrai KPI n'est pas celui qu'on affiche. C'est celui qu'on découvre quand on creuse sous la surface.";
...
export function detectSignalType(text) {
  ...
  for (var i = 0; i < SIGNAL_TYPES.length; i++) {
    var match = SIGNAL_TYPES[i].keywords.some(function (k) {
...
export function generateSignalScript(signalText, signalType, bricks, targetRoleId) {
  ...
  var opener = "";
  if (signalType.type === "levee_fonds")
    opener =
      "Vous venez de lever des fonds. Dans les 6 prochains mois, vous allez structurer une équipe qui n'existe pas encore.";
  else if (signalType.type === "recrutement")
    opener = "J'ai vu que vous recrutez. Ce type de poste signale un problème que je connais bien.";
  else if (signalType.type === "mouvement")
    opener =
      "J'ai noté un changement dans votre équipe de direction. Les 90 premiers jours vont définir la trajectoire.";
  else if (signalType.type === "expansion")
    opener = "Vous ouvrez un nouveau terrain. Les erreurs des 6 premiers mois coûtent cher.";
  else if (signalType.type === "reorganisation")
    opener = "Une réorganisation crée toujours un besoin que l'organigramme ne montre pas encore.";
  else opener = "J'ai identifié un signal dans votre actualité récente.";

  var proof = bestBrick
    ? "J'ai résolu un problème similaire : " +
      (bestBrick.text.length > 80 ? bestBrick.text.slice(0, 80) + "..." : bestBrick.text)
    : "";
  var cost = cauchemar
    ? "Ce type de problème coûte entre " +
      formatCost(cauchemar.costRange[0]) +
      " et " +
      formatCost(cauchemar.costRange[1]) +
      " par an."
    : "";
...
export function computeUserTerritory(bricks, vault, targetRoleId) {
    ...
    roleData.kpis.forEach(function (kpi) {
      if (kpi.elasticity === "élastique") {
        kpi.name
          ...
          .split(/[\s\/\(\)]+/)
          .forEach(function (w) {
            if (w.length > 3 && territoryKw.indexOf(w) === -1) territoryKw.push(w);
          });
      }
    });
  }
  pillars.forEach(function (p) {
    ...
    (p.title || "")
      ...
      .split(/\s+/)
      .forEach(function (w) {
        if (w.length > 4 && territoryKw.indexOf(w) === -1) territoryKw.push(w);
      });
  ...
  if (roleData) {
    roleData.kpis.forEach(function (kpi) {
      kpiNames.push(kpi.name.toLowerCase());
    });
  }
  ...
  return {
    ...
    elasticKpis: roleData
      ? roleData.kpis
          .filter(function (k) {
            return k.elasticity === "élastique";
          })
          .map(function (k) {
            return k.name;
          })
...
export function detectPostGap(text) {
  ...
  if (hasClaim && !hasNumber)
    gaps.push({
      type: "quoi_sans_combien",
      label: "Le quoi sans le combien",
      desc: "L'auteur affirme sans chiffrer. Ton chiffre sera le seul du fil.",
    });

  // Gap 2: method without situation
  var hasMethod = ["methode", "process", "etapes", "framework", "strategie", "approche", "technique"].some(
    function (m) {
      return lower.indexOf(m) !== -1;
    }
  );
  var hasSituation = ["chez", "quand j'ai", "dans mon", "en 20", "face a", "contexte", "equipe de", "pendant"].some(
    function (m) {
      return lower.indexOf(m) !== -1;
    }
  );
  if (hasMethod && !hasSituation)
    gaps.push({
      type: "comment_sans_ou",
      label: "Le comment sans le où",
      desc: "Méthode abstraite. Ton vécu ancre la théorie dans le réel.",
    });
  ...
  if (hasOpinion && !hasExperience)
    gaps.push({
      type: "avis_sans_preuve",
      label: "L'avis sans la preuve",
      desc: "Opinion nue. Ton expérience vécue apporte ce que l'opinion seule ne donne pas.",
    });

  // Gap 4: problem without solution
  var hasProblem = ["probleme", "difficulte", "challenge", "obstacle", "bloque", "galere", "crise", "echec"].some(
    function (m) {
      return lower.indexOf(m) !== -1;
    }
  );
  var hasSolution = ["solution", "resolu", "regle", "mis en place", "lance", "deploye", "construit", "resultat"].some(
    function (m) {
      return lower.indexOf(m) !== -1;
    }
  );
  if (hasProblem && !hasSolution)
    gaps.push({
      type: "probleme_sans_solution",
      label: "Le problème sans la sortie",
      desc: "Constat sans issue. Ta brique montre comment tu as résolu un problème similaire.",
    });

  if (gaps.length === 0)
    gaps.push({
      type: "general",
      label: "Pas de trou flagrant",
      desc: "Le post est complet. Apporte un angle que l'auteur n'a pas couvert.",
    });
...
export function runCommentFilters(postText, bricks, vault, targetRoleId) {
  ...
  var territoryHits = 0;
  territory.keywords.forEach(function (kw) {
    if (postLower.indexOf(kw) !== -1) territoryHits++;
  });
  ...
  validated.forEach(function (b) {
    ...
    var relevance = 0;
    if (topic.topic !== "general") {
      var topicEntry = COMMENT_TOPICS.find(function (t) {
        return t.topic === topic.topic;
      });
      if (topicEntry) {
        topicEntry.keywords.forEach(function (k) {
          if (bLower.indexOf(k) !== -1) relevance += 2;
        });
      }
    }
    var postWords = postLower.split(/\s+/).filter(function (w) {
      return w.length > 4;
    });
    postWords.forEach(function (w) {
      if (bLower.indexOf(w) !== -1) relevance++;
    });
  ...
  var filter2Reason = filter2Pass
    ? "Brique trouvée (" +
      (bestBrick.brickType === "cicatrice" ? "cicatrice" : bestBrick.brickCategory) +
      ") — pertinence " +
      bestRelevance +
      "/10."
...
export function auditComment(commentText, bricks, vault) {
  ...
  validated.forEach(function (b) {
    ...
    kpiWords.forEach(function (kw) {
      if (lower.indexOf(kw) !== -1) hasKpiRef = true;
    });
  });
  ...
  tests.push({
    name: "Preuve",
    pass: preuvePass,
    detail: preuvePass ? "Chiffre ou KPI détecté." : "Aucune preuve. Opinion nue.",
  });
  ...
  pillars.forEach(function (p) {
    ...
    words.forEach(function (w) {
      var clean = w.replace(/[.,;:!?()]/g, "");
      if (clean.length > 4 && lower.indexOf(clean) !== -1) {
        pillarHit = true;
        pillarMatched = p.title;
      }
    });
  });
  tests.push({
    name: "Pilier",
    pass: pillarHit,
    detail: pillarHit ? "Ancré sur : " + pillarMatched : "Hors territoire. Risque de dilution.",
  });
  ...
  var hasTension = /le vrai sujet|ce qui manque|la question que personne|curieux de|ce qui change/.test(
    tail.toLowerCase()
  );
  ...
  tests.push({
    name: "Relance",
    pass: relancePass,
    detail: relancePass ? "Question ou tension ouverte détectée." : "Commentaire fermé. Aucune relance.",
  });
...
export function generateLinkedInComment(postText, bricks, vault, targetRoleId) {
  ...
  if (validated.length === 0)
    return { comment: "Score vide. Valide des briques d'abord.", topic: "general", filterResult: null };
  ...
  if (filterResult.verdict === "PASSE") {
    ...
    var likeAdvice = hasAvoidPattern
      ? {
          action: "NE LIKE PAS",
          reason: "Contenu toxique ou concurrent. Le like valide une thèse que tu combats.",
          color: "#e94560",
        }
      : {
          action: "LIKE POSSIBLE",
          reason:
            "Post hors de ton territoire mais correct. Le like maintient ta présence sans engager ton positionnement.",
          color: "#ff9800",
        };
  ...
  situation = situation
    .replace(/[\+\-]?\d+[\.,]?\d*\s*[%KM€]*/g, "")
    .replace(/\d+[\.,]?\d*\s*(mois|semaines|jours|ans|comptes|commerciaux|personnes|équipes)/g, "$1")
    .replace(/\(\s*\)/g, "")
    .replace(/\s{2,}/g, " ")
    .trim();
  ...
  if (vault && vault.selectedPillars && vault.selectedPillars.length > 0) {
    var takePillar = vault.selectedPillars.find(function (p) {
  ...
  if (gap.type === "quoi_sans_combien") {
    acknowledge = "L'intention est là.";
    ...
    acknowledge = "Point clé.";
  }

  // TEMPS 2 — Injection de preuve (situation sans chiffre)
  var proofText = "";
  if (bestBrick.brickType === "cicatrice") {
    proofText = "J'ai vécu l'inverse. " + situation + " L'échec m'a forcé à changer de méthode.";
  } else if (bestBrick.brickCategory === "decision") {
    proofText = "J'ai dû trancher dans un contexte similaire. " + situation;
  ...
  var closing = "";
  if (gap.type === "quoi_sans_combien") {
    closing = "Sur quel indicateur tu mesures le delta avant/après ?";
  } else if (gap.type === "comment_sans_ou") {
    closing = "Dans quel contexte tu as vu cette méthode tenir sous pression ?";
  } else if (gap.type === "avis_sans_preuve") {
    closing = "Curieux de savoir quelle situation t'a amené à cette conviction.";
  } else if (gap.type === "probleme_sans_solution") {
    closing = "Quelle a été la première action concrète pour en sortir ?";
  } else {
    closing = "Curieux de savoir comment ça se passe chez vous.";
  }
  ...
  if (diltsLevel <= 2) {
    hook =
      brick.brickType === "cicatrice"
        ? "J'ai planté. " + (brick.kpi || "un projet") + "."
        : (number ? number + ". " : "") + "Pas un objectif. Un résultat.";
    body = situation + ".";
    close = calibration.framingClose;
  } else if (diltsLevel === 3) {
    hook = "La méthode que personne n'applique sur " + (brick.kpi || "ce sujet").toLowerCase() + ".";
    body = situation + "." + (number ? " Résultat : " + number + "." : "");
    close = calibration.framingClose;
    ...
    body = "Mon expérience : " + situation + "." + (number ? " " + number + " de delta." : "");
    close = calibration.framingClose;
  } else {
    hook = "On me reconnaît à ça.";
    body = situation + ".";
    if (pillar && pillar.title) body += " J'écris sur " + pillar.title.toLowerCase().replace(/\.$/, "") + ".";
    close = calibration.framingClose;
  }
...
export function marieHookAudit(draft) {
  ...
  var hasTension =
    /\?|!|\.{3}|pas un|personne|le vrai|contrairement|en r[eé]alit[eé]|plant[eé]|[eé]chou[eé]|perdu/.test(hookLower);
  var hasNumber = /\d+[%KM€]/.test(hook);
  var soWhat = hasTension || hasNumber;
  tests.push({
    name: "So What",
    pass: soWhat,
    detail: soWhat ? "Tension ou chiffre présent" : "Accroche plate, aucune réaction",
  });

  // 2. Ennemi — antagoniste identifie
  var hasEnemy =
    /personne ne|le consensus|l'erreur|[aà] tort|le probl[eè]me|on croit que|la majorit[eé]|le mythe|le pi[eè]ge|on me dit/.test(
      lower
    );
  tests.push({ name: "Ennemi", pass: hasEnemy, detail: hasEnemy ? "Antagoniste détecté" : "Pas d'ennemi, trop mou" });

  // 3. Consensus — clivant ou contre-intuitif
  var isClivant =
    /contrairement|le contraire|refuse|en r[eé]alit[eé]|personne ne dit|[aà] tort|mon parti|impopulaire/.test(lower);
  var notConsensus = hasEnemy || isClivant || draft.diltsLevel >= 4;
  tests.push({
    name: "Consensus",
    pass: notConsensus,
    detail: notConsensus ? "Position clivante ou contre-intuitive" : "Tout le monde est d'accord, aucun intérêt",
  });

  // 4. Alienation — repousse les non-cibles
  var hasSpecificity =
    /\d+|m[eé]thode|process|framework|churn|pipeline|r[eé]tention|conversion|onboarding|recrutement/.test(lower);
  tests.push({
    name: "Aliénation",
    pass: hasSpecificity,
    detail: hasSpecificity ? "Vocabulaire spécifique, filtre actif" : "Trop générique, parle à tout le monde",
  });
  ...
  var hasIaSmell = iaWords.some(function (w) {
  ...
  tests.push({
    name: "Authenticité",
    pass: !hasIaSmell,
    detail: hasIaSmell ? "Vocabulaire IA détecté, sonne faux" : "Ton assumé",
  });

  // 6. Memorabilite — phrase retenue 1h apres
  var hasImage =
    /j'ai plant[eé]|j'ai vu|le jour o[uù]|ce moment|[aà] ce moment|en face de|quand j'ai|la premi[eè]re fois/.test(
      lower
    );
  ...
  tests.push({
    name: "Mémorabilité",
    pass: memorable,
    detail: memorable ? "Image ou chiffre mémorisable" : "Oubliable en 30 secondes",
  });

  var score = tests.filter(function (t) {
    return t.pass;
  }).length;
...
export function luisEnriqueAudit(draft, bricks) {
  ...
  tests.push({
    name: "Utilité",
    pass: useful,
    detail: useful ? "Contenu concret et actionnable" : "Bruit — rien d'actionnable",
  });
  ...
  var brick = bricks.find(function (b) {
    return b.id === draft.brickId;
  });
  var hasProof = brick && brick.blinded;
  var isClear = !isEgo && (hasProof || draft.diltsLevel <= 3);
  tests.push({ name: "Clarté", pass: isClear, detail: isEgo ? "Ego — tu te regardes écrire" : "Clair et ancré" });
  ...
  tests.push({
    name: "Aide",
    pass: helpsOthers,
    detail: helpsOthers ? "Le lecteur apprend quelque chose" : "Le lecteur apprend que tu es bon. C'est tout.",
  });
...
export function tagVision2026(draft) {
  ...
  var format =
    VISION_2026_FORMATS.find(function (f) {
      return f.id === draft.format;
    }) || VISION_2026_FORMATS[0];
  draft.formatLabel = format.label;
  draft.formatDesc = format.desc;
  draft.stockPotential = draft.diltsLevel >= 3;
  if (draft.stockPotential) {
    draft.stockAngle = "Cet angle mérite un article long. Développe en article LinkedIn cette semaine.";
  }
  return draft;
}
...
export function generateWeeklyPosts(bricks, vault, targetRoleId) {
  ...
  var cicatrice = validated.find(function (b) {
    return b.brickType === "cicatrice" && usedBrickIds.indexOf(b.id) === -1;
  });
...
export function generateSleepComment(bricks, vault, targetRoleId) {
  ...
  return {
    ...
    suggestion:
      "Trouve un post dans ton secteur sur " +
      (brick.kpi || "ton sujet").toLowerCase() +
      '. Commente avec : "' +
      situation +
      '"',
...
export function generateDormantRelaunch(bricks, vault, targetRoleId, monthsInactive) {
  ...
  return {
    ...
    suggestion:
      "Envoie un message à un ancien collègue ou contact dormant. Angle : \"J'ai vu que [actualité de son entreprise]. Ça rejoint un cas que j'ai géré en " +
      roleLabel +
      '."',
    brickSource: brick.text.slice(0, 50),
    effort: "3 minutes",
...
export function proposeSleepBrick(vault) {
  ...
  return {
    ...
    suggestion:
      "En " +
      weeks +
      " semaines, tu as probablement vécu au moins 1 situation notable. Un problème résolu. Une décision prise. Un résultat mesuré. Ajoute-la à ton Score. L'IA la transforme en brique.",
    effort: "5 minutes",
  };
}
