...
export function expertWritingAudit(text) {
  ...
  luisEnrique.push({ label: "Lecteur vs ego", passed: readerFocused, detail: readerFocused ? "Équilibre je/vous correct." : "Trop de 'je' (" + jeCount + "). Réoriente vers le lecteur." });
...
export function generatePositions(bricks, vault) {
  ...
  sorted.slice(0, 2).forEach(function(pillar) {
    ...
    if (brick.brickType === "cicatrice") {
      ...
      text += "J'ai dû trancher dans ce contexte. " + brick.text + " L'arbitrage n'était pas évident. Mais c'est là que se construit la crédibilité.";
    } else if (brick.brickCategory === "influence") {
...
export function generateSignalScript(signalText, signalType, bricks, targetRoleId) {
  ...
  else if (signalType.type === "mouvement") opener = "J'ai noté un changement dans votre équipe de direction. Les 90 premiers jours vont définir la trajectoire.";
  else if (signalType.type === "expansion") opener = "Vous ouvrez un nouveau terrain. Les erreurs des 6 premiers mois coûtent cher.";
  else if (signalType.type === "reorganisation") opener = "Une réorganisation crée toujours un besoin que l'organigramme ne montre pas encore.";
  ...
  var cost = cauchemar ? "Ce type de problème coûte entre " + formatCost(cauchemar.costRange[0]) + " et " + formatCost(cauchemar.costRange[1]) + " par an." : "";
...
export function generateLinkedInComment(postText, bricks, vault, targetRoleId) {
  var validated = bricks.filter(function(b) { return b.status === "validated" && b.brickType !== "take"; });
  if (validated.length === 0) return { comment: "Score vide. Valide des briques d'abord.", topic: "general", filterResult: null };
...
export function marieHookAudit(draft) {
  ...
  tests.push({ name: "So What", pass: soWhat, detail: soWhat ? "Tension ou chiffre présent" : "Accroche plate, aucune réaction" });
  ...
  tests.push({ name: "Consensus", pass: notConsensus, detail: notConsensus ? "Position clivante ou contre-intuitive" : "Tout le monde est d'accord, aucun intérêt" });
  ...
  tests.push({ name: "Aliénation", pass: hasSpecificity, detail: hasSpecificity ? "Vocabulaire spécifique, filtre actif" : "Trop générique, parle à tout le monde" });
  ...
  tests.push({ name: "Authenticité", pass: !hasIaSmell, detail: hasIaSmell ? "Vocabulaire IA détecté, sonne faux" : "Ton assumé" });
  ...
  tests.push({ name: "Mémorabilité", pass: memorable, detail: memorable ? "Image ou chiffre mémorisable" : "Oubliable en 30 secondes" });
...
export function luisEnriqueAudit(draft, bricks) {
  ...
  tests.push({ name: "Utilité", pass: useful, detail: useful ? "Contenu concret et actionnable" : "Bruit — rien d'actionnable" });
...
export function generateDormantRelaunch(bricks, vault, targetRoleId, monthsInactive) {
  ...
  return {
    ...
    suggestion: "Envoie un message à un ancien collègue ou contact dormant. Angle : \"J'ai vu que [actualité de son entreprise]. Ça rejoint un cas que j'ai géré en " + roleLabel + ".\"",
    brickSource: brick.text.slice(0, 50),
    effort: "3 minutes",
  };
}
...
export function proposeSleepBrick(vault) {
  ...
  return {
    type: "passive_brick",
    suggestion: "En " + weeks + " semaines, tu as probablement vécu au moins 1 situation notable. Un problème résolu. Une décision prise. Un résultat mesuré. Ajoute-la à ton Score. L'IA la transforme en brique.",
    effort: "5 minutes",
  };
}
