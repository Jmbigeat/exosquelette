...
function matchWord(text, word) { return new RegExp("\\b" + word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "\\b", "i").test(text); }
export function matchKpiToReference(kpiText, roleId) {
  ...
  roleKpis.forEach(function(ref) {
    ...
    var score = 0;
    words.forEach(function(w) { if (matchWord(lower, w)) score++; });
    var aliases = {
  ...
  if (bestScore >= 1 && bestScore < 2) {
    console.log("[matchKpiToReference] weak match (score=" + bestScore + "): \"" + kpiText + "\" → \"" + (bestMatch ? bestMatch.name : "null") + "\" for role " + roleId);
  }
...
export function generateAdaptiveSeeds(roleId) {
  ...
  seeds.push({
    ...
    blameDetection: true, sideProject: false,
  });
  ...
  seeds.push({
    ...
    externalizeDetection: true, sideProject: false,
  });
  ...
  seeds.push({
    ...
    surfacePatterns: t1.surfacePatterns, sideProject: false,
  });
  ...
  seeds.push({
    ...
    surfacePatterns: t2.surfacePatterns, sideProject: false,
  });
