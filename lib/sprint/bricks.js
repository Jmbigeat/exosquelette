...
export function matchKpiToReference(kpiText, roleId) {
  ...
  roleKpis.forEach(function (ref) {
    ...
    var words = refLower.split(/[\s\/\(\)]+/).filter(function (w) {
      return w.length > 3;
    });
    var score = 0;
    words.forEach(function (w) {
      if (matchWord(lower, w)) score++;
    });
    var aliases = {
    ...
    Object.keys(aliases).forEach(function (alias) {
      if (matchWord(lower, alias) && refLower.indexOf(aliases[alias].toLowerCase()) !== -1) score += 2;
    });
    if (score > bestScore) {
      bestScore = score;
      bestMatch = ref;
...
export function computeCrossRoleMatching(bricks, currentRoleId, trajectoryToggle) {
  var validated = bricks.filter(function (b) {
    return b.status === "validated" && b.kpi;
  });
  ...
  roleIds.forEach(function (roleId) {
    ...
    validated.forEach(function (b) {
      ...
      if (match) {
        totalMatches++;
        if (match.elasticity === "élastique") {
          elasticMatches++;
          matchedKpis.push({
            kpi: match.name,
            elasticity: match.elasticity,
            brick: b.text.length > 50 ? b.text.slice(0, 50) + "..." : b.text,
          });
        } else if (match.elasticity === "stable") {
          stableMatches++;
        }
      }
    });
...
export function generateAdaptiveSeeds(roleId) {
  ...
  var elasticKpis = kpis.filter(function (k) {
    return k.elasticity === "élastique";
  });
  var stableKpis = kpis.filter(function (k) {
    return k.elasticity === "stable";
  });

  // Map KPIs to cauchemars for nightmare text
  function findCauchemar(kpi) {
    return (
      getActiveCauchemars().find(function (c) {
        return c.kpis.some(function (ck) {
          return (
            kpi.name.toLowerCase().indexOf(ck.toLowerCase().slice(0, 6)) !== -1 ||
            ck.toLowerCase().indexOf(kpi.name.toLowerCase().slice(0, 6)) !== -1
          );
        });
      }) || null
    );
  ...
  var orderedKpis = elasticKpis.concat(stableKpis).concat(
    kpis.filter(function (k) {
      return k.elasticity === "sous_pression";
    })
  );
  ...
  orderedKpis.slice(0, 5).forEach(function (kpi, i) {
    ...
    seeds.push({
      id: id++,
      type: "preuve",
      brickCategory: tpl.brickCategory,
      ...
      controlRisk: kpi.elasticity === "sous_pression" ? "sous_pression" : null,
      sectoralNote: null,
      verbNote: null,
      omegaNote: null,
  ...
  seeds.push({
    ...
    question: dTpl.question(),
    context: dTpl.context(),
    hint: dTpl.hint(),
    generatedText: null,
    advocacyText: null,
    internalAdvocacy: null,
  ...
  seeds.push({
    ...
    question: iTpl.question(),
    context: iTpl.context(),
    hint: iTpl.hint(),
    generatedText: null,
    advocacyText: null,
    internalAdvocacy: null,
    ...
    omegaNote: null,
    missionText: null,
    nightmareText: iTpl.nightmareGen(),
    anonymizedText: null,
    kpi: "Influence transverse",
    elasticity: "élastique",
  ...
  seeds.push({
    ...
    elasticityNote: null,
    skills: [],
    usedIn: ["Simulateur", "Duel", "Posts LinkedIn"],
    blameDetection: true,
    sideProject: false,
  });
  ...
  seeds.push({
    ...
    verbNote: null,
    omegaNote: null,
    missionText: c2.missionText(),
    nightmareText: null,
    anonymizedText: null,
    kpi: "Capacité d'adaptation",
    elasticity: null,
    elasticityNote: null,
    skills: [],
    usedIn: ["Simulateur", "Duel", "CV"],
  ...
  seeds.push({
    ...
    elasticityNote: null,
    skills: [],
    usedIn: ["Piliers de Singularité", "Posts LinkedIn", "Commentaires LinkedIn"],
  ...
  seeds.push({
    ...
    elasticityNote: null,
    skills: [],
    usedIn: ["Piliers de Singularité", "Posts LinkedIn", "Script de contact"],
  ...
  seeds.push({
    ...
    omegaNote: null,
    missionText: null,
    nightmareText: ua.nightmareGen ? ua.nightmareGen() : null,
    anonymizedText: null,
    kpi: "Avantage injuste",
    elasticity: null,
    elasticityNote: null,
    skills: [],
    usedIn: ["Rapport d'impact", "CV header", "Bio LinkedIn", "Script de contact"],
    sideProject: false,
  });
...
export function generateBrickVersions(brick, targetRoleId) {
  ...
  var startsWithAction = actionStarters.some(function (a) {
    return cvVersion.toLowerCase().indexOf(a) < 15 && cvVersion.toLowerCase().indexOf(a) !== -1;
  });
  ...
  if (category === "cicatrice") {
    rhVersion =
      "J'ai traversé une situation difficile. " +
      interviewBase +
      " Cette expérience m'a appris à prendre du recul et à ajuster ma méthode. C'est ce type de moment qui structure un parcours.";
  } else if (category === "decision") {
    rhVersion =
      "J'ai été confronté à un choix stratégique. " +
      interviewBase +
      " Ce que j'en retiens, c'est ma capacité à trancher sous pression et à assumer les conséquences.";
  } else if (category === "influence") {
    rhVersion =
      "J'ai dû aligner des personnes aux intérêts divergents. " +
      interviewBase +
      " Ce qui m'a marqué, c'est l'importance de la lecture politique dans l'exécution.";
  } else {
    rhVersion =
      "Dans mon parcours, un moment clé a été quand " +
      interviewBase.charAt(0).toLowerCase() +
      interviewBase.slice(1) +
      " Ce résultat illustre ma façon de travailler : je mesure et j'ajuste jusqu'au résultat.";
  }
  ...
  if (category === "cicatrice") {
    n1Version =
      "Le problème était concret. " +
      interviewBase +
      " La correction que j'ai appliquée ensuite a fonctionné parce que j'avais compris la cause racine, pas juste le symptôme.";
  } else if (category === "decision") {
    n1Version =
      "Deux options s'opposaient. " +
      interviewBase +
      " J'ai chiffré les risques et tranché. La méthode compte autant que le résultat.";
  } else if (category === "influence") {
    n1Version =
      "Le blocage venait des personnes, pas du process. " +
      interviewBase +
      " J'ai résolu ça en travaillant les objections une par une, en commençant par le plus résistant.";
  } else {
    n1Version =
      "Sur le terrain, voici ce qui s'est passé. " +
      interviewBase +
      " La méthode est reproductible. Je l'ai testée sur ce contexte, elle s'adapte à d'autres.";
  }
  ...
  var cauchemar = getActiveCauchemars().find(function (c) {
    return (
      ...
      c.kpis.some(function (k) {
        return kpi.toLowerCase().indexOf(k.toLowerCase().slice(0, 6)) !== -1;
      })
    );
  });
  var costFrame = cauchemar
    ? " Ce type de problème coûte entre " +
      formatCost(cauchemar.costRange[0]) +
      " et " +
      formatCost(cauchemar.costRange[1]) +
      " par an quand il n'est pas résolu."
    : "";
  if (category === "cicatrice") {
    dirVersion =
      "L'enjeu business était réel. " +
      interviewBase +
      costFrame +
      " L'échec m'a coûté du temps mais m'a donné un cadre d'analyse que j'applique systématiquement.";
  } else if (category === "decision") {
    dirVersion =
      "En termes d'impact P&L, voici l'arbitrage. " +
      interviewBase +
      costFrame +
      " La décision a protégé la marge et accéléré l'exécution.";
  } else if (category === "influence") {
    dirVersion =
      "Le sujet était politique avant d'être opérationnel. " +
      interviewBase +
      costFrame +
      " L'alignement a débloqué l'exécution sur tout le périmètre.";
  } else {
    dirVersion =
      "L'impact business est mesurable. " +
      interviewBase +
      costFrame +
      " Ce delta se traduit directement en valeur pour l'organisation.";
  }
  ...
  if (cauchemar) {
    discoveryQuestions.push(
      cauchemar.nightmareShort.replace(/\.$/, "") + " — c'est une situation que vous rencontrez aujourd'hui ?"
    );
  }
  ...
  return {
    ...
    discoveryQuestions: discoveryQuestions.map(function (q) {
      return cleanRedac(q, "livrable");
    }),
  };
}
