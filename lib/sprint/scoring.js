...
function axeSingularite(validated, pillars, signature) {
  ...
  var total = Math.round(cicatricePct * 0.30 + elasticPct * 0.25 + pillarPct * 0.25 + signaturePct * 0.20);
  return { pct: total, cicatrices: cicatrices.length, elasticArmored: elasticArmored.length, pillars: selectedPillars.length, signatureFormulated: !!(signature && signature.formulation), signatureArmored: !!(signature && signature.armored) };
}
...
function generateDensityWarnings(axes, validated, duelResults) {
  ...
  if (axes[1].pct > 0 && axes[1].pct < 100 && validated.length > 0) warnings.push(axes[1].pct + "% de tes briques sont blindées. Les briques non blindées ne survivent pas au stress test d'un recruteur.");
  if (axes[2].detail.coveredCount < 5 && axes[2].detail.totalCount > 0) warnings.push(axes[2].detail.coveredCount + "/5 cauchemars couverts. Le recruteur a 5 peurs. Tu en neutralises " + axes[2].detail.coveredCount + ".");
  if (!duelResults || duelResults.length === 0) warnings.push("Tu n'as pas passé le Duel. Plafond : 90%.");
  if (axes[5] && axes[5].detail && axes[5].detail.selectedCount > 0) warnings.push("Ton CV utilise " + axes[5].detail.selectedCount + " briques. La sélection recommandée : 5 briques blindées de 3 types différents.");
  return warnings;
}
...
export function detectBluffRisk(bricks) {
  ...
  coverage.forEach(function(cc) {
    ...
    if (allSousPression) {
      ...
      var hasDecision = coveringBricks.some(function(b) { return hasDecisionMarkers(b.text || ""); });
      if (hasDecision) {
        risks.push({ cauchemar: cauch.label, severity: "alerte",
          reason: "Ce KPI est automatisable, mais ta brique montre un arbitrage humain. Le recruteur verra la décision, pas juste le chiffre. Renforce l'angle décision."
        });
      } else {
