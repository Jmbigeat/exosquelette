...
import { hasDecisionMarkers } from "./analysis.js";

/* Global active cauchemars — set by Sprint component, used by all utility functions */
var _activeCauchemars = null;
export function getActiveCauchemars() {
  return _activeCauchemars || CAUCHEMARS_CIBLES;
}
export function setActiveCauchemarsGlobal(c) {
  _activeCauchemars = c;
}
...
export function assessBrickArmor(brick) {
  if (!brick || !brick.text)
    return { status: "vulnerable", depth: 0, missing: ["chiffres", "décision", "influence", "transférabilité"] };
  ...
  if (brick.stressTest) {
    ["angle1", "angle2", "angle3", "angle4", "angle5"].forEach(function (a) {
      ...
      if (st && st.response) {
        Object.keys(st.response).forEach(function (k) {
          if (st.response[k]) stResponses += " " + st.response[k];
        });
      }
    });
  }
  ...
  var hasTransferability =
    brick.stressTestAngle3Validated ||
    [
      "process",
      "méthode",
      "methode",
  ...
  var depth =
    (hasNumbers ? 1 : 0) + (hasDecisionMarkers ? 1 : 0) + (hasInfluenceMarkers ? 1 : 0) + (hasTransferability ? 1 : 0);
  var missing = [];
  ...
  return {
    ...
    hasDecisionMarkers: hasDecisionMarkers,
    hasInfluenceMarkers: hasInfluenceMarkers,
    hasTransferability: hasTransferability,
  };
}
...
function axeCouvertureCauchemars(validated, nightmares) {
  ...
  var coveredKpis = {};
  validated.forEach(function (b) {
    if (b.kpi) coveredKpis[b.kpi] = true;
  });
  var coveredCount = 0;
...
// Axe 4 — Singularité (15%) : cicatrices(30%) + elastic armored(25%) + pillars(25%) + signature(20%)
// Chantier 7: signature replaces the old unfair_advantage placeholder
function axeSingularite(validated, pillars, signature) {
  ...
  // Signature: 0% = no signature, 70% = formulated, 100% = formulated + armored
  var signaturePct = 0;
  if (signature && signature.formulation) {
    signaturePct = signature.armored ? 100 : 70;
  }

  var total = Math.round(cicatricePct * 0.3 + elasticPct * 0.25 + pillarPct * 0.25 + signaturePct * 0.2);
  return {
    pct: total,
    cicatrices: cicatrices.length,
    elasticArmored: elasticArmored.length,
    pillars: selectedPillars.length,
    signatureFormulated: !!(signature && signature.formulation),
    signatureArmored: !!(signature && signature.armored),
  };
}
...
function generateDensityWarnings(axes, validated, duelResults) {
  var warnings = [];
  if (axes[0].pct < 50) warnings.push("Tu as " + validated.length + " briques. Les candidatures solides en ont 8+.");
  if (axes[1].pct > 0 && axes[1].pct < 100 && validated.length > 0)
    warnings.push(
      axes[1].pct +
        "% de tes briques sont blindées. Les briques non blindées ne survivent pas au stress test d'un recruteur."
    );
  if (axes[2].detail.coveredCount < 5 && axes[2].detail.totalCount > 0)
    warnings.push(
      axes[2].detail.coveredCount +
        "/5 cauchemars couverts. Le recruteur a 5 peurs. Tu en neutralises " +
        axes[2].detail.coveredCount +
        "."
    );
  if (!duelResults || duelResults.length === 0) warnings.push("Tu n'as pas passé le Duel. Plafond : 90%.");
  if (axes[5] && axes[5].detail && axes[5].detail.selectedCount > 0)
    warnings.push(
      "Ton CV utilise " +
        axes[5].detail.selectedCount +
        " briques. La sélection recommandée : 5 briques blindées de 3 types différents."
    );
  return warnings;
}
...
export function computeDensityScore(bricksOrParams, cauchemarsLegacy) {
  var bricks, nightmares, pillars, signature, duelResults, cvBricks;
  ...
  if (bricksOrParams && !Array.isArray(bricksOrParams) && typeof bricksOrParams === "object" && bricksOrParams.bricks) {
    ...
    signature = bricksOrParams.signature || null;
    ...
    return {
      ...
      axes: [
        { name: "Matériau brut", weight: 15, pct: 0, weighted: 0, detail: {} },
        {
          ...
          pct: 0,
          weighted: 0,
          detail: { armoredCount: 0, credibleCount: 0, vulnerableCount: 0, armors: [] },
        },
        {
          name: "Couverture cauchemars",
          weight: 20,
          pct: 0,
          weighted: 0,
          detail: { coveredCount: 0, totalCount: 0, diversityBonus: false },
        },
  ...
  var a4 = axeSingularite(validated, pillars, signature);
  ...
  var axes = [
    {
      ...
      weight: 15,
      pct: a1,
      weighted: Math.round(a1 * 0.15),
      detail: { count: validated.length },
    },
    { name: "Blindage", weight: 25, pct: a2.pct, weighted: Math.round(a2.pct * 0.25), detail: a2 },
    {
      name: "Couverture cauchemars",
      weight: 20,
      pct: Math.min(100, a3.pct),
      weighted: Math.round(Math.min(120, a3.pct) * 0.2),
      detail: a3,
    },
    { name: "Singularité", weight: 15, pct: a4.pct, weighted: Math.round(a4.pct * 0.15), detail: a4 },
    { name: "Duel", weight: 10, pct: a5.pct, weighted: Math.round(a5.pct * 0.1), detail: a5 },
    { name: "CV prêt", weight: 15, pct: a6.pct, weighted: Math.round(a6.pct * 0.15), detail: a6 },
  ];

  var total = axes.reduce(function (sum, ax) {
    return sum + ax.weighted;
  }, 0);
  ...
  // V1 compat fields
  var hasCicatrice = validated.some(function (b) {
    return b.brickType === "cicatrice";
  });
  var hasDecision = validated.some(function (b) {
    return b.brickCategory === "decision";
  });
  var hasInfluence = validated.some(function (b) {
    return b.brickCategory === "influence";
  });
  var correctedCount = validated.filter(function (b) {
    return b.corrected;
  }).length;
...
export function computeCauchemarCoverage(bricks) {
  ...
  return getActiveCauchemars().map(function (c) {
    var covered = c.kpis.some(function (k) {
      return coveredKpis[normKpi(k)];
    });
    var coveringBricks = [];
    c.kpis.forEach(function (k) {
      var nk = normKpi(k);
      if (kpiBricks[nk]) coveringBricks = coveringBricks.concat(kpiBricks[nk]);
    });
    var hasElasticCovering = coveringBricks.some(function (b) {
      return b.kpiRefMatch && b.kpiRefMatch.elasticity === "élastique";
    });
    return {
      id: c.id,
      label: c.label,
      nightmareShort: c.nightmareShort,
      covered: covered,
      costRange: c.costRange,
      costUnit: c.costUnit,
      costContext: c.costContext,
      coveringBricks: coveringBricks,
      hasElasticCovering: hasElasticCovering,
    };
  });
}
...
export function computeNegotiationBrief(bricks, cauchemars) {
  ...
  var coveredCauchemars = coverage.filter(function (c) {
    return c.covered;
  });
  ...
  coveredCauchemars.forEach(function (cc) {
    ...
    var hasCicatrice = coveringBricks.some(function (b) {
      return b.brickType === "cicatrice";
    });
    var hasDecision = coveringBricks.some(function (b) {
      return b.brickCategory === "decision";
    });
    var hasChiffre = coveringBricks.some(function (b) {
    ...
    lines.push({
      ...
      costLow: cauch.costRange[0],
      costHigh: cauch.costRange[1],
  ...
  return {
    ...
    lines: lines,
    coveredCount: coveredCauchemars.length,
    totalCount: getActiveCauchemars().length,
    marketContext: {
      ...
      ancrage:
        "Le candidat moyen arrive sans chiffre. " +
        MARKET_DATA.nego.cherchent_a_negocier +
        "% tentent de négocier. Toi tu arrives avec le coût du problème. Tu es dans les " +
        (100 - MARKET_DATA.nego.cherchent_a_negocier) +
        "% qui cadrent la discussion.",
    },
  };
}
...
export function detectBluffRisk(bricks) {
  ...
  coverage.forEach(function (cc) {
    ...
    var allSousPression =
      coveringBricks.length > 0 &&
      coveringBricks.every(function (b) {
        return b.kpiRefMatch && b.kpiRefMatch.elasticity === "sous_pression";
      });
    var singleBrick = coveringBricks.length === 1;
    var noChiffre = !coveringBricks.some(function (b) {
      return b.brickCategory === "chiffre";
    });
    if (allSousPression) {
      // Fix 14: nuance if brick has decision markers — value is in the decision, not the KPI
      var hasDecision = coveringBricks.some(function (b) {
        return hasDecisionMarkers(b.text || "");
      });
      if (hasDecision) {
        risks.push({
          cauchemar: cauch.label,
          severity: "alerte",
          reason:
            "Ce KPI est automatisable, mais ta brique montre un arbitrage humain. Le recruteur verra la décision, pas juste le chiffre. Renforce l'angle décision.",
        });
      } else {
        risks.push({
          cauchemar: cauch.label,
          severity: "critique",
          reason:
            "Ta preuve repose sur un KPI automatisable. Le recruteur sait que l'IA fait ce travail. Tu te positionnes comme le remède avec un outil que tout le monde a.",
        });
      }
      ...
      risks.push({
        ...
        severity: "alerte",
        reason:
          "Un seul indice. Pas de chiffre. Si le problème persiste après ton arrivée, tu es le fusible. Renforce cette preuve ou change de terrain.",
      });
    }
  });
  return risks;
...
export function computeEffort(bricks) {
  ...
  bricks.forEach(function (b) {
    if (b.status === "validated") {
      if (b.brickType === "cicatrice") {
        total += EFFORT_WEIGHTS.brick_cicatrice;
        breakdown.cicatrices++;
      } else if (b.brickCategory === "decision" || b.brickCategory === "influence") {
        total += EFFORT_WEIGHTS.brick_decision;
        breakdown.briques++;
      } else {
        total += EFFORT_WEIGHTS.brick_chiffre;
        breakdown.briques++;
      }
      if (b.corrected) {
        total += EFFORT_WEIGHTS.correction;
        breakdown.corrections++;
      }
    }
    if (b.type === "mission") {
      total += EFFORT_WEIGHTS.mission_assigned;
      breakdown.missions++;
    }
  });
...
export function computeCauchemarCoverageDetailed(bricks, nightmareCosts) {
  ...
  return getActiveCauchemars().map(function (c) {
    ...
    c.kpis.forEach(function (k) {
      var nk = normKpi(k);
      if (bricksByKpi[nk]) coveringBricks = coveringBricks.concat(bricksByKpi[nk]);
    });
    // Vulnerability of covering bricks
    var vulnerabilities = coveringBricks.map(function (b) {
      return auditBrickVulnerability(b);
    });
...
export function auditBrickVulnerability(brick) {
  ...
  if (depth >= 4)
    return { level: "blindee", color: "#4ecca3", msg: "Brique blindée. Chiffre, méthode, contexte et résultat." };
  if (depth >= 2)
    return {
      level: "credible",
      color: "#3498db",
      msg:
        "Brique crédible mais pas blindée." +
        (!hasNumber ? " Il manque un chiffre." : "") +
        (!hasMethod ? " Il manque une méthode." : "") +
        (!hasContext ? " Il manque un contexte." : "") +
        " Si tu te positionnes comme le remède, blinde cette brique.",
    };
  return {
    level: "vulnerable",
    color: "#e94560",
    msg: "Brique vulnérable. Si le problème persiste après ton embauche, tu deviens la cible. Ajoute un chiffre, une méthode et un contexte.",
  };
}

/**
 * Pour chaque brique validée, estime combien de livrables elle alimenterait.
 * Simule la logique de sélection des 11 generators sans les appeler.
 *
 * Catégories :
 * - B (7 generators) : subset par armorScore/ranking → éligible si armorScore ≥ 3
 * - C (4 generators) : match spécifique → éligible selon propriétés
 *
 * @param {Array} bricks - toutes les briques
 * @param {string} targetRoleId - rôle cible
 * @param {Array} cauchemars - cauchemars actifs
 * @param {object|null} signature - objet signature
...
export function computeBrickDeliverableCount(bricks, targetRoleId, cauchemars, signature) {
  ...
  if (validated.length === 0) return counts;

  // Sort by armorScore descending (for top-N checks)
  ...
  var top3Ids = {};
  sorted.slice(0, 3).forEach(function (b) {
    top3Ids[b.id] = true;
  });

  // Cauchemar KPI matching helper
  var activeCauchs = cauchemars || getActiveCauchemars();
  function matchesCauchemar(brick) {
    if (!brick.kpi) return false;
    var bKpi = brick.kpi.toLowerCase();
    return activeCauchs.some(function (c) {
      return (
        c.kpis &&
        c.kpis.some(function (k) {
          return bKpi.indexOf(k.toLowerCase().slice(0, 6)) !== -1;
        })
      );
    });
  }

  // Best brick (highest armorScore with cauchemar match)
  var bestBrickId = null;
  sorted.forEach(function (b) {
    if (!bestBrickId && matchesCauchemar(b)) bestBrickId = b.id;
  });
  if (!bestBrickId && sorted.length > 0) bestBrickId = sorted[0].id;

  validated.forEach(function (b) {
    var count = 0;
    var armor = b.armorScore || 0;
    var isArmored = armor >= 3;
    var isTop5 = !!top5Ids[b.id];
    var isTop3 = !!top3Ids[b.id];
    var isBest = b.id === bestBrickId;
    var hasCauchMatch = matchesCauchemar(b);

    // 1. One-Pager: top 5 by armorScore (prefer ≥3)
    if (isTop5) count++;

    // 2. CV calibré: greedy coverage top 5 — approximate by armor + cauchemar match
    if (isTop5 && (isArmored || hasCauchMatch)) count++;

    // 3. Bio: best single brick
    if (isBest) count++;

    // 4. Script contact: 1 strongest brick covering costliest cauchemar
    if (isBest && hasCauchMatch) count++;

    // 5. Followup: armored bricks matching challenges
    if (isArmored && hasCauchMatch) count++;

    // 6. Plan 30j RH: 1 strongest brick (blinded preferred)
    if (isBest) count++;

    // 7. Discovery call: armored pool for cauchemar-brick pairing
    if (isArmored) count++;

    // 8. Interview questions: armored subset + cauchemar levels
    if (isArmored) count++;

    // 9. Fiche de combat: top 3 + category-specific pools
    if (isTop3) count++;
    else if (
      b.brickCategory === "decision" ||
      b.brickCategory === "influence" ||
      b.brickType === "elastic" ||
      b.elasticity === "élastique"
    )
      count++;

    // 10. Négociation: blinded/top 3 + coverage costing
    if (isTop3 || (b.blinded && isArmored)) count++;

    // 11. Plan 90j N+1: category-specific (elastic, decision, influence)
    if (
      b.brickType === "elastic" ||
      b.elasticity === "élastique" ||
      b.brickCategory === "decision" ||
...
/**
 * computeOpenLoops — Zeigarnik (16h)
 * Détecte les boucles ouvertes du candidat pour créer l'effet de tension.
 * @param {{ bricks: Array, cauchemars: Array, signature: object|null, duelResults: *, densityScore: number }} params
...
export function computeOpenLoops(params) {
  ...
  var cauchemars = params.cauchemars || [];
  var signature = params.signature || null;
  var duelResults = params.duelResults;
  var densityScore = params.densityScore || 0;
  ...
  var unarmored = validated.filter(function (b) {
    return (b.armorScore || 0) < 4;
  });
  if (unarmored.length > 0) {
    loops.push({
      id: "unarmored_bricks",
      label: unarmored.length + " brique" + (unarmored.length > 1 ? "s" : "") + " non blindée" + (unarmored.length > 1 ? "s" : ""),
      action: "Passe-les au stress test pour verrouiller tes preuves.",
      priority: 1,
    });
  }
  ...
  var coverage = computeCauchemarCoverage(bricks);
  var uncoveredCount = coverage.filter(function (c) {
    return !c.covered;
  }).length;
  if (uncoveredCount > 0) {
    loops.push({
      id: "uncovered_cauchemars",
      label: uncoveredCount + " cauchemar" + (uncoveredCount > 1 ? "s" : "") + " sans preuve",
      action: "Forge une brique sur chaque cauchemar non couvert.",
      priority: 2,
    });
  }

  // 3. Signature absente — seulement si 2+ briques armorScore >= 3
  var wellArmored = validated.filter(function (b) {
    return (b.armorScore || 0) >= 3;
  });
  if (!signature && wellArmored.length >= 2) {
    loops.push({
      id: "no_signature",
      label: "Signature comportementale absente",
      action: "Déverrouille ta signature pour calibrer tes livrables.",
      priority: 3,
    });
  }

  // 4. Duel non réalisé — seulement si densityScore >= 40
  var hasDuel = duelResults && (Array.isArray(duelResults) ? duelResults.length > 0 : typeof duelResults === "object" && Object.keys(duelResults).length > 0);
  if (!hasDuel && densityScore >= 40) {
    loops.push({
      id: "duel_not_done",
      label: "Duel non réalisé",
      action: "Lance le duel pour tester ta posture face au recruteur.",
      priority: 4,
    });
  }
