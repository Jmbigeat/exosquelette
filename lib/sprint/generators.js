...
function bioBuildAnchorText(brick) {
  var text = (brick.text || "").trim();
  text = text.replace(/^(j'ai|j'|je|nous avons|nous)\s+/i, "");
  text = text.replace(/^(mon parcours|mes compétences|ma carrière|mon expérience|mon|mes|ma)\s+/i, "");
  if (text.length > 0) text = text.charAt(0).toUpperCase() + text.slice(1);
  if (text.length > 210) {
    var cut = text.lastIndexOf(".", 210);
    if (cut > 50) { text = text.slice(0, cut + 1); }
    else { cut = text.lastIndexOf(" ", 207); text = cut > 50 ? text.slice(0, cut) + "." : text.slice(0, 207) + "..."; }
  }
...
export function generateBio(bricks, vault, trajectoryToggle) {
  ...
  return cleanRedac(bioStripVocabInterdit(blocks.join("\n\n")), "livrable");
}
 ...
 */
export function generateCVLine(brick, targetRoleId) {
  var text = (brick.text || "").trim();
  if (!text) return "";
  var bestNum = extractBestNum(text);

  // Strip first-person openers
  var clean = text.replace(/^(j'ai|j'|je|nous avons|nous)\s+/i, "");
  clean = clean.replace(/^(mon|mes|ma)\s+/i, "");
  if (clean.length > 0) clean = clean.charAt(0).toUpperCase() + clean.slice(1);

  // For scars: reframe with lesson verb
  if (brick.brickType === "scar") {
    var scarVerb = CV_SCAR_VERBS[Math.abs(hashCode(text)) % CV_SCAR_VERBS.length];
    // Try to extract the core lesson
    var afterIdx = text.toLowerCase().indexOf("après");
    var apprisIdx = text.toLowerCase().indexOf("appris");
    var corrigeIdx = text.toLowerCase().indexOf("corrigé");
    if (afterIdx !== -1) {
      clean = scarVerb + " " + text.slice(afterIdx).replace(/^après\s+/i, "").replace(/^(avoir|une|un)\s+/i, "");
    } else if (apprisIdx !== -1 || corrigeIdx !== -1) {
      clean = scarVerb + " le process — " + (bestNum ? bestNum + " de résultat" : "leçon intégrée");
    } else {
      clean = scarVerb + " " + clean.charAt(0).toLowerCase() + clean.slice(1);
    }
    if (clean.length > 0) clean = clean.charAt(0).toUpperCase() + clean.slice(1);
  }

  // For elastics: reframe with transferability verb
  if (brick.brickType === "elastic") {
    var elasticVerb = CV_ELASTIC_VERBS[Math.abs(hashCode(text)) % CV_ELASTIC_VERBS.length];
    var hasTransfer = /transpos|adapt|appliqu|étendu|répliq/i.test(clean);
    if (!hasTransfer) {
      clean = elasticVerb + " " + clean.charAt(0).toLowerCase() + clean.slice(1);
    }
  }

  // Ensure starts with action verb for proof/manual
  if ((brick.brickType === "proof" || brick.brickType === "manual" || !brick.brickType) && !/^[A-ZÀ-Ú]/.test(clean)) {
    var verb = CV_ACTION_VERBS[Math.abs(hashCode(text)) % CV_ACTION_VERBS.length];
    clean = verb + " " + clean;
  }

  // Truncate to 150 chars
  if (clean.length > 150) {
    var cut = clean.lastIndexOf(".", 150);
    if (cut > 40) { clean = clean.slice(0, cut + 1); }
    else { cut = clean.lastIndexOf(" ", 147); clean = cut > 40 ? clean.slice(0, cut) + "." : clean.slice(0, 147) + "..."; }
  }
 ...
 */
export function generateInterviewVersions(brick, targetRoleId, cauchemars) {
  var text = (brick.text || "").trim();
  var bestNum = extractBestNum(text) || "un résultat mesurable";
  var roleData = targetRoleId && KPI_REFERENCE[targetRoleId] ? KPI_REFERENCE[targetRoleId] : null;
  var roleLabel = roleData ? roleData.role : "ce poste";
  var isScar = brick.brickType === "scar";
  var isElastic = brick.brickType === "elastic";

  // Find matching cauchemar for context
  ...
  // Extract context clues from brick text
  var summary = extractBrickSummary(text);

  // ── VERSION RH ──
  var rh = "";
  if (isScar) {
    rh += "J'ai traversé une situation difficile : " + summary.toLowerCase() + ".\n";
    ...
    rh += "Cette expérience vient d'un contexte différent : " + summary.toLowerCase() + ".\n";
    rh += "Ce qui est intéressant, c'est que la mécanique se transpose.\n";
    ...
    rh += "Ce que j'en retiens pour " + roleLabel + " : " + bestNum + " de résultat prouvé sur ce type d'enjeu.";
  }

  // ── VERSION N+1 ──
  var n1 = "";
  if (isScar) {
    n1 += "Le process en place ne fonctionnait pas. Résultat : " + summary.toLowerCase() + ".\n";
    n1 += "Diagnostic : le problème était structurel, pas conjoncturel.\n";
    n1 += "Correctif appliqué : refonte du process avec un contrôle en amont.\n";
    ...
    n1 += "Méthode appliquée : " + summary.toLowerCase() + ".\n";
    n1 += "Le mécanisme est transposable parce que le levier est le même.\n";
    n1 += "Résultat : " + bestNum + ".";
  } else {
    n1 += "Le problème : " + cauchLabel + ".\n";
    n1 += "Action : " + summary.toLowerCase() + ".\n";
    n1 += "Résultat : " + bestNum + ".\n";
    n1 += "La méthode est reproductible sur un périmètre similaire.";
  }
  ...
  if (isScar) {
    ...
    direction += "Ce type de situation se reproduit quand le process n'est pas structuré. Ma réponse serait de l'anticiper.";
  } else if (isElastic) {
    direction += "L'enjeu business est transverse : " + cauchLabel + ".\n";
    direction += "La méthode testée dans un autre contexte a produit " + bestNum + ".\n";
    direction += "Le delta est réplicable sur votre périmètre.\n";
    direction += "Ce type de situation se reproduit à chaque changement de segment. Ma réponse serait d'appliquer le même cadre.";
  } else {
    direction += "L'enjeu P&L : " + cauchLabel + (costRef ? " — coût sectoriel : " + costRef : "") + ".\n";
    direction += "Décision prise : " + summary.toLowerCase() + ".\n";
    direction += "Impact mesuré : " + bestNum + ".\n";
    direction += "Ce type de situation se reproduit quand le marché se tend. Ma réponse serait la même méthode, calibrée au contexte.";
  }
...
export function generateReplacementReport(bricks, targetRoleId, currentSalary, internalSignals) {
  ...
  // Component 4 — Risque cauchemars couverts par le candidat
  var cauchemars = getActiveCauchemars();
  var cauchCost = 0;
  var coveredCauchs = [];
  cauchemars.forEach(function(c) {
    ...
    if (covers.length > 0) {
      ...
      cauchCost += avgCost;
      coveredCauchs.push({ label: c.label, cost: avgCost });
    }
  });
 ...
 */
export function generateInterviewQuestions(bricks, targetRoleId, nightmares, offerSignals, signature) {
  var validated = bricks.filter(function(b) { return b.status === "validated" && b.brickType !== "take"; });
  ...
  // Compléter avec cauchemars si pas assez de signaux
  if (questions.length < 2) {
    ...
    cauchWithBrick.forEach(function(cw) {
      ...
      var questionText = "Sur un rôle " + roleLabel + ", " + cw.cauchemar.label.toLowerCase() + " est souvent un enjeu. Comment votre équipe le gère aujourd'hui ?";
      questions.push({
        level: 3,
        text: questionText,
  ...
  if (armored.length >= 3) {
    ...
    var bestNum = extractBestNum(bestArmored.text);
    var costCauch = cauchWithBrick.find(function(cw) { return cw.cauchemar.costRange && cw.cauchemar.costRange[1] > 0; });
    if (costCauch) {
      var costStr = formatCost(costCauch.cauchemar.costRange[0]) + " - " + formatCost(costCauch.cauchemar.costRange[1]);
      questions.push({
        level: 5,
        text: "Si " + costCauch.cauchemar.label.toLowerCase() + " coûte entre " + costStr + " par an au secteur, quel est le mécanisme que vous avez mis en place pour le contenir ? Ou c'est encore un sujet ouvert ?",
        demonstrates: "Tu chiffres le problème du recruteur avant qu'il ne le fasse. Il réalise que tu comprends l'enjeu business, pas juste le poste.",
