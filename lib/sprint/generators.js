...
var CV_ACTION_VERBS = [
  { pattern: /\bconstruit/i, label: "Construit" },
  { pattern: /\bdéployé/i, label: "Déployé" },
  { pattern: /\bréduit/i, label: "Réduit" },
  { pattern: /\bcréé/i, label: "Créé" },
  { pattern: /\bformé/i, label: "Formé" },
  { pattern: /\binstauré/i, label: "Instauré" },
  { pattern: /\bimposé/i, label: "Imposé" },
  { pattern: /\bdécoupé/i, label: "Découpé" },
  { pattern: /\béliminé/i, label: "Éliminé" },
  { pattern: /\brécupéré/i, label: "Récupéré" },
  { pattern: /\bconçu/i, label: "Conçu" },
  { pattern: /\bstructuré/i, label: "Structuré" },
  { pattern: /\bdirigé/i, label: "Dirigé" },
  { pattern: /\blancé/i, label: "Lancé" },
  { pattern: /\bnégocié/i, label: "Négocié" },
  { pattern: /\btransformé/i, label: "Transformé" },
  { pattern: /\brestructuré/i, label: "Restructuré" },
  { pattern: /\bpiloté/i, label: "Piloté" },
  { pattern: /\boptimisé/i, label: "Optimisé" },
  { pattern: /\bautomatisé/i, label: "Automatisé" },
];

export function generateCVLine(brick) {
  var text = brick.text || "";
  var num = extractBestNum(text);
  var verb = null;
  for (var i = 0; i < CV_ACTION_VERBS.length; i++) {
    if (CV_ACTION_VERBS[i].pattern.test(text)) { verb = CV_ACTION_VERBS[i].label; break; }
  }
  var scope = brick.kpi || "";
  if (!scope) {
    var scopeMatch = text.match(/(?:le|la|les|un|une|des|du|l')\s+([A-ZÀ-Ü][a-zà-ü]+(?:\s+[a-zà-ü]+){0,3})/);
    if (scopeMatch) scope = scopeMatch[1];
  }
  var parts = [];
  if (verb) parts.push(verb);
  if (scope) parts.push(scope.length > 40 ? scope.slice(0, 37) + "..." : scope);
  if (num) { parts.push("— " + num); }
  var line = parts.join(" ");
  if (!line || line.length < 5) line = text;
  return line.length > 90 ? line.slice(0, 87) + "..." : line;
}

function extractBestNum(text) {
