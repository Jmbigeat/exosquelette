...
export function detectDiltsLevel(text) {
  ...
  // Chantier 19 — confidence: 1 marker = 0.3, 2 = 0.6, 3+ = 1.0
  var confidence = maxScore >= 3 ? 1.0 : maxScore === 2 ? 0.6 : maxScore === 1 ? 0.3 : 0;
  var label = getDiltsLabel(dominant);
...
export function getDiltsLabel(level) {
  var d = DILTS_LEVELS.find(function (l) {
...
export function checkDiltsSequence(posts) {
  ...
  var levels = last3.map(function (p) {
    return p.diltsLevel || 1;
  });
  ...
  if (allSame) {
    ...
    return {
      ...
      name: d.name,
      message:
        "Tes 3 derniers posts sont au niveau " +
        levels[0] +
        " (" +
        d.name +
        "). Ta séquence stagne. Monte d'un niveau.",
    };
  }
...
/**
 * detectDiltsStagnation — Chantier 19
 * Detects stagnation in Dilts progression across posts.
 * Stagnation = 3 consecutive posts at the same level.
 * @param {Array} posts - generated posts with diltsLevel
 * @returns {{ stagnating: boolean, level: number|null, message: string|null }}
 */
export function detectDiltsStagnation(posts) {
  if (!posts || posts.length < 3) return { stagnating: false, level: null, message: null };
  var last3 = posts.slice(-3);
  var levels = last3.map(function (p) {
    return p.diltsLevel || 1;
  });
  if (levels[0] === levels[1] && levels[1] === levels[2]) {
    var lvl = levels[0];
    if (lvl >= 6) return { stagnating: false, level: null, message: null };
    var messages = {
      1: "Tu restes dans le contexte. Montre ce que tu as fait, pas seulement où.",
      2: "Tu empiles les actions. Explique ta méthode, pas seulement le résultat.",
      3: "Tu décris le comment. Prends position sur le pourquoi.",
      4: "Tu donnes ton avis. Assume qui tu es.",
      5: "Tu te positionnes. Parle de l'impact sur les autres.",
    };
    var d = getDiltsLabel(lvl);
    return {
      stagnating: true,
      level: lvl,
      message: "Ta séquence stagne au niveau " + lvl + " (" + d.name + "). " + (messages[lvl] || "Monte d'un niveau."),
    };
  }
  return { stagnating: false, level: null, message: null };
}
...
export var DILTS_CALIBRATION = {
  ...
  1: {
    brickPriority: ["chiffre"],
    framingOpen: "",
    framingClose: "Les chiffres ne mentent pas. Ceux-là sont les miens.",
  },
  2: {
    brickPriority: ["chiffre", "influence"],
    framingOpen: "",
    framingClose: "Ce que j'ai fait parle. Le reste est du bruit.",
  },
  3: {
    brickPriority: ["decision", "influence"],
    framingOpen: "Ma méthode est reproductible. ",
    framingClose: "Le cadre compte autant que le talent. C'est lui qui tient sous pression.",
  },
  4: {
    brickPriority: ["cicatrice", "decision"],
    framingOpen: "",
    framingClose: "Le consensus dit le contraire. Mon expérience dit ça.",
  },
  5: {
    brickPriority: ["cicatrice", "decision"],
    framingOpen: "",
    framingClose: "Ce n'est pas ce que je fais. C'est ce qui me définit.",
  },
};
...
export function computeDiltsTarget(diltsHistory) {
  if (!diltsHistory || diltsHistory.length === 0) {
    return {
      targetLevel: 2,
      reason: "Premier post. On ancre sur du concret — ce que tu fais.",
      postsAtCurrent: 0,
      sequencePosition: 0,
      completed: 0,
    };
...
export function selectBrickForDiltsTarget(bricks, targetLevel, usedBrickIds) {
  var available = bricks.filter(function (b) {
    return b.status === "validated" && b.brickType !== "take" && usedBrickIds.indexOf(b.id) === -1;
  });
  if (available.length === 0)
    available = bricks.filter(function (b) {
...
export var DILTS_EDITORIAL_MAPPING = {
  2: {
    registre: "Ce que j'ai fait",
    sujets: "projet mené, résultat obtenu, situation gérée",
    brickTypes: ["chiffre", "cicatrice"],
    prospectPerception: "Le prospect enregistre un praticien.",
  },
  3: {
    registre: "Comment je le fais",
    sujets: "méthode appliquée, framework utilisé, processus construit",
    brickTypes: ["chiffre", "decision"],
    prospectPerception: "Le prospect apprend de toi. Tu te distingues.",
  },
  4: {
    registre: "Ce que je crois et pourquoi",
    sujets: "conviction née de l'expérience, désaccord avec le consensus",
    brickTypes: ["cicatrice", "decision"],
    prospectPerception: "Le prospect a une raison de se souvenir de toi. Tu occupes un territoire.",
  },
  5: {
    registre: "Qui je suis dans cet écosystème",
    sujets: "rôle dans la conversation sectorielle, définition du métier",
    brickTypes: ["cicatrice", "decision"],
    prospectPerception: "Le prospect associe un problème à ton nom. Tu es le raccourci mental.",
  },
};
