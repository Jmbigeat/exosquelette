...
/**
 * Extracts structural elements from a brick.
 * Uses brick.fields.result directly when available (new bricks),
 * falls back to text heuristic for legacy bricks without .fields.
 *
 * @param {object} brick - validated brick (with .text, optionally .fields)
 * @returns {{
...
export function extractBrickCore(brick) {
  if (!brick || !brick.text)
    return {
      ...
      constraint: null,
      kpiLabel: null,
      brickType: brick ? brick.brickType || "proof" : "proof",
    };
  ...
  // Fast path: if brick has structured fields with a result field, extract number from it directly.
  // This is more reliable than the midpoint heuristic on concatenated text.
  // Legacy bricks (from Supabase) may not have .fields — fallback below handles them.
  if (brick.fields && brick.fields.result) {
    var resultField = brick.fields.result;
    // Extract "de X à Y" from result field
    var fieldDeA = resultField.match(
      /(?:de|passé de|porté de|réduit de|augmenté de)\s+([\d,.\s]+[%K€M]*)\s+[àa]\s+([\d,.\s]+[%K€M]*)/i
    );
    if (fieldDeA) {
      problemNumber = fieldDeA[1].trim();
      resultNumber = fieldDeA[2].trim();
      mainNumber = resultNumber;
    }
    // Extract delta (+X% / -X%)
    if (!mainNumber) {
      var fieldDelta = resultField.match(/([+\-]\d[\d,.\s]*%)/);
      if (fieldDelta) {
        mainNumber = fieldDelta[1].trim();
        resultNumber = mainNumber;
      }
    }
    // Extract any number with suffix
    if (!mainNumber) {
      var fieldNums = resultField.match(
        /([\d][\d\s.,]*\s*(?:K€|M€|k€|€|%|mois|semaines|comptes|AE|commerciaux|personnes|clients|projets|jours|lignes?|tests?|rôles?|modules?))/gi
      );
      if (fieldNums && fieldNums.length > 0) {
        resultNumber = fieldNums[fieldNums.length - 1].trim();
        mainNumber = resultNumber;
      }
    }
    // Extract raw number from result field
    if (!mainNumber) {
      var fieldRaw = resultField.match(/\d[\d\s.,]*/g);
      if (fieldRaw) {
        var best = null;
        var bestVal = 0;
        fieldRaw.forEach(function (n) {
    ...
    // If result field had a number, also try to get problemNumber from situation field
    if (mainNumber && brick.fields.situation) {
      var sitNums = brick.fields.situation.match(
        /([\d][\d\s.,]*\s*(?:K€|M€|k€|€|%|mois|semaines|comptes|AE|commerciaux|personnes|clients|projets|jours|lignes?|tests?|rôles?|modules?))/gi
      );
      if (sitNums && sitNums.length > 0 && !problemNumber) {
        problemNumber = sitNums[0].trim();
      }
    }
  }

  // Heuristic fallback: parse from concatenated brick.text (legacy bricks without .fields)
  // Pattern "passé de X à Y" or "de X à Y"
  var deAMatch = !mainNumber
    ? text.match(/(?:passé|porté|réduit|augmenté|amélioré)\s+de\s+([\d,.\s]+[%K€M]*)\s+[àa]\s+([\d,.\s]+[%K€M]*)/i)
    : null;
  if (deAMatch) {
  ...
  if (!mainNumber && allNums.length > 0) {
    // Split by text position: first half = situation/problem, second half = result
    var midpoint = Math.floor(text.length / 2);
    var firstHalfNums = [];
    var secondHalfNums = [];
    allNums.forEach(function (num) {
      var pos = text.indexOf(num);
      if (pos < midpoint) firstHalfNums.push(num.trim());
      else secondHalfNums.push(num.trim());
    });
    // Prefer second-half numbers as results
    if (secondHalfNums.length > 0) {
      resultNumber = secondHalfNums[secondHalfNums.length - 1];
      mainNumber = resultNumber;
      if (firstHalfNums.length > 0) problemNumber = firstHalfNums[0];
    } else {
      // All numbers in first half — last one is most likely result
...
export function formatAnchorLine(core) {
  ...
  if (line.length > 210) {
    // Step 1: drop constraint from context if embedded
    if (core.constraint && core.context && core.context.indexOf(core.constraint.slice(0, 15)) !== -1) {
      var ctxClean = core.context.replace(core.constraint, "").replace(/\s+/g, " ").trim();
      var p1 = [];
      if (core.actionVerb) p1.push(core.actionVerb.charAt(0).toUpperCase() + core.actionVerb.slice(1));
      if (core.problemNumber && core.resultNumber && core.problemNumber !== core.resultNumber) {
        p1.push("de " + core.problemNumber + " à " + core.resultNumber);
      } else if (core.mainNumber) {
        p1.push(core.mainNumber);
      }
      if (ctxClean) p1.push(ctxClean);
      line = p1.join(" ").replace(/\s+/g, " ").trim();
    }
  }
  if (line.length > 210) {
    // Step 2: shorten context to 10 words max
    var ctx10 = core.context
      ? core.context.split(/\s+/).slice(0, 10).join(" ")
      : "";
    var p2 = [];
    if (core.actionVerb) p2.push(core.actionVerb.charAt(0).toUpperCase() + core.actionVerb.slice(1));
    if (core.problemNumber && core.resultNumber && core.problemNumber !== core.resultNumber) {
      p2.push("de " + core.problemNumber + " à " + core.resultNumber);
    } else if (core.mainNumber) {
      p2.push(core.mainNumber);
    }
    if (ctx10) p2.push(ctx10);
    line = p2.join(" ").replace(/\s+/g, " ").trim();
  }
  if (line.length > 210) {
    // Step 3: verb + number + 5-word context — number and verb never sacrificed
    var ctx5 = core.context
      ? core.context.split(/\s+/).slice(0, 5).join(" ")
      : "";
    var p3 = [];
    if (core.actionVerb) p3.push(core.actionVerb.charAt(0).toUpperCase() + core.actionVerb.slice(1));
    if (core.mainNumber) p3.push(core.mainNumber);
    if (ctx5) p3.push(ctx5);
    line = p3.join(" ").replace(/\s+/g, " ").trim();
