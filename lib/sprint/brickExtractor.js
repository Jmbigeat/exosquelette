...
export function extractBrickCore(brick) {
  ...
  if (brick.fields && brick.fields.result) {
    ...
    var fieldDeA = resultField.match(/(?:de|passé de|porté de|réduit de|augmenté de)\s+([\d,.\s]+[%K€M]*)\s+[àa]\s+([\d,.\s]+[%K€M]*)/i);
    if (fieldDeA) {
    ...
    if (!mainNumber) {
      ...
      if (fieldDelta) { mainNumber = fieldDelta[1].trim(); resultNumber = mainNumber; }
    }
    // Extract any number with suffix
    if (!mainNumber) {
      var fieldNums = resultField.match(/([\d][\d\s.,]*\s*(?:K€|M€|k€|€|%|mois|semaines|comptes|AE|commerciaux|personnes|clients|projets|jours|lignes?|tests?|rôles?|modules?))/gi);
  ...
  var deAMatch = !mainNumber ? text.match(/(?:passé|porté|réduit|augmenté|amélioré)\s+de\s+([\d,.\s]+[%K€M]*)\s+[àa]\s+([\d,.\s]+[%K€M]*)/i) : null;
  if (deAMatch) {
  ...
  var allNums = text.match(/([\d][\d\s.,]*\s*(?:K€|M€|k€|€|%|mois|semaines|comptes|AE|commerciaux|personnes|clients|projets|jours|lignes?|tests?|rôles?|modules?))/gi) || [];
