...
export var SEED_TEMPLATES = {
  chiffre_1: {
    ...
    question: function(kpi, hasMention) { return (hasMention ? "Tu mentionnes une activité liée à " : "Le recruteur cherche un chiffre sur ") + kpi.name.toLowerCase() + ". Quel indicateur a bougé ? De combien ? En combien de temps ?"; },
    ...
    missionText: function(kpi) { return "Tu n'as pas de chiffre sur " + kpi.name.toLowerCase() + ". Vérifie tes anciens outils (CRM, reporting, dashboard). Cherche le delta avant/après ton intervention. Reviens avec le chiffre."; },
  ...
  chiffre_2: {
    brickCategory: "chiffre", type: "preuve",
    question: function(kpi) { return "Quel processus as-tu changé ou construit qui a eu un impact mesurable sur " + kpi.name.toLowerCase() + " ?"; },
  ...
  chiffre_5: {
    ...
    nightmareGen: function(kpi) { return "L'offre mentionne " + kpi.name.toLowerCase() + ". Le décideur veut voir un impact prouvé."; },
  },
  decision: {
