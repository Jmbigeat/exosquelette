...
export function estimateReadiness(cvText, offersText) {
  ...
  var methodMarkers = ["mis en place", "deploye", "structure", "construit", "lance", "cree", "restructure", "implemente", "optimise", "pilote", "stress-test", "scoring", "framework", "stack", "architecture", "pipeline", "workflow", "sprint", "extraction", "diagnostic", "audit", "grille", "matrice", "algorithme", "heuristique", "calibration", "matching", "parsing", "template-engine"];
...
export function hasInfluenceMarkers(text) {
  var alignmentWords = ["aligne", "convaincu", "obtenu l'accord", "debloque", "fait valider", "rallie", "consensus", "accepte", "convaincu d'utiliser", "premier utilisateur", "premier client", "testé par", "adopté par", "recommandé par", "early adopter", "beta testeur", "converti"];
...
export function analyzeVerbs(text) {
  ...
  var contribueImpact = /contribu[eé]\s+[àa]\s+(?:changer|transformer|r[eé]duire|lancer|construire|cr[eé]er|d[eé]ployer|am[eé]liorer|augmenter|doubler|tripler|[eé]liminer)/i;
  ...
  processVerbs.forEach(function(v) { if (matchWord(lower, v) && !isExcluded(v, lower)) { p++; foundProcess.push(v); } });
...
export function extractBrickSummary(text) {
  ...
  function sentencesOf(blockList) {
    ...
    blockList.forEach(function(bl) {
      bl.split(/(?:\.\s|[!?]\s?)/).map(function(s) { return s.trim(); }).filter(function(s) { return s.length > 5; }).forEach(function(s) { result.push(s); });
    });
    return result;
  }
  ...
  var best = tailSentences.find(function(s) { return hasNum.test(s) && actionVerbs.test(s); });
  if (!best) best = headSentences.find(function(s) { return hasNum.test(s) && actionVerbs.test(s); });
  if (!best) best = tailSentences.find(function(s) { return hasNum.test(s); });
  if (!best) best = headSentences.find(function(s) { return hasNum.test(s); });
  if (!best) best = tailSentences.find(function(s) { return actionVerbs.test(s); });
  if (!best) best = headSentences.find(function(s) { return actionVerbs.test(s); });
  if (!best) {
