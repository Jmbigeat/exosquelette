...
export function generateMaskedHypotheses(bricks, nightmares) {
  ...
  var sortedKeywords = Object.keys(transferKeywords)
    .sort(function (a, b) {
  ...
  var hyp1Sources = [
    ...
    "ratio réactif/proactif: " + Math.round(meta.reactiveRatio * 100) + "/" + Math.round(meta.proactiveRatio * 100),
  ];
  if (coveredNightmares.length > 0) hyp1Sources.push("cauchemars neutralisés: " + coveredNightmares.length);

  var hypothesis1 = {
    text:
      "Ta capacité distinctive est de " +
      archetypeDesc +
      ". " +
      (meta.tempo === "court" ? "Tu interviens vite — tempo court." : "Tu construis sur la durée — tempo long."),
  ...
  var nightmareLabels = coveredNightmares
    .map(function (n) {
      return n.label || n.nightmareShort;
    })
    .join(" et ");

  var hyp2Text =
    coveredNightmares.length > 0
      ? "Tu neutralises " +
        nightmareLabels +
        " par une approche " +
        (meta.modifier === "transverse" ? "transverse" : "directe") +
        (topKeywords ? " centrée sur " + topKeywords : "") +
        "."
      : "Ta méthode " +
        (meta.modifier === "transverse" ? "transverse" : "directe") +
        (topKeywords ? " autour de " + topKeywords : "") +
        " produit des résultats que les profils classiques ne répliquent pas.";

  var hypothesis2 = {
    text: hyp2Text,
    confidence: Math.min(85, 40 + coveredNightmares.length * 15 + (topKeywords ? 10 : 0)),
    sources: [
      "cauchemars: " + (nightmareLabels || "aucun"),
      "mots-clés transférabilité: " + (topKeywords || "aucun"),
      "modificateur: " + meta.modifier,
    ],
  };
...
export function computeMetaPatterns(bricks) {
  ...
  bricks.forEach(function (b) {
      ...
      ["angle1", "angle2", "angle3", "angle4", "angle5"].forEach(function (a) {
        ...
        if (st && st.response) {
          Object.keys(st.response).forEach(function (k) {
            if (st.response[k]) fullText += " " + st.response[k].toLowerCase();
          });
        }
      });
    }
    ...
    reactiveMarkers.forEach(function (m) {
      if (fullText.indexOf(m) !== -1) reactiveSignals++;
    });
    proactiveMarkers.forEach(function (m) {
      if (fullText.indexOf(m) !== -1) proactiveSignals++;
    });

    // Direct vs transverse
    VERBS_DIRECT.forEach(function (v) {
      if (fullText.indexOf(v) !== -1) directVerbs++;
    });
    VERBS_TRANSVERSE.forEach(function (v) {
      if (fullText.indexOf(v) !== -1) transverseVerbs++;
    });
...
export function crossReferenceSignature(candidateResponse, hypotheses) {
  ...
  var responseWords = responseLower
    .split(/\s+/)
    .filter(function (w) {
      return w.length > 3;
    })
  ...
  var scores = hypotheses.map(function (hyp, idx) {
    ...
    responseWords.forEach(function (rw) {
      if (
        hypWords.some(function (hw) {
          return hw === rw || hw.indexOf(rw) !== -1 || rw.indexOf(hw) !== -1;
        })
      ) {
        matchCount++;
      }
    ...
    Object.keys(conceptMarkers).forEach(function (concept) {
      var inResponse = conceptMarkers[concept].some(function (m) {
        return responseLower.indexOf(m) !== -1;
      });
      var inHypothesis = conceptMarkers[concept].some(function (m) {
        return hyp.text.toLowerCase().indexOf(m) !== -1;
      });
  ...
  if (bestScore >= 2) {
    return {
      ...
      diagnostic:
        "Convergence partielle. Ta réponse croise une partie de ce que tes briques révèlent. L'écart entre ce que tu dis et ce que tes preuves montrent est lui-même un signal.",
    };
  }
  return {
    type: "divergence",
    ...
    diagnostic:
      "Ce que tu décris ne recoupe pas ce que tes briques prouvent. L'écart n'est pas un problème — c'est une information. Choisis la force que tu veux armer.",
  };
}
...
export function validateSignatureFormulation(formulation, bricks) {
  ...
  var hasContext = contextMarkers.some(function (m) {
    return textLower.indexOf(m) !== -1;
  });
  ...
  var formulationWords = textLower
    .split(/\s+/)
    .filter(function (w) {
      return w.length > 4;
    })
  ...
  if (failedFilters.length > 0) {
    var parts = [];
    if (failedFilters.indexOf("densité") !== -1)
      parts.push("Ajoute un chiffre ou un contexte précis (marché, équipe, durée).");
    if (failedFilters.indexOf("cohérence") !== -1)
      parts.push("Utilise au moins un terme qui apparaît dans tes briques blindées.");
...
export function isSignatureArmored(formulation, bricks) {
  ...
  var hasNonReplic = nonReplicMarkers.some(function (m) {
    return text.indexOf(m) !== -1;
  });
...
/**
 * Teste si une brique "nourrit" la Signature du candidat.
 * Heuristique : au moins 1 mot-clé (>3 chars) de la formulation ou des metaPatterns
 * apparaît dans le texte de la brique.
 *
 * Faux négatifs acceptables (brique pertinente non détectée = pas signalée orpheline).
 * Faux positifs à minimiser (brique dans la Signature signalée orpheline = confusant).
 *
 * @param {object} brick
 * @param {object} signature - { formulation, metaPatterns }
 * @returns {boolean}
 */
export function brickFeedsSignature(brick, signature) {
  if (!signature || !signature.formulation) return true;
  var text = ((brick.editText || brick.text || "") + " " + (brick.kpi || "")).toLowerCase();

  // Extract keywords from formulation
  var formulationWords = signature.formulation
    .toLowerCase()
    .split(/\s+/)
    .map(function (w) {
      return w.replace(/[.,;:!?'"()]/g, "");
  ...
  // Add keywords from metaPatterns (archetype, modifier)
  var meta = signature.metaPatterns || {};
  if (meta.archetype) formulationWords.push(meta.archetype);

  return formulationWords.some(function (kw) {
    return text.indexOf(kw) !== -1;
  });
}

/**
 * Détecte les briques blindées 4/4 qui ne nourrissent pas la Signature.
 * Retourne un array vide si pas de Signature ou si toutes les briques 4/4 sont alignées.
 *
 * @param {Array} bricks
 * @param {object|null} signature
 * @returns {Array} briques orphelines
 */
export function detectOrphanArmoredBricks(bricks, signature) {
  if (!signature || !signature.formulation) return [];
  var armored = (bricks || []).filter(function (b) {
    return b.status === "validated" && (b.armorScore || 0) === 4;
  });
  return armored.filter(function (b) {
    return !brickFeedsSignature(b, signature);
  });
}
