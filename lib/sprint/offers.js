...
export function parseInternalSignals(internalJobDescription, targetRoleId) {
  ...
  var lower = internalJobDescription.toLowerCase()
    .replace(/[éèê]/g, "e").replace(/[àâ]/g, "a")
    .replace(/[ùû]/g, "u").replace(/[ôö]/g, "o").replace(/[îï]/g, "i");

  var signalDefs = [
    {
      id: "objectifs_non_atteints",
      label: "Objectifs non atteints",
      keywords: ["objectif", "target", "kpi", "resultat", "sous-performance", "sous performance", "en dessous", "retard", "ecart", "gap", "non atteint", "manque", "insuffisan"],
      leverage: "Le manager sait que les objectifs ne sont pas atteints. Il cherche quelqu'un qui corrige sans qu'il ait à micro-manager.",
    },
    {
      id: "surcharge_equipe",
      label: "Surcharge équipe",
      keywords: ["charge", "capacite", "heures sup", "heure supplementaire", "turnover", "burn", "surcharge", "sous-effectif", "sous effectif", "epuise", "fatigue", "saturation", "deborde"],
      leverage: "L'équipe craque. Le manager cherche un relais. Ton arrivée ou ta montée en compétence réduit la pression immédiate.",
    },
    {
      id: "departs_recents",
      label: "Départs récents",
      keywords: ["depart", "demission", "remplacement", "vacance", "poste vacant", "quitte", "parti", "perdu", "turnover", "attrition", "fuite"],
      leverage: "Des départs fragilisent l'équipe. Le manager paie le coût de remplacement. Ta rétention vaut plus que ton salaire.",
    },
    {
      id: "restructuration",
      label: "Restructuration interne",
      keywords: ["reorg", "fusion", "transformation", "changement", "restructur", "reorganis", "nouvelle orga", "transition", "migration", "pivot"],
      leverage: "En période de restructuration, la stabilité a un prix. Le manager perd des repères. Tu es un point fixe.",
    },
    {
      id: "projet_critique",
      label: "Projet critique",
      keywords: ["deadline", "urgent", "prioritaire", "critique", "livraison", "date limite", "imperatif", "bloque", "retard projet", "sprint", "release", "go live", "mise en production"],
      leverage: "Un projet critique dépend de toi. Si tu pars, la deadline explose. Le coût du retard dépasse le coût de ta demande.",
    },
  ];
...
export function aggregateOfferSignals(offers, roleId) {
  ...
  var external = offers.filter(function(o) { return o.type !== "internal"; });
...
export function detectSectoralDispersion(offers) {
  ...
  if (sectorList.length >= 2) {
    return { sectors: sectorList, message: "Tes offres couvrent " + sectorList.length + " secteurs différents (" + sectorList.join(", ") + "). Ta densité risque de se diluer." };
  }
  return null;
}
