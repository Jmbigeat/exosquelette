import { CAUCHEMAR_TEMPLATES_BY_ROLE, TRANSVERSAL_CAUCHEMARS, OFFER_URGENCY_KEYWORDS, SECTOR_KEYWORDS, KPI_REFERENCE } from "./references.js";
...
/**
 * Returns transversal cauchemar templates applicable to the given role.
 */
function getTransversalsForRole(roleId) {
  return TRANSVERSAL_CAUCHEMARS.filter(function (c) {
    return c.applicableRoles === "all" ||
      (Array.isArray(c.applicableRoles) && c.applicableRoles.indexOf(roleId) !== -1);
  });
}
...
export function parseOfferSignals(offersText, roleId) {
  ...
  var lower = offersText
    ...
    .replace(/[ôö]/g, "o")
    .replace(/[îï]/g, "i");
  var roleTemplates = CAUCHEMAR_TEMPLATES_BY_ROLE[roleId] || CAUCHEMAR_TEMPLATES_BY_ROLE.enterprise_ae;
  var templates = roleTemplates.concat(getTransversalsForRole(roleId));
...
export function buildActiveCauchemars(parsedOffers, roleId) {
  if (parsedOffers && parsedOffers.cauchemars && parsedOffers.cauchemars.length >= 3) {
    // Append transversal cauchemars not already in parsed results
    var base = parsedOffers.cauchemars.slice();
    var baseLabels = {};
    base.forEach(function (c) { baseLabels[c.label] = true; });
    var nextId = base.length + 1;
    getTransversalsForRole(roleId).forEach(function (t) {
      if (!baseLabels[t.label]) {
        var costStr = formatCost(t.cost[0]) + "-" + formatCost(t.cost[1]);
        base.push({
          id: nextId++,
          label: t.label,
  ...
  // Fallback: generate from role templates + transversals without offer matching
  var roleTemplates = CAUCHEMAR_TEMPLATES_BY_ROLE[roleId] || CAUCHEMAR_TEMPLATES_BY_ROLE.enterprise_ae;
  var templates = roleTemplates.concat(getTransversalsForRole(roleId));
