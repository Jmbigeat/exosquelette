...
export default function SprintPage() {
  ...
  var handleStateChange = useCallback(
    function (state) {
      if (!user || user.id === "dev") return;
      ...
      saveTimerRef.current = setTimeout(function () {
        ...
        function trySave() {
          saveSprint(user.id, sprintId, state)
            .then(function (res) {
              ...
              saveStatusTimerRef.current = setTimeout(function () {
                setSaveStatus(null);
              }, 2000);
            })
            .catch(function () {
              ...
              if (attempt <= 3) {
                ...
                try {
                  localStorage.setItem("offline_bricks_backup", JSON.stringify(state));
                } catch (e) {}
              }
            });
        }
    ...
    <div style={wrapSprint}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        ...
        <button
          ...
          style={{ background: "none", border: "none", color: "#495670", fontSize: 11, cursor: "pointer" }}
        >
          Déconnexion
        </button>
      </div>
